...
void esp_nn_fully_connected_s8_test() {
  ...
  for (int itr = 0; itr < 15; itr++) {
    ...
    /* C function */
    esp_nn_fully_connected_s8_ansi(input, input_offset, row_len, filter_data, filter_offset, NULL, output_c,
                                   out_channels, out_offset, out_shift, out_mult, activation_min, activation_max);

    total_c = profile_c_end();
    profile_opt_start();

    /* Optimized function */
    esp_nn_fully_connected_s8(input, input_offset, row_len, filter_data, filter_offset, NULL, output_opt, out_channels,
    ...
    printf(ANSI_COLOR_GREEN "[%3d] passed [row_len %" PRIu16 ", out_ch %" PRIu16 "]" ANSI_COLOR_RESET, itr, row_len,
           out_channels);
