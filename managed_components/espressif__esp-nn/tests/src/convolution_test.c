...
void esp_nn_depthwise_conv_s8_test() {
  ...
  for (int itr = 0; itr < 15; itr++) {
    ...
    if (bias == NULL || input == NULL || filter_data == NULL || out_data_c == NULL || out_data_opt == NULL ||
        bias == NULL) {
    ...
    dw_conv_params_t conv_params = {.in_offset = input_offset,
                                    ...
                                    .ch_mult = ch_mult,
                                    .stride = {stride_wd, stride_ht},
                                    .padding = {pad_wd, pad_ht},
                                    .dilation = {0, 0},
    ...
    /* C function */
    esp_nn_depthwise_conv_s8_ansi(&input_dims, input, &filter_dims, filter_data + 4, bias + 1, &output_dims, out_data_c,
    ...
    /* Optimized function */
    esp_nn_depthwise_conv_s8(&input_dims, input, &filter_dims, filter_data + 4, bias + 1, &output_dims, out_data_opt,
                             &conv_params, &quant_data);
    ...
    if (ret == false) {
      printf(ANSI_COLOR_RED "[%3d] failed [pad: (%d, %d), stride: (%d, %d)"
             ...
             itr, pad_wd, pad_ht, stride_wd, stride_ht, out_wd, out_ht, filter_wd, filter_ht, channels, ch_mult);
#if 0
            printf("Output: \n");
...
void esp_nn_conv_s8_test() {
  ...
  for (int itr = 0; itr < 15; itr++) {
    ...
    if (input == NULL || filter_data == NULL || out_data_c == NULL || out_data_opt == NULL) {
    ...
    /* C function */
    esp_nn_conv_s8_ansi(&input_dims, input, &filter_dims, filter_data, bias, &output_dims, out_data_c, &conv_params,
                        &quant_data);
    ...
    /* Optimized function */
    esp_nn_conv_s8(&input_dims, input, &filter_dims, filter_data, bias, &output_dims, out_data_opt, &conv_params,
                   &quant_data);
    ...
    if (ret == false) {
      printf(ANSI_COLOR_RED "[%3d] failed [pad: (%d, %d), stride: (%d, %d)"
             ...
             itr, pad_wd, pad_ht, stride_wd, stride_ht, out_wd, out_ht, out_channels, filter_wd, filter_ht,
             in_channels);
#if 0
            printf("Output: \n");
            PRINT_ARRAY_HEX(out_data_opt, out_size / out_ht, out_ht);
    ...
    printf(ANSI_COLOR_GREEN "[%3d] passed [pad: (%d, %d), stride: (%d, %d)"
           ...
           itr, pad_wd, pad_ht, stride_wd, stride_ht, out_wd, out_ht, out_channels, filter_wd, filter_ht, in_channels);
    printf("\tcycles: c %8" PRIu32 ", opt %8" PRIu32 "\n", total_c, total_opt);
