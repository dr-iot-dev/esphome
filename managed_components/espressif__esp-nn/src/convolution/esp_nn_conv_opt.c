...
__attribute__((noinline)) static void esp_nn_conv_s8_1x1(const data_dims_t *input_dims, const int8_t *input_data,
...
void esp_nn_conv_s8_opt(const data_dims_t *input_dims, const int8_t *input_data, const data_dims_t *filter_dims,
  ...
  if (filter_wd == 1 && filter_ht == 1) {
    esp_nn_conv_s8_1x1(input_dims, input_data, filter_data, bias, output_dims, out_data, conv_params, quant_data);
    return;
  }
      ...
      for (out_ch_idx = 0; out_ch_idx < out_channels; out_ch_idx++) {
        ...
        for (filter_y_idx = filter_y_start; filter_y_idx < filter_y_end; filter_y_idx++) {
          for (filter_x_idx = filter_x_start; filter_x_idx < filter_x_end; filter_x_idx++) {
            ...
            const int8_t *input_ptr = input_data + (in_row * input_wd + in_col) * in_channels;
            const int8_t *filter_ptr = filter_data + out_ch_idx * in_channels * filter_ht * filter_wd +
