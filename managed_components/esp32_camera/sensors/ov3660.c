...
static int calc_sysclk(int xclk, bool pll_bypass, int pll_multiplier, int pll_sys_div, int pll_pre_div,
                       bool pll_root_2x, int pll_seld5, bool pclk_manual, int pclk_div) {
...
static int set_image_options(sensor_t *sensor) {
  ...
  if (sensor->status.binning) {
    ...
    ret = write_reg(sensor->slv_addr, 0x4520, 0xb0) || write_reg(sensor->slv_addr, X_INCREMENT, 0x11)  // odd:1, even: 1
