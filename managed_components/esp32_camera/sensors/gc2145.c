...
static int set_framesize(sensor_t *sensor, framesize_t framesize) {
  ...
  const struct subsample_cfg subsample_cfgs[] = {
      ...
      // {60, 420, 0x77, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, //1/7 // A smaller ratio brings a larger view,
      // but it reduces the frame rate
  ...
  for (; i < sizeof(subsample_cfgs) / sizeof(struct subsample_cfg); i++) {
    ...
    if ((win_w * cfg->ratio_numerator / cfg->ratio_denominator >= w) &&
      ...
      ESP_LOGI(TAG, "subsample win:%dx%d, ratio:%f", win_w, win_h,
               (float) cfg->ratio_numerator / (float) cfg->ratio_denominator);
      break;
    }
  }
