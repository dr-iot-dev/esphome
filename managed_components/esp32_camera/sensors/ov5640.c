...
static int calc_sysclk(int xclk, bool pll_bypass, int pll_multiplier, int pll_sys_div, int pre_div, bool root_2x,
  ...
  unsigned int PLL_CLK =
      pll_bypass ? (xclk)
                 : (VCO / pll_sys_div * 2 / 5);  // 5 here is 10bit mode / 2, for 8bit it should be 4 (reg 0x3034)

  unsigned int PCLK = PLL_CLK / pll_pclk_root_div / ((pclk_manual && pclk_div) ? pclk_div : 2);

  unsigned int SYSCLK = PLL_CLK / 4;

  ESP_LOGI(TAG, "Calculated XVCLK: %d Hz, REFIN: %u Hz, VCO: %u Hz, PLL_CLK: %u Hz, SYSCLK: %u Hz, PCLK: %u Hz", xclk,
           REFIN, VCO, PLL_CLK, SYSCLK, PCLK);
  return SYSCLK;
}

static int set_pll(sensor_t *sensor, bool bypass, uint8_t multiplier, uint8_t sys_div, uint8_t pre_div, bool root_2x,
  ...
  ESP_LOGI(TAG,
           "Set PLL: bypass: %u, multiplier: %u, sys_div: %u, pre_div: %u, root_2x: %u, pclk_root_div: %u, "
           "pclk_manual: %u, pclk_div: %u",
           bypass, multiplier, sys_div, pre_div, root_2x, pclk_root_div, pclk_manual, pclk_div);

  calc_sysclk(sensor->xclk_freq_hz, bypass, multiplier, sys_div, pre_div, root_2x, pclk_root_div, pclk_manual,
              pclk_div);
...
static int set_image_options(sensor_t *sensor) {
  ...
  if (write_reg(sensor->slv_addr, TIMING_TC_REG20, reg20) || write_reg(sensor->slv_addr, TIMING_TC_REG21, reg21) ||
  ...
  if (!sensor->status.binning) {
    ret = write_reg(sensor->slv_addr, 0x4520, 0x10) || write_reg(sensor->slv_addr, X_INCREMENT, 0x11)  // odd:1, even: 1
          || write_reg(sensor->slv_addr, Y_INCREMENT, 0x11);                                           // odd:1, even: 1
  } else {
    ret = write_reg(sensor->slv_addr, 0x4520, 0x0b) || write_reg(sensor->slv_addr, X_INCREMENT, 0x31)  // odd:3, even: 1
  ...
  ESP_LOGD(TAG, "Set Image Options: Compression: %u, Binning: %u, V-Flip: %u, H-Mirror: %u, Reg-4514: 0x%02x",
           sensor->pixformat == PIXFORMAT_JPEG, sensor->status.binning, sensor->status.vflip, sensor->status.hmirror,
           reg4514);
  return ret;
}
...
static int set_framesize(sensor_t *sensor, framesize_t framesize) {
  ...
  if (sensor->pixformat == PIXFORMAT_JPEG) {
    ...
    // Set PLL: bypass: 0, multiplier: sys_mul, sys_div: 4, pre_div: 2, root_2x: 0, pclk_root_div: 2, pclk_manual: 1,
    // pclk_div: 4
  } else {
...
static int set_ae_level(sensor_t *sensor, int level) {
  ...
  ret = write_reg(sensor->slv_addr, 0x3a0f, level_high) || write_reg(sensor->slv_addr, 0x3a10, level_low) ||
        write_reg(sensor->slv_addr, 0x3a1b, level_high) || write_reg(sensor->slv_addr, 0x3a1e, level_low) ||
        write_reg(sensor->slv_addr, 0x3a11, fast_high) || write_reg(sensor->slv_addr, 0x3a1f, fast_low);
...
static int set_special_effect(sensor_t *sensor, int effect) {
  ...
  ret = write_reg(sensor->slv_addr, 0x5580, regs[0]) || write_reg(sensor->slv_addr, 0x5583, regs[1]) ||
        write_reg(sensor->slv_addr, 0x5584, regs[2]) || write_reg(sensor->slv_addr, 0x5003, regs[3]);
...
static int set_sharpness(sensor_t *sensor, int level) {
  ...
  ret = write_reg_bits(sensor->slv_addr, 0x5308, 0x40, false)  // 0x40 means auto
        || write_reg(sensor->slv_addr, 0x5300, 0x10) || write_reg(sensor->slv_addr, 0x5301, 0x10) ||
        write_reg(sensor->slv_addr, 0x5302, mt_offset_1) || write_reg(sensor->slv_addr, 0x5303, mt_offset_2) ||
