...
#define WRITE_REGS_OR_RETURN(regs, regs_entry_len) \
  ret = set_regs(sensor, regs, regs_entry_len); \
  if (ret) { \
    return ret; \
  }
...
static int reset(sensor_t *sensor) {
  ...
  // ESP_LOGI(TAG, "set_reg=%0x", set_reg(sensor, 0x0100, 0xffff, 0x00)); // write 0x80 to enter test mode if you want
  // to test the sensor ESP_LOGI(TAG, "0x0100=%0x", get_reg(sensor, 0x0100, 0xffff));
  if (ret) {
