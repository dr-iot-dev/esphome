...
#define WRITE_REGS_OR_RETURN(regs) \
  ret = write_regs(slv_addr, regs); \
  if (ret) { \
    return ret; \
  }
#define WRITE_REG_OR_RETURN(reg, val) \
  ret = set_reg(sensor, reg, 0xFF, val); \
  if (ret) { \
    return ret; \
  }
#define SET_REG_BITS_OR_RETURN(reg, offset, length, val) \
  ret = set_reg_bits(sensor, reg, offset, length, val); \
  if (ret) { \
    return ret; \
  }
...
static int set_special_effect(
    sensor_t *sensor, int sleep_mode_enable)  // For sc03ags sensor, This API used for sensor sleep mode control.
{
  // Add some others special control in this API, use switch to control different funcs, such as ctrl_id.
  int ret = 0;
  SET_REG_BITS_OR_RETURN(
      0x0100, 0, 1,
      !(sleep_mode_enable & 0x01));  // 0: enable sleep mode. In sleep mode, the registers can be accessed.
  return ret;
}
...
static int set_agc_gain(sensor_t *sensor, int gain) {
  ...
  if (gain < 0x20) { /*1x ~ 2x*/
    fine_gain = gain - 16;
    coarse_gain = 0x03;
    fine_again_reg_v = ((0x01 << 4) & 0x10) | (fine_gain & 0x0f);
...
static int init_status(sensor_t *sensor) { return 0; }
