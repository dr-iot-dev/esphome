...
static int set_pll(sensor_t *sensor, bool bypass, uint8_t multiplier, uint8_t sys_div, uint8_t pre_div, bool root_2x,
                   uint8_t seld5, bool pclk_manual, uint8_t pclk_div) {
  return -1;
}
...
static int set_framesize(sensor_t *sensor, framesize_t framesize) {
  ...
  if (framesize == FRAMESIZE_QVGA) {
    ...
    ret = write_regs(
        sensor->slv_addr,
        sensor_framesize_VGA_xyskip);  // Resolution:640*360 This configuration is equally-scaled without deforming
...
static int set_wb_mode(sensor_t *sensor, int mode) {
  ...
  switch (mode) {
    case 1:  // Sunny
      ret = write_reg16(sensor->slv_addr, 0x3290, 0x01) || write_reg16(sensor->slv_addr, 0x3291, 0x38) ||
            write_reg16(sensor->slv_addr, 0x3296, 0x01) || write_reg16(sensor->slv_addr, 0x3297, 0x68) ||
    ...
    case 2:  // Cloudy
      ...
      ret = write_reg16(sensor->slv_addr, 0x3290, 0x01) || write_reg16(sensor->slv_addr, 0x3291, 0x51) ||
            write_reg16(sensor->slv_addr, 0x3296, 0x01) || write_reg16(sensor->slv_addr, 0x3297, 0x00) ||
            write_reg16(sensor->slv_addr, 0x3060, 0x01);
      break;

    case 3:  // INCANDESCENCE]
      ret = write_reg16(sensor->slv_addr, 0x3290, 0x01) || write_reg16(sensor->slv_addr, 0x3291, 0x30) ||
            write_reg16(sensor->slv_addr, 0x3296, 0x01) || write_reg16(sensor->slv_addr, 0x3297, 0xCB) ||
            write_reg16(sensor->slv_addr, 0x3060, 0x01);
      break;

    case 4:  // FLUORESCENT
      ret = write_reg16(sensor->slv_addr, 0x3290, 0x01) || write_reg16(sensor->slv_addr, 0x3291, 0x70) ||
            write_reg16(sensor->slv_addr, 0x3296, 0x01) || write_reg16(sensor->slv_addr, 0x3297, 0xFF) ||
            write_reg16(sensor->slv_addr, 0x3060, 0x01);
      break;
...
static int set_special_effect(sensor_t *sensor, int effect) {
  ...
  ret = write_reg(sensor->slv_addr, 0x32F1, regs[0]) || write_reg(sensor->slv_addr, 0x32F4, regs[1]) ||
        write_reg(sensor->slv_addr, 0x32F5, regs[2]) || write_reg(sensor->slv_addr, 0x3060, regs[3]);
...
static int get_denoise(sensor_t *sensor) { return (read_reg(sensor->slv_addr, 0x5306) / 4) + 1; }
