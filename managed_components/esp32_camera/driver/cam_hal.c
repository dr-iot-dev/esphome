...
static esp_err_t cam_dma_config(const camera_config_t *config) {
  ...
  cam_obj->frames = (cam_frame_t *) heap_caps_aligned_calloc(
      alignof(cam_frame_t), 1, cam_obj->frame_cnt * sizeof(cam_frame_t), MALLOC_CAP_DEFAULT);
  ...
  for (int x = 0; x < cam_obj->frame_cnt; x++) {
    ...
    if (cam_obj->psram_mode) {
      ...
      cam_obj->frames[x].dma =
          allocate_dma_descriptors(cam_obj->dma_node_cnt, cam_obj->dma_node_buffer_size, cam_obj->frames[x].fb.buf);
  ...
  if (!cam_obj->psram_mode) {
    ...
    if (NULL == cam_obj->dma_buffer) {
      ESP_LOGE(TAG, "%s(%d): DMA buffer %d Byte malloc failed, the current largest free block:%d Byte", __FUNCTION__,
               __LINE__, (int) cam_obj->dma_buffer_size, (int) heap_caps_get_largest_free_block(MALLOC_CAP_DMA));
      return ESP_FAIL;
    }
...
esp_err_t cam_config(const camera_config_t *config, framesize_t frame_size, uint16_t sensor_pid) {
  ...
  xTaskCreatePinnedToCore(cam_task, "cam_task", CAM_TASK_STACK, NULL, configMAX_PRIORITIES - 2, &cam_obj->task_handle,
                          0);
