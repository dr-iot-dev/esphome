...
void ll_cam_dma_reset(cam_obj_t *cam) {
  ...
  // GDMA.channel[cam->dma_num].in.sram_size.in_size = 6;//This register is used to configure the size of L2 Tx FIFO for
  // Rx channel. 0:16 bytes, 1:24 bytes, 2:32 bytes, 3: 40 bytes, 4: 48 bytes, 5:56 bytes, 6: 64 bytes, 7: 72 bytes, 8:
  // 80 bytes. GDMA.channel[cam->dma_num].in.wight.rx_weight = 7;//The weight of Rx channel 0-15
}
...
esp_err_t ll_cam_config(cam_obj_t *cam, const camera_config_t *config) {
  ...
  LCD_CAM.cam_ctrl.cam_vs_eof_en =
      0;  // 1: CAM_VSYNC to generate in_suc_eof. 0: in_suc_eof is controlled by reg_cam_rec_data_cyclelen

  LCD_CAM.cam_ctrl1.val = 0;
  LCD_CAM.cam_ctrl1.cam_rec_data_bytelen =
      LCD_CAM_DMA_NODE_BUFFER_MAX_SIZE - 1;  // Cannot be assigned to 0, and it is easy to overflow
...
esp_err_t ll_cam_init_isr(cam_obj_t *cam) {
  ...
  ret = esp_intr_alloc_intrstatus(
      ...
      (uint32_t) &LCD_CAM.lc_dma_int_st.val, LCD_CAM_CAM_VSYNC_INT_ST_M, ll_cam_vsync_isr, cam, &cam->cam_intr_handle);
  if (ret != ESP_OK) {
...
uint8_t ll_cam_get_dma_align(cam_obj_t *cam) { return 16 << GDMA.channel[cam->dma_num].in.conf1.in_ext_mem_bk_size; }
...
esp_err_t ll_cam_set_sample_mode(cam_obj_t *cam, pixformat_t pix_format, uint32_t xclk_freq_hz, uint16_t sensor_pid) {
  if (pix_format == PIXFORMAT_GRAYSCALE) {
    if (sensor_pid == OV3660_PID || sensor_pid == OV5640_PID || sensor_pid == NT99141_PID ||
        sensor_pid == SC031GS_PID || sensor_pid == BF20A6_PID || sensor_pid == GC0308_PID) {
