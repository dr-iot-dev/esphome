...
esp_err_t ll_cam_config(cam_obj_t *cam, const camera_config_t *config) {
  ...
  // I2S0.lc_conf.ext_mem_bk_size = 0; // DMA access external memory block size. 0: 16 bytes, 1: 32 bytes, 2:64 bytes,
  // 3:reserved
...
esp_err_t ll_cam_init_isr(cam_obj_t *cam) {
  return esp_intr_alloc(ETS_I2S0_INTR_SOURCE, ESP_INTR_FLAG_LOWMED | CAMERA_ISR_IRAM_FLAG, ll_cam_dma_isr, cam,
                        &cam->cam_intr_handle);
}
