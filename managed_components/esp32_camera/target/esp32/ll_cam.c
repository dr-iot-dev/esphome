...
esp_err_t ll_cam_init_isr(cam_obj_t *cam) {
  return esp_intr_alloc(ETS_I2S0_INTR_SOURCE, ESP_INTR_FLAG_LOWMED | ESP_INTR_FLAG_IRAM, ll_cam_dma_isr, cam,
                        &cam->cam_intr_handle);
}

void ll_cam_do_vsync(cam_obj_t *cam) {}

uint8_t ll_cam_get_dma_align(cam_obj_t *cam) { return 0; }
...
static bool ll_cam_calc_rgb_dma(cam_obj_t *cam) {
  ...
  ESP_LOGI(TAG,
           "node_size: %4u, nodes_per_line: %u, lines_per_node: %u, dma_half_buffer_min: %5u, dma_half_buffer: %5u,"
           ...
           (unsigned) (dma_buffer_size * cam->dma_bytes_per_item), (unsigned) image_size);
