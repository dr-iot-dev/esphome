...
static esp_err_t init_camera(uint32_t xclk_freq_hz, pixformat_t pixel_format, framesize_t frame_size, uint8_t fb_count,
                             int sccb_sda_gpio_num, int sccb_port) {
  ...
  camera_config_t camera_config = {
      ...
      .pin_sccb_sda = sccb_sda_gpio_num,  // If pin_sccb_sda is -1, sccb will use the already initialized i2c port
                                          // specified by `sccb_i2c_port`.
      ...
      .frame_size =
          frame_size,  // QQVGA-UXGAQQVGA-UXGA, For ESP32, do not use sizes above QVGA when not JPEG. The performance of
                       // the ESP32-S series has improved a lot, but JPEG mode always gives better frame rates.

      .jpeg_quality = 12,  // 0-63, for OV series camera sensors, lower number means higher quality
      .fb_count =
          fb_count,  // When jpeg mode is used, if fb_count more than one, the driver will work in continuous mode.
...
static void camera_performance_test(uint32_t xclk_freq, uint32_t pic_num) {
  ...
  for (; format_s <= format_e; format_s++) {
    for (size_t i = 0; i <= max_size; i++) {
      ESP_LOGI(TAG, "\n\n===> Testing format:%s resolution: %d x %d <===", get_cam_format_name(*format_s),
  ...
  for (size_t i = 0; i <= max_size; i++) {
    printf("%4d x %4d ,     %5.2f,     %6d,      %5.2f,     %7d,      %5.2f,     %7d \n", resolution[i].width,
           resolution[i].height, results[0].fps[i], results[0].size[i], results[1].fps[i], results[1].size[i],
...
TEST_CASE("Camera driver performance test", "[camera]") { camera_performance_test(20 * 1000000, 16); }
...
static float jpg_decode_test(uint8_t decoder_index, decode_type_t type, const uint8_t *jpg, uint32_t length,
                             uint32_t img_w, uint32_t img_h, uint32_t times) {
...
static void img_jpeg_decode_test(uint16_t pic_index, uint16_t lib_index) {
  ...
  jpg_decode_test(lib_index, DECODE_RGB565, imgs[pic_index].buf, imgs[pic_index].length, imgs[pic_index].w,
                  imgs[pic_index].h, 16);
}
...
TEST_CASE("Conversions image 227x149 jpeg decode test", "[camera]") { img_jpeg_decode_test(0, 0); }

TEST_CASE("Conversions image 320x240 jpeg decode test", "[camera]") { img_jpeg_decode_test(1, 0); }

TEST_CASE("Conversions image 480x320 jpeg decode test", "[camera]") { img_jpeg_decode_test(2, 0); }
