...
class Registry {
 public:
  ...
  bool FlatBufferToText(const uint8_t *flatbuf, size_t len, std::string *dest) {
    ...
    std::string ident(reinterpret_cast<const char *>(flatbuf) + sizeof(uoffset_t), kFileIdentifierLength);
    ...
    if (err) {
      lasterror_ = "unable to generate text for FlatBuffer binary: " + std::string(err);
      return false;
    }
    return true;
  }
 ...
 private:
  bool LoadSchema(const std::string &ident, Parser *parser) {
    ...
    // Parse schema.
    parser->opts = opts_;
    if (!parser->Parse(schematext.c_str(), include_paths_.data(), schema.path_.c_str())) {
      lasterror_ = parser->error_;
      return false;
    }
    return true;
