...
inline bool IsTypedVectorElementType(Type t) { return (t >= FBT_INT && t <= FBT_STRING) || t == FBT_BOOL; }
...
class Sized : public Object {
 public:
  ...
  size_t read_size() const { return static_cast<size_t>(ReadUInt64(data_ - byte_width_, byte_width_)); }
...
class FixedTypedVector : public Object {
 public:
  ...
  bool IsTheEmptyFixedTypedVector() const { return data_ == FixedTypedVector::EmptyFixedTypedVector().data_; }
...
class Reference {
 public:
  ...
  bool AsBool() const { return (type_ == FBT_BOOL ? ReadUInt64(data_, parent_width_) : AsUInt64()) != 0; }
...
#pragma GCC diagnostic pop
  ...
  bool MutateBool(bool b) { return type_ == FBT_BOOL && Mutate(data_, b, parent_width_, BIT_WIDTH_8); }
...
template<> inline FixedTypedVector Reference::As<FixedTypedVector>() const { return AsFixedTypedVector(); }
template<> inline Map Reference::As<Map>() const { return AsMap(); }

inline uint8_t PackedType(BitWidth bit_width, Type type) { return static_cast<uint8_t>(bit_width | (type << 2)); }
...
inline Reference GetRoot(const std::vector<uint8_t> &buffer) { return GetRoot(buffer.data(), buffer.size()); }
...
class Builder FLATBUFFERS_FINAL_CLASS {
 public:
  ...
  size_t Blob(const std::vector<uint8_t> &v) { return CreateBlob(v.data(), v.size(), 0, FBT_BLOB); }
 ...
 private:
  ...
  Value CreateVector(size_t start, size_t vec_len, size_t step, bool typed, bool fixed, const Value *keys = nullptr) {
    ...
    return Value(static_cast<uint64_t>(vloc),
                 keys ? FBT_MAP : (typed ? ToTypedVector(vector_type, fixed ? vec_len : 0) : FBT_VECTOR), bit_width);
  }
...
class Verifier FLATBUFFERS_FINAL_CLASS {
 public:
  Verifier(const uint8_t *buf, size_t buf_len,
           ...
           std::vector<uint8_t> *reuse_tracker = nullptr, bool _check_alignment = true, size_t max_depth = 64)
      : buf_(buf),
        size_(buf_len),
        depth_(0),
...
// clang-format off
  ...
  bool VerifyFrom(size_t elem, size_t elem_len) const { return Check(elem_len < size_ && elem <= size_ - elem_len); }
  ...
  bool VerifyByteWidth(size_t width) { return Check(width == 1 || width == 2 || width == 4 || width == 8); }
...
#define FLEX_CHECK_VERIFIED(P, PACKED_TYPE) \
  ...
  bool VerifyVector(Reference r, const uint8_t *p, Type elem_type) {
    ...
    auto elem_byte_width = r.type_ == FBT_STRING || r.type_ == FBT_BLOB ? uint8_t(1) : r.byte_width_;
