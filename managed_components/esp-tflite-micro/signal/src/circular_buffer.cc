...
size_t CircularBufferCapacity(const tflm_signal::CircularBuffer *cb) { return cb->capacity; }

bool CircularBufferFull(const tflm_signal::CircularBuffer *cb) { return cb->read == cb->write && cb->empty == 0; }

bool CircularBufferEmpty(const tflm_signal::CircularBuffer *cb) { return cb->empty == 1; }
...
void CircularBufferWrite(tflm_signal::CircularBuffer *cb, const int16_t *values, size_t n) {
  if (n > 0) {
...
int16_t *CircularBufferReserveForWrite(tflm_signal::CircularBuffer *cb, size_t n) {
...
void CircularBufferExtend(tflm_signal::CircularBuffer *cb, size_t count, int32_t n) {
  if (n > 0 && count > 0) {
    ASSERT(CircularBufferCanWrite(cb) >= count * n);
    ASSERT(CircularBufferAvailable(cb) >= count);
    const size_t capacity = cb->capacity;
    // start pos of region to copy
    const size_t start = (count > cb->write) ? cb->write + capacity - count : cb->write - count;
...
int16_t CircularBufferPeek(const tflm_signal::CircularBuffer *cb, size_t index) {
...
const int16_t *CircularBufferPeekDirect(const tflm_signal::CircularBuffer *cb, size_t index) {
  ASSERT(CircularBufferAvailable(cb) > index);
  size_t target = cb->read + index;
  while (target >= cb->capacity) {
    target -= cb->capacity;
...
const int16_t *CircularBufferPeekMax(const tflm_signal::CircularBuffer *cb, size_t *n) {
  if (CircularBufferAvailable(cb) > 0) {
    *n = (cb->write <= cb->read) ? cb->capacity - cb->read : cb->write - cb->read;
...
void CircularBufferGet(tflm_signal::CircularBuffer *cb, size_t n, int16_t *values) {
