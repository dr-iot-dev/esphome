...
void FilterbankAccumulateChannels(const FilterbankConfig *config, const uint32_t *input, uint64_t *output) {
  ...
  for (int i = 0; i < config->num_channels + 1; i++) {
    ...
    for (int j = 0; j < config->channel_widths[i]; ++j) {
      weight_accumulator += config->weights[weight_start + j] * static_cast<uint64_t>(input[freq_start + j]);
      unweight_accumulator += config->unweights[weight_start + j] * static_cast<uint64_t>(input[freq_start + j]);
    }
