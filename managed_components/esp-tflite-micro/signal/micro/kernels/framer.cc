  ...
  for (int i = 0; i < params->outer_dims; ++i) {
    ...
    if (params->prefill) {
      tflite::tflm_signal::CircularBufferWriteZeros(params->circular_buffers[i],
                                                    params->frame_size - params->frame_step);
    }
  }
}
...
void *FramerInit(TfLiteContext *context, const char *buffer, size_t length) {
  ...
  auto *params =
      static_cast<TFLMSignalFramerParams *>(context->AllocatePersistentBuffer(context, sizeof(TFLMSignalFramerParams)));
...
TfLiteStatus FramerPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteTensor *output_valid = micro_context->AllocateTempOutputTensor(node, kOutputValidTensor);
  ...
  params->state_buffers =
      static_cast<int8_t **>(context->AllocatePersistentBuffer(context, params->outer_dims * sizeof(int8_t *)));
  params->circular_buffers = static_cast<tflite::tflm_signal::CircularBuffer **>(
      context->AllocatePersistentBuffer(context, params->outer_dims * sizeof(tflite::tflm_signal::CircularBuffer *)));
  for (int i = 0; i < params->outer_dims; i++) {
    ...
    size_t capacity = (params->frame_size + params->frame_step - 1) / params->frame_step * params->frame_step;

    size_t state_size = tflite::tflm_signal::CircularBufferGetNeededMemory(capacity);
    params->state_buffers[i] =
        static_cast<int8_t *>(context->AllocatePersistentBuffer(context, state_size * sizeof(int8_t)));
    params->circular_buffers[i] =
        tflite::tflm_signal::CircularBufferInit(capacity, params->state_buffers[i], state_size);
  }
...
TfLiteStatus FramerEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteEvalTensor *output_valid = tflite::micro::GetEvalOutput(context, node, kOutputValidTensor);
  ...
  *output_valid_data = true;
  ...
  for (int i = 0; i < params->outer_dims; i++) {
    for (int frame = 0; frame < params->n_frames; frame++) {
      ...
      tflite::tflm_signal::CircularBufferWrite(params->circular_buffers[i], &input_data[input_idx], params->frame_step);

      if (tflite::tflm_signal::CircularBufferAvailable(params->circular_buffers[i]) >=
          static_cast<size_t>(params->frame_size)) {
        tflite::tflm_signal::CircularBufferGet(params->circular_buffers[i], params->frame_size,
                                               &output_data[output_idx]);
        tflite::tflm_signal::CircularBufferDiscard(params->circular_buffers[i], params->frame_step);
      } else {
...
TFLMRegistration *Register_FRAMER() {
  static TFLMRegistration r = tflite::micro::RegisterOp(FramerInit, FramerPrepare, FramerEval, nullptr, FramerReset);
  return &r;
}
}  // namespace tflm_signal
