...
void *IrfftInit(TfLiteContext *context, const char *buffer, size_t length) {
  ...
  auto *params = static_cast<TfLiteAudioFrontendIrfftParams *>(
      context->AllocatePersistentBuffer(context, sizeof(TfLiteAudioFrontendIrfftParams)));
  ...
  params->state = reinterpret_cast<int8_t *>(context->AllocatePersistentBuffer(context, state_size * sizeof(int8_t)));
...
template<TfLiteType TfLiteTypeEnum> TfLiteStatus IrfftPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  auto *params = reinterpret_cast<TfLiteAudioFrontendIrfftParams *>(node->user_data);
  RuntimeShape input_shape = GetTensorShape(input);
  RuntimeShape output_shape = GetTensorShape(output);
  // Divide by 2 because input is complex.
  params->input_length = input_shape.Dims(input_shape.DimensionsCount() - 1) / 2;
...
TfLiteStatus IrfftEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  const Complex<T> *input_data = tflite::micro::GetTensorData<Complex<T>>(input);
  ...
  for (int input_idx = 0, output_idx = 0; input_idx < params->input_size;
    ...
    (*apply_func)(params->state, &input_data[input_idx], &output_data[output_idx]);
  }
  return kTfLiteOk;
}
...
void *IrfftInitAll(TfLiteContext *context, const char *buffer, size_t length) {
  ...
  switch (tensor_type) {
    case TensorType_INT16: {
      return IrfftInit<int16_t, tflm_signal::IrfftInt16GetNeededMemory, tflm_signal::IrfftInt16Init>(context, buffer,
                                                                                                     length);
    }
    case TensorType_INT32: {
      return IrfftInit<int32_t, tflm_signal::IrfftInt32GetNeededMemory, tflm_signal::IrfftInt32Init>(context, buffer,
                                                                                                     length);
    }
    case TensorType_FLOAT32: {
      return IrfftInit<float, tflm_signal::IrfftFloatGetNeededMemory, tflm_signal::IrfftFloatInit>(context, buffer,
                                                                                                   length);
    }
    default:
      return nullptr;
...
TFLMRegistration *Register_IRFFT() {
  static TFLMRegistration r = tflite::micro::RegisterOp(IrfftInitAll, IrfftPrepareAll, IrfftEvalAll);
  return &r;
}

TFLMRegistration *Register_IRFFT_FLOAT() {
  static TFLMRegistration r =
      tflite::micro::RegisterOp(IrfftInit<float, IrfftFloatGetNeededMemory, IrfftFloatInit>,
...
TFLMRegistration *Register_IRFFT_INT16() {
  static TFLMRegistration r =
      tflite::micro::RegisterOp(IrfftInit<int16_t, IrfftInt16GetNeededMemory, IrfftInt16Init>,
...
TFLMRegistration *Register_IRFFT_INT32() {
  static TFLMRegistration r =
      tflite::micro::RegisterOp(IrfftInit<int32_t, IrfftInt32GetNeededMemory, IrfftInt32Init>,
