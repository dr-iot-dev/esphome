...
TfLiteStatus DelayPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  auto *params = reinterpret_cast<TFLMSignalFrontendDelayParams *>(node->user_data);
  ...
  params->circular_buffers = static_cast<tflm_signal::CircularBuffer **>(
      context->AllocatePersistentBuffer(context, params->outer_dims * sizeof(tflm_signal::CircularBuffer *)));
  ...
  for (int i = 0; i < params->outer_dims; i++) {
    ...
    params->circular_buffers[i] = tflm_signal::CircularBufferInit(capacity, params->state_buffers[i], state_size);
    tflm_signal::CircularBufferWriteZeros(params->circular_buffers[i], params->delay_length);
  }
...
TfLiteStatus DelayEval(TfLiteContext *context, TfLiteNode *node) {
  auto *params = reinterpret_cast<TFLMSignalFrontendDelayParams *>(node->user_data);
  const TfLiteEvalTensor *input = micro::GetEvalInput(context, node, kInputTensor);
  ...
  for (int dim_index = 0, sample_index = 0; dim_index < params->outer_dims;
    ...
    tflm_signal::CircularBufferWrite(params->circular_buffers[dim_index], &input_data[sample_index],
                                     params->frame_size);
    tflm_signal::CircularBufferGet(params->circular_buffers[dim_index], params->frame_size, &output_data[sample_index]);
    tflm_signal::CircularBufferDiscard(params->circular_buffers[dim_index], params->frame_size);
  }
  return kTfLiteOk;
}
...
TFLMRegistration *Register_DELAY() {
  static TFLMRegistration r = micro::RegisterOp(DelayInit, DelayPrepare, DelayEval, nullptr, DelayReset);
  return &r;
}
}  // namespace tflm_signal
