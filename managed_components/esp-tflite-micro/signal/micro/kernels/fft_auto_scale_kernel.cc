...
TfLiteStatus FftAutoScaleEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteEvalTensor *scale_bit = tflite::micro::GetEvalOutput(context, node, kScaleBitTensor);
  ...
  *scale_bit_data = tflm_signal::FftAutoScale(input_data, output->dims->data[0], output_data);
  return kTfLiteOk;
}
...
TFLMRegistration *Register_FFT_AUTO_SCALE() {
  static TFLMRegistration r = tflite::micro::RegisterOp(nullptr, FftAutoScalePrepare, FftAutoScaleEval);
  return &r;
}
