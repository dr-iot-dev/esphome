...
void *WindowInit(TfLiteContext *context, const char *buffer, size_t length) {
  ...
  auto *params =
      static_cast<TFLMSignalWindowParams *>(context->AllocatePersistentBuffer(context, sizeof(TFLMSignalWindowParams)));
...
TfLiteStatus WindowPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteTensor *weights = micro_context->AllocateTempInputTensor(node, kWeightsTensor);
...
TfLiteStatus WindowEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  const TfLiteEvalTensor *weights = tflite::micro::GetEvalInput(context, node, kWeightsTensor);
  ...
  for (int i = 0; i < params->input_size; i += weight_size) {
    ::tflm_signal::ApplyWindow(&input_data[i], weight_data, weight_size, params->shift, &output_data[i]);
  }
  return kTfLiteOk;
}
}  // namespace
...
TFLMRegistration *Register_WINDOW() {
  static TFLMRegistration r = tflite::micro::RegisterOp(WindowInit, WindowPrepare, WindowEval);
  return &r;
}
