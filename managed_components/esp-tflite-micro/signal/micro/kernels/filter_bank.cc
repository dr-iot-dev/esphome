...
void *FilterBankInit(TfLiteContext *context, const char *buffer, size_t length) {
  TFLITE_DCHECK(context->AllocatePersistentBuffer != nullptr);

  auto *params = static_cast<TFLMSignalFilterBankParams *>(
      context->AllocatePersistentBuffer(context, sizeof(TFLMSignalFilterBankParams)));
  if (params == nullptr) {
    return nullptr;
  }
  ...
  params->work_area = static_cast<uint64_t *>(
      context->AllocatePersistentBuffer(context, (params->config.num_channels + 1) * sizeof(uint64_t)));
...
TfLiteStatus FilterBankEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  const TfLiteEvalTensor *input0 = tflite::micro::GetEvalInput(context, node, kInputTensor);
  const TfLiteEvalTensor *input1 = tflite::micro::GetEvalInput(context, node, kWeightTensor);
  const TfLiteEvalTensor *input2 = tflite::micro::GetEvalInput(context, node, kUnweightTensor);
  const TfLiteEvalTensor *input3 = tflite::micro::GetEvalInput(context, node, kChFreqStartsTensor);
  const TfLiteEvalTensor *input4 = tflite::micro::GetEvalInput(context, node, kChWeightStartsTensor);
  const TfLiteEvalTensor *input5 = tflite::micro::GetEvalInput(context, node, kChannelWidthsTensor);
  ...
  params->config.channel_frequency_starts = tflite::micro::GetTensorData<int16_t>(input3);
  params->config.channel_weight_starts = tflite::micro::GetTensorData<int16_t>(input4);
  ...
  tflm_signal::FilterbankAccumulateChannels(&params->config, input_data, params->work_area);
...
TFLMRegistration *Register_FILTER_BANK() {
  static TFLMRegistration r = tflite::micro::RegisterOp(FilterBankInit, FilterBankPrepare, FilterBankEval);
  return &r;
}
