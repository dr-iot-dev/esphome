...
void *FilterBankLogInit(TfLiteContext *context, const char *buffer, size_t length) {
  TFLITE_DCHECK(context->AllocatePersistentBuffer != nullptr);

  auto *params =
      static_cast<TFLMSignalLogParams *>(context->AllocatePersistentBuffer(context, sizeof(TFLMSignalLogParams)));
...
TfLiteStatus FilterBankLogEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  tflm_signal::FilterbankLog(input_data, num_channels, params->output_scale, params->input_correction_bits,
                             output_data);
  return kTfLiteOk;
}
...
TFLMRegistration *Register_FILTER_BANK_LOG() {
  static TFLMRegistration r = tflite::micro::RegisterOp(FilterBankLogInit, FilterBankLogPrepare, FilterBankLogEval);
  return &r;
}
