...
template<typename T> struct TFLMSignalOverlapAddParams {
  int32_t frame_size;
  int32_t frame_step;
  int32_t outer_dims;
  int32_t n_frames;
...
template<typename T> void OverlapAddResetState(TFLMSignalOverlapAddParams<T> *params) {
  for (int i = 0; i < params->outer_dims; i++) {
    memset(params->state_buffers[i], 0, sizeof(T) * params->frame_size);
  }
}

template<typename T> void *OverlapAddInit(TfLiteContext *context, const char *buffer, size_t length) {
  const uint8_t *buffer_t = reinterpret_cast<const uint8_t *>(buffer);

  auto *params = static_cast<TFLMSignalOverlapAddParams<T> *>(
      context->AllocatePersistentBuffer(context, sizeof(TFLMSignalOverlapAddParams<T>)));
...
TfLiteStatus OverlapAddPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  auto *params = reinterpret_cast<TFLMSignalOverlapAddParams<T> *>(node->user_data);
  RuntimeShape input_shape = GetTensorShape(input);
  RuntimeShape output_shape = GetTensorShape(output);
  TF_LITE_ENSURE(context, input_shape.DimensionsCount() >= 2);
  TF_LITE_ENSURE_EQ(context, input_shape.DimensionsCount(), output_shape.DimensionsCount() + 1);
  ...
  params->outer_dims = input_shape.FlatSize() / (params->frame_size * params->n_frames);
  params->state_buffers =
      static_cast<T **>(context->AllocatePersistentBuffer(context, params->outer_dims * sizeof(T *)));
  TF_LITE_ENSURE(context, params != nullptr);

  for (int i = 0; i < params->outer_dims; i++) {
    params->state_buffers[i] =
        static_cast<T *>(context->AllocatePersistentBuffer(context, params->frame_size * sizeof(T)));
  }
...
template<typename T> TfLiteStatus OverlapAddEval(TfLiteContext *context, TfLiteNode *node) {
  auto *params = reinterpret_cast<TFLMSignalOverlapAddParams<T> *>(node->user_data);
  ...
  for (int i = 0; i < params->outer_dims; i++) {
    ...
    for (int frame = 0; frame < params->n_frames; frame++) {
      ...
      tflm_signal::OverlapAdd(&input_data[input_index], buffer, params->frame_size, &output_data[output_index],
                              params->frame_step);
    }
  }
  return kTfLiteOk;
...
template<typename T> void OverlapAddReset(TfLiteContext *context, void *buffer) {
  OverlapAddResetState(static_cast<TFLMSignalOverlapAddParams<T> *>(buffer));
}

void *OverlapAddInitAll(TfLiteContext *context, const char *buffer, size_t length) {
  const uint8_t *buffer_t = reinterpret_cast<const uint8_t *>(buffer);
...
TfLiteStatus OverlapAddPrepareAll(TfLiteContext *context, TfLiteNode *node) {
  auto *params = reinterpret_cast<TFLMSignalOverlapAddParams<void> *>(node->user_data);
...
TFLMRegistration *Register_OVERLAP_ADD() {
  static TFLMRegistration r = tflite::micro::RegisterOp(OverlapAddInitAll, OverlapAddPrepareAll, OverlapAddEvalAll,
                                                        nullptr, OverlapAddResetAll);
  return &r;
}

TFLMRegistration *Register_OVERLAP_ADD_FLOAT() {
  static TFLMRegistration r = tflite::micro::RegisterOp(OverlapAddInit<float>, OverlapAddPrepare<float, kTfLiteFloat32>,
...
TFLMRegistration *Register_OVERLAP_ADD_INT16() {
  static TFLMRegistration r =
      tflite::micro::RegisterOp(OverlapAddInit<int16_t>, OverlapAddPrepare<int16_t, kTfLiteInt16>,
