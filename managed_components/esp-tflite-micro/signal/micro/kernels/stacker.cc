...
void *StackerInit(TfLiteContext *context, const char *buffer, size_t length) {
  ...
  auto *params = static_cast<TFLMSignalStackerParams *>(
      context->AllocatePersistentBuffer(context, sizeof(TFLMSignalStackerParams)));
  if (params == nullptr) {
    return nullptr;
  }
  ...
  params->buffer_size = params->num_channels * (params->stacker_left_context + params->stacker_right_context + 1);
  params->step_size = params->num_channels * params->stacker_step;
  params->stacker_has_first_frame = false;

  size_t state_size = tflm_signal::CircularBufferGetNeededMemory(params->buffer_size);
  params->state = static_cast<int8_t *>(context->AllocatePersistentBuffer(context, sizeof(int8_t) * state_size));

  if (params->state == nullptr) {
    return nullptr;
  }

  params->circular_buffer = tflm_signal::CircularBufferInit(params->buffer_size, params->state, state_size);
  return params;
}
...
TfLiteStatus StackerEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  tflm_signal::CircularBufferWrite(params->circular_buffer, input_data, params->num_channels);

  // The first frame is replicated an extra left_context times to pad.
  if (params->stacker_has_first_frame == false) {
    tflm_signal::CircularBufferExtend(params->circular_buffer, params->num_channels, params->stacker_left_context);
    params->stacker_has_first_frame = true;
  }

  int16_t *output_data = tflite::micro::GetTensorData<int16_t>(output);
  bool *output_valid_data = tflite::micro::GetTensorData<bool>(output_valid);
  if (tflm_signal::CircularBufferAvailable(params->circular_buffer) >= params->buffer_size) {
    tflm_signal::CircularBufferGet(params->circular_buffer, params->buffer_size, output_data);
    tflm_signal::CircularBufferDiscard(params->circular_buffer, params->step_size);
...
TFLMRegistration *Register_STACKER() {
  static TFLMRegistration r =
      tflite::micro::RegisterOp(StackerInit, StackerPrepare, StackerEval, /*Free*/ nullptr, StackerReset);
  return &r;
}
}  // namespace tflm_signal
