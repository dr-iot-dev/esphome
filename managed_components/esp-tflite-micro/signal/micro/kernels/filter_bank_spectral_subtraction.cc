...
void FilterBankSpectralSubtractionResetState(TFLMSignalSpectralSubtractionParams *params) {
  memset(params->noise_estimate, 0, sizeof(uint32_t) * params->config.num_channels);
}

void *FilterBankSpectralSubtractionInit(TfLiteContext *context, const char *buffer, size_t length) {
  TFLITE_DCHECK(context->AllocatePersistentBuffer != nullptr);

  auto *params = static_cast<TFLMSignalSpectralSubtractionParams *>(
      context->AllocatePersistentBuffer(context, sizeof(TFLMSignalSpectralSubtractionParams)));
  ...
  params->config.alternate_one_minus_smoothing = fbw.ElementAsInt32(kAlternateOneMinusSmoothingIndex);
  params->config.alternate_smoothing = fbw.ElementAsInt32(kAlternateSmoothingIndex);
  params->config.clamping = fbw.ElementAsBool(kClampingIndex);
  params->config.min_signal_remaining = fbw.ElementAsInt32(kMinSignalRemainingIndex);
  params->config.num_channels = fbw.ElementAsInt32(kNumChannelsIndex);
  params->config.one_minus_smoothing = fbw.ElementAsInt32(kOneMinusSmoothingIndex);
  params->config.one_minus_smoothing = fbw.ElementAsInt32(kOneMinusSmoothingIndex);
  ...
  params->config.spectral_subtraction_bits = fbw.ElementAsInt32(kSpectralSubtractionBitsIndex);
  params->noise_estimate = static_cast<uint32_t *>(
      context->AllocatePersistentBuffer(context, params->config.num_channels * sizeof(uint32_t)));
...
TfLiteStatus FilterBankSpectralSubtractionPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteTensor *noise_estimate = micro_context->AllocateTempOutputTensor(node, kNoiseEstimateTensor);
  ...
  auto *params = reinterpret_cast<TFLMSignalSpectralSubtractionParams *>(node->user_data);
...
TfLiteStatus FilterBankSpectralSubtractionEval(TfLiteContext *context, TfLiteNode *node) {
  auto *params = reinterpret_cast<TFLMSignalSpectralSubtractionParams *>(node->user_data);
  ...
  TfLiteEvalTensor *noise_estimate = tflite::micro::GetEvalOutput(context, node, kNoiseEstimateTensor);
  ...
  uint32_t *noise_estimate_data = tflite::micro::GetTensorData<uint32_t>(noise_estimate);

  FilterbankSpectralSubtraction(&params->config, input_data, output_data, params->noise_estimate);

  memcpy(noise_estimate_data, params->noise_estimate, params->noise_estimate_size);

  return kTfLiteOk;
}

void FilterBankSpectralSubtractionReset(TfLiteContext *context, void *buffer) {
  FilterBankSpectralSubtractionResetState(static_cast<TFLMSignalSpectralSubtractionParams *>(buffer));
}
...
TFLMRegistration *Register_FILTER_BANK_SPECTRAL_SUBTRACTION() {
  static TFLMRegistration r = tflite::micro::RegisterOp(
      FilterBankSpectralSubtractionInit, FilterBankSpectralSubtractionPrepare, FilterBankSpectralSubtractionEval,
