...
template<class T> size_t TfLiteVarArrayGetSizeInBytes(const int size) {
...
template<class T, class U> int TfLiteVarArrayEqualsArray(const T *const a, const int b_size, const U *const b_data) {
...
template<class T> int TfLiteVarArrayEqual(const T *const a, const T *const b) {
...
template<class T> T *TfLiteVarArrayCreate(const int size) {
...
template<class T> T *TfLiteVarArrayCopy(const T *const src) {
  if (!src) {
    return nullptr;
  }
...
template<class T> void TfLiteVarArrayFree(T *a) { free(a); }

}  // namespace

extern "C" {

size_t TfLiteIntArrayGetSizeInBytes(int size) { return TfLiteVarArrayGetSizeInBytes<TfLiteIntArray>(size); }

int TfLiteIntArrayEqual(const TfLiteIntArray *a, const TfLiteIntArray *b) { return TfLiteVarArrayEqual(a, b); }
...
TfLiteIntArray *TfLiteIntArrayCreate(int size) { return TfLiteVarArrayCreate<TfLiteIntArray>(size); }

TfLiteIntArray *TfLiteIntArrayCopy(const TfLiteIntArray *src) { return TfLiteVarArrayCopy(src); }

void TfLiteIntArrayFree(TfLiteIntArray *a) { TfLiteVarArrayFree(a); }

#endif  // TF_LITE_STATIC_MEMORY

int TfLiteFloatArrayGetSizeInBytes(int size) { return TfLiteVarArrayGetSizeInBytes<TfLiteFloatArray>(size); }

#ifndef TF_LITE_STATIC_MEMORY

TfLiteFloatArray *TfLiteFloatArrayCreate(int size) { return TfLiteVarArrayCreate<TfLiteFloatArray>(size); }

TfLiteFloatArray *TfLiteFloatArrayCopy(const TfLiteFloatArray *src) { return TfLiteVarArrayCopy(src); }
...
void TfLiteTensorDataFree(TfLiteTensor *t) {
  ...
  } else if (t->allocation_type == kTfLiteDynamic || t->allocation_type == kTfLitePersistentRo) {
    if (t->data.raw) {
#ifdef TF_LITE_TENSORFLOW_PROFILER
...
void TfLiteTensorReset(TfLiteType type, const char *name, TfLiteIntArray *dims, TfLiteQuantizationParams quantization,
...
TfLiteStatus TfLiteTensorResizeMaybeCopy(size_t num_bytes, TfLiteTensor *tensor, bool preserve_data) {
...
TfLiteAllocationStrategy TfLiteTensorGetAllocationStrategy(const TfLiteTensor *const t) {
...
TfLiteRunStability TfLiteTensorGetBufferAddressStability(const TfLiteTensor *const t) {
  switch (t->allocation_type) {
    case kTfLiteMemNone:
