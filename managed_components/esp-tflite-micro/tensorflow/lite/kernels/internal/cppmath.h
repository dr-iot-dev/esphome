...
#if defined(TF_LITE_USE_GLOBAL_CMATH_FUNCTIONS) || (defined(__ANDROID__) && !defined(__NDK_MAJOR__)) || \
    defined(__ZEPHYR__)
...
#define DECLARE_STD_GLOBAL_SWITCH1(tf_name, std_name) \
  template<class T> inline T tf_name(const T x) { return TF_LITE_GLOBAL_STD_PREFIX::std_name(x); }
