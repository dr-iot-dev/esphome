...
bool ReduceDimensionsForBroadcast(const RuntimeShape &input1_shape, const RuntimeShape &input2_shape,
...
inline void GetActivationMinMax(FusedActivationFunctionType ac, float *output_activation_min,
...
template<typename T> inline T ActivationFunctionWithMinMax(T x, T output_activation_min, T output_activation_max) {
  using std::max;
  using std::min;
...
inline void BiasAndClamp(float clamp_min, float clamp_max, int bias_size, const float *bias_data, int array_size,
                         float *array_data) {
  if (bias_size == 0)
    return;
  ...
  for (int array_offset = 0; array_offset < array_size; array_offset += bias_size) {
    for (int i = 0; i < bias_size; i++) {
      array_data[array_offset + i] =
          ActivationFunctionWithMinMax(array_data[array_offset + i] + bias_data[i], clamp_min, clamp_max);
    }
  }
#endif
}

TFLITE_NOINLINE int32_t MultiplyByQuantizedMultiplier(int32_t x, int32_t quantized_multiplier, int shift);

TFLITE_NOINLINE int32_t MultiplyByQuantizedMultiplier(int64_t x, int32_t quantized_multiplier, int shift);
...
#ifdef USE_NEON
inline int32x4x4_t MultiplyByQuantizedMultiplier4Rows(int32x4x4_t input_val, int32_t quantized_multiplier, int shift) {
  ...
  int32x4x4_t result;
  result.val[0] =
      vrshlq_s32(vqdmulhq_s32(vshlq_s32(input_val.val[0], left_shift_dup), multiplier_dup), right_shift_dup);

  result.val[1] =
      vrshlq_s32(vqdmulhq_s32(vshlq_s32(input_val.val[1], left_shift_dup), multiplier_dup), right_shift_dup);

  result.val[2] =
      vrshlq_s32(vqdmulhq_s32(vshlq_s32(input_val.val[2], left_shift_dup), multiplier_dup), right_shift_dup);

  result.val[3] =
      vrshlq_s32(vqdmulhq_s32(vshlq_s32(input_val.val[3], left_shift_dup), multiplier_dup), right_shift_dup);
...
#else
inline int32_t MultiplyByQuantizedMultiplierSmallerThanOneExp(int32_t x, int32_t quantized_multiplier, int left_shift) {
  using gemmlowp::RoundingDivideByPOT;
  using gemmlowp::SaturatingRoundingDoublingHighMul;
  return RoundingDivideByPOT(SaturatingRoundingDoublingHighMul(x, quantized_multiplier), -left_shift);
}

inline int32_t MultiplyByQuantizedMultiplierGreaterThanOne(int32_t x, int32_t quantized_multiplier, int left_shift) {
  using gemmlowp::SaturatingRoundingDoublingHighMul;
...
template<typename T> inline int CountLeadingSignBits(T integer_input) {
  ...
  return integer_input >= 0 ? CountLeadingZeros(static_cast<U>(integer_input)) - 1
         : integer_input != std::numeric_limits<T>::min() ? CountLeadingZeros(2 * static_cast<U>(-integer_input) - 1)
                                                          : 0;
#endif
}
...
inline typename std::enable_if<std::is_same<Func, FloatT (*)(FloatT)>::value, FloatT>::type LUTTransform(
    Func transform, const void * /*transform_params*/, FloatT value) {
  static_assert(std::is_floating_point<FloatT>::value, "FloatT must be a floating-point type.");
  return transform(value);
}

template<typename FloatT, typename Func>
inline typename std::enable_if<std::is_same<Func, FloatT (*)(FloatT, const void *)>::value, FloatT>::type LUTTransform(
    Func transform, const void *transform_params, FloatT value) {
  static_assert(std::is_floating_point<FloatT>::value, "FloatT must be a floating-point type.");
...
inline void LUTPopulateInt8(float input_scale, int32_t input_zero_point, float output_scale, int32_t output_zero_point,
                            Func transform, const void *transform_params, T *lut) {
  static_assert(std::is_same<T, uint8_t>::value || std::is_same<T, int8_t>::value, "T must be an uint8 or int8 type.");
...
inline void LUTPopulateInt16(FloatT input_scale, int32_t input_zero_point, FloatT output_scale,
                             int32_t output_zero_point, Func transform, const void *transform_params, int16_t *lut) {
  static_assert(std::is_floating_point<FloatT>::value, "FloatT must be a floating-point type.");
  ...
  const FloatT output_min = output_scale * (std::numeric_limits<int16_t>::min() - output_zero_point);
  const FloatT output_max = output_scale * (std::numeric_limits<int16_t>::max() - output_zero_point);
  ...
  const FloatT output_scaling_inv =
      static_cast<FloatT>(std::numeric_limits<int16_t>::max() - std::numeric_limits<int16_t>::min() + 1) /
  ...
  for (int i = 0; i < nb_steps; i++) {
    ...
    const FloatT val_midpoint = LUTTransform<FloatT>(transform, transform_params, input_min + i * step + half_step);
    const FloatT val_next = LUTTransform<FloatT>(transform, transform_params, input_min + (i + 1) * step);

    const FloatT sample_val = TfLiteRound(val * output_scaling_inv);
    const FloatT midpoint_interp_val =
        TfLiteRound((val_next * output_scaling_inv + TfLiteRound(val * output_scaling_inv)) / 2);
    ...
    lut[i] = static_cast<int16_t>(std::min<FloatT>(std::max<FloatT>(sample_val - bias, table_min), table_max));
  }

  lut[nb_steps] = static_cast<int16_t>(std::min<FloatT>(
      std::max<FloatT>(TfLiteRound(LUTTransform<FloatT>(transform, transform_params, input_max) * output_scaling_inv),
                       table_min),
      table_max));
}
...
template<typename T>
inline typename std::enable_if<std::is_same<T, uint8_t>::value || std::is_same<T, int8_t>::value, void>::type
LUTPopulate(float input_scale, int32_t input_zero_point, float output_scale, int32_t output_zero_point,
            float (*transform)(float), T *lut) {
  detail::LUTPopulateInt8(input_scale, input_zero_point, output_scale, output_zero_point, transform, nullptr, lut);
}
...
LUTPopulate(float input_scale, int32_t input_zero_point, float output_scale, int32_t output_zero_point,
  ...
  detail::LUTPopulateInt8(input_scale, input_zero_point, output_scale, output_zero_point, transform, transform_params,
                          lut);
}
...
inline typename std::enable_if<std::is_same<T, int16_t>::value, void>::type LUTPopulate(
    ...
    float (*transform)(float), T *lut) {
  detail::LUTPopulateInt16<float>(input_scale, input_zero_point, output_scale, output_zero_point, transform, nullptr,
                                  lut);
}
...
inline typename std::enable_if<std::is_same<T, int16_t>::value, void>::type LUTPopulate(
    float input_scale, int32_t input_zero_point, float output_scale, int32_t output_zero_point,
    float (*transform)(float, const void *), const void *transform_params, T *lut) {
  detail::LUTPopulateInt16<float>(input_scale, input_zero_point, output_scale, output_zero_point, transform,
...
inline typename std::enable_if<std::is_same<T, int16_t>::value, void>::type LUTPopulate(
  ...
  detail::LUTPopulateInt16<double>(input_scale, input_zero_point, output_scale, output_zero_point, transform, nullptr,
                                   lut);
}
...
template<typename T> constexpr int LUTSize() {
  static_assert(std::is_same<T, uint8_t>::value || std::is_same<T, int8_t>::value || std::is_same<T, int16_t>::value,
...
inline int8_t LUTLookup(int8_t value, const int8_t *lut) { return lut[static_cast<uint8_t>(value)]; }
...
gemmlowp::FixedPoint<tRawType, tIntegerBits> SaturatingAddNonGemmlowp(gemmlowp::FixedPoint<tRawType, tIntegerBits> a,
...
template<> inline std::int16_t SaturatingSub(std::int16_t a, std::int16_t b) {
  ...
  return static_cast<std::int16_t>(std::min(static_cast<int32_t>(32767), std::max(static_cast<int32_t>(-32768), diff)));
}
...
template<typename tRawType, int tIntegerBits>
gemmlowp::FixedPoint<tRawType, tIntegerBits> SaturatingSub(gemmlowp::FixedPoint<tRawType, tIntegerBits> a,
                                                           gemmlowp::FixedPoint<tRawType, tIntegerBits> b) {
  return gemmlowp::FixedPoint<tRawType, tIntegerBits>::FromRaw(SaturatingSub(a.raw(), b.raw()));
}
// End section to be moved to gemmlowp.

template<typename IntegerType> IntegerType SaturatingRoundingMultiplyByPOTParam(IntegerType x, int exponent) {
  if (exponent == 0) {
    return x;
  }
  using ScalarIntegerType = typename gemmlowp::FixedPointRawTypeTraits<IntegerType>::ScalarRawType;
  const IntegerType min = gemmlowp::Dup<IntegerType>(std::numeric_limits<ScalarIntegerType>::min());
  const IntegerType max = gemmlowp::Dup<IntegerType>(std::numeric_limits<ScalarIntegerType>::max());
  const int ScalarIntegerTypeBits = 8 * sizeof(ScalarIntegerType);

  const std::int32_t threshold = ((1 << (ScalarIntegerTypeBits - 1 - exponent)) - 1);
  const IntegerType positive_mask = gemmlowp::MaskIfGreaterThan(x, gemmlowp::Dup<IntegerType>(threshold));
  const IntegerType negative_mask = gemmlowp::MaskIfLessThan(x, gemmlowp::Dup<IntegerType>(-threshold));
...
template<typename tRawType, int tIntegerBits>
gemmlowp::FixedPoint<tRawType, tIntegerBits> SaturatingRoundingMultiplyByPOTParam(
    gemmlowp::FixedPoint<tRawType, tIntegerBits> a, int exponent) {
  return gemmlowp::FixedPoint<tRawType, tIntegerBits>::FromRaw(SaturatingRoundingMultiplyByPOTParam(a.raw(), exponent));
}
...
inline gemmlowp::FixedPoint<int32_t, OutputIntegerBits> log_x_for_x_greater_than_or_equal_to_1_impl(
  ...
  const FixedPoint0 sqrt_half = GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(FixedPoint0, 1518500250, std::sqrt(0.5));
  const FixedPoint0 one_quarter = GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(FixedPoint0, 536870912, 1.0 / 4.0);

  const FixedPoint0 alpha_n =
      GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(FixedPoint0, 117049297, 11.0 / 240.0 * std::sqrt(std::sqrt(2.0)));
  ...
  const FixedPointAccum z_b_pow_2_adj =
      SaturatingSub(FixedPointAccum::FromRaw(SaturatingRoundingMultiplyByPOTParam(
                        static_cast<int32_t>(InputIntegerBits - z_b_headroom), 31 - kAccumIntegerBits)),
                    shifted_quarter);

  const FixedPoint0 r = FixedPoint0::FromRaw(std::min(r_a_raw, r_b_raw));
  const FixedPointAccum z_pow_2_adj = FixedPointAccum::FromRaw(std::max(z_a_pow_2_adj.raw(), z_b_pow_2_adj.raw()));
...
template<int OutputIntegerBits, int InputIntegerBits>
inline gemmlowp::FixedPoint<int32_t, OutputIntegerBits> log_x_for_x_greater_than_or_equal_to_1(
    gemmlowp::FixedPoint<int32_t, InputIntegerBits> input_val) {
  ...
  return log_x_for_x_greater_than_or_equal_to_1_impl<OutputIntegerBits, InputIntegerBits>(input_val);
}
...
inline int32_t GetReciprocal(int32_t x, int x_integer_digits, int *num_bits_over_unit) {
  ...
  *num_bits_over_unit = x_integer_digits - headroom_plus_one;
  const int32_t shifted_sum_minus_one =
      static_cast<int32_t>((static_cast<uint32_t>(x) << headroom_plus_one) - (static_cast<uint32_t>(1) << 31));

  gemmlowp::FixedPoint<int32_t, 0> shifted_scale =
      gemmlowp::one_over_one_plus_x_for_x_in_0_1(gemmlowp::FixedPoint<int32_t, 0>::FromRaw(shifted_sum_minus_one));
  return shifted_scale.raw();
}

inline void GetInvSqrtQuantizedMultiplierExp(int32_t input, int reverse_shift, int32_t *output_inv_sqrt,
                                             int *output_shift) {
...
inline int SubscriptToIndex(const NdArrayDesc<5> &desc, int indexes[5]) {
  return indexes[0] * desc.strides[0] + indexes[1] * desc.strides[1] + indexes[2] * desc.strides[2] +
         indexes[3] * desc.strides[3] + indexes[4] * desc.strides[4];
}

inline int SubscriptToIndex(const NdArrayDesc<8> &desc, int indexes[8]) {
  return indexes[0] * desc.strides[0] + indexes[1] * desc.strides[1] + indexes[2] * desc.strides[2] +
         indexes[3] * desc.strides[3] + indexes[4] * desc.strides[4] + indexes[5] * desc.strides[5] +
...
//   same slice.
template<int N>
inline void NdArrayDescsForElementwiseBroadcast(const Dims<N> &input0_dims, const Dims<N> &input1_dims,
...
template<int N> TFLITE_NOINLINE void CopyDimsToDesc(const RuntimeShape &input_shape, NdArrayDesc<N> *desc_out) {
  int desc_stride = 1;
  for (int i = N - 1; i >= 0; --i) {
...
template<int N>
inline void NdArrayDescsForElementwiseBroadcast(const RuntimeShape &input0_shape, const RuntimeShape &input1_shape,
                                                NdArrayDesc<N> *desc0_out, NdArrayDesc<N> *desc1_out) {
  TFLITE_DCHECK(desc0_out != nullptr);
  TFLITE_DCHECK(desc1_out != nullptr);
...
typename std::enable_if<DIM != N - 1, void>::type NDOpsHelperImpl(const NdArrayDesc<N> &output, const Calc &calc,
                                                                  int indexes[N]) {
...
template<int N, typename Calc> inline void NDOpsHelper(const NdArrayDesc<N> &output, const Calc &calc) {
  int indexes[N] = {0};
...
// the fixed Modulus.
template<unsigned Modulus, typename Integer> Integer RoundDown(Integer i) { return i - (i % Modulus); }
...
// the fixed Modulus.
template<unsigned Modulus, typename Integer> Integer RoundUp(Integer i) { return RoundDown<Modulus>(i + Modulus - 1); }
...
template<int KernelRows> inline int LegacyHowManyThreads(int max_num_threads, int rows, int cols, int depth) {
  ...
  if (thread_count > 1) {
    ...
    const std::uint64_t cubic_size = std::uint64_t(rows) * std::uint64_t(cols) * std::uint64_t(depth);
