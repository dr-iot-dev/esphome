...
inline bool Reduce(const In *input_data, const int *input_dims, const int *output_dims, const int input_num_dims,
...
inline bool Reduce(const In *input_data, const int *input_dims, const int *output_dims, const int input_num_dims,
  ...
  do {
    ...
    if (IsFirstReduction(input_iter, num_axis, axis)) {
      ...
      output_data[output_offset] = reducer_next(output_data[output_offset], input_data[input_offset]);
    }
  } while (NextIndex(input_num_dims, input_dims, input_iter));
  return true;
}
...
inline bool ResolveAxis(const int num_dims, const int *axis, const int64_t num_axis, int *out_axis, int *out_num_axis) {
...
template<typename In, typename Out>
inline bool ReduceSumImpl(const In *input_data, const int *input_dims, const int *output_dims, const int input_num_dims,
                          const int output_num_dims, const int *axis, const int num_axis, int *input_iter,
                          Out *output_data) {
  ...
  return Reduce<In, Out>(input_data, input_dims, output_dims, input_num_dims, output_num_dims, axis, num_axis,
...
template<typename T>
inline bool ReduceGeneric(const T *input_data, const int *input_dims, const int input_num_dims, T *output_data,
  ...
  return Reduce<T, T>(input_data, input_dims, output_dims, input_num_dims, output_num_dims, resolved_axis,
...
inline bool Mean(const T *input_data, const int *input_dims, const int input_num_dims, T *output_data,
                 const int *output_dims, const int output_num_dims, const int *axis, const int num_axis_dimensions,
  ...
  if (!ReduceSumImpl<T, U>(input_data, input_dims, output_dims, input_num_dims, output_num_dims, resolved_axis,
...
inline void Mean(const tflite::MeanParams &op_params, const RuntimeShape &unextended_input_shape,
...
template<typename T, typename U>
inline bool QuantizedMeanOrSum(const T *input_data, int32_t input_zero_point, const int *input_dims,
                               const int input_num_dims, T *output_data, int32_t output_multiplier, int output_shift,
  ...
  for (int idx = 0; idx < num_resolved_axis; ++idx) {
    size_t current = static_cast<size_t>(input_dims[resolved_axis[idx]]);
    // Overflow prevention.
    if (current > static_cast<size_t>(std::numeric_limits<int64_t>::max() / num_elements_in_axis)) {
      return false;
    }
    num_elements_in_axis *= current;
  }
  ...
  if (!compute_sum) {
    ...
    output_multiplier = static_cast<int32_t>((static_cast<int64_t>(output_multiplier) << shift) / num_elements_in_axis);
    output_shift = output_shift - shift;
  }

  for (size_t idx = 0; idx < num_outputs; ++idx) {
    const U shifted_sum = static_cast<U>(temp_sum[idx] - input_zero_point * num_elements_in_axis);
    int32_t output = MultiplyByQuantizedMultiplier(shifted_sum, output_multiplier, output_shift) + output_zero_point;
...
template<typename T, typename U>
inline bool QuantizedMeanOrSumExtraArgs(const T *input_data, int32_t input_zero_point, float input_scale,
                                        const int *input_dims, const int input_num_dims, T *output_data,
                                        float output_scale, int32_t output_multiplier, int output_shift,
                                        int32_t output_zero_point, const int *output_dims, const int output_num_dims,
                                        const int *axis, const int num_axis_dimensions, bool keep_dims, int *temp_index,
                                        int *resolved_axis, U *temp_sum, bool compute_sum) {
  return QuantizedMeanOrSum<T, U>(input_data, input_zero_point, input_dims, input_num_dims, output_data,
                                  output_multiplier, output_shift, output_zero_point, output_dims, output_num_dims,
                                  axis, num_axis_dimensions, keep_dims, temp_index, resolved_axis, temp_sum,
                                  compute_sum);
}

template<typename T>
inline bool QuantizedReduceProd(const T *input_data, int32_t input_zero_point, const RuntimeShape &input_shape,
                                T *output_data, int32_t output_zero_point, const RuntimeShape &output_shape,
                                const int *axis, const int64_t num_axis_dimensions, bool keep_dims, int *temp_index,
                                int *resolved_axis, int32_t *temp_prod, int32_t scaling_multiplier, int scaling_shift) {
  const int32_t kMinValue = std::numeric_limits<T>::min();
  const int32_t kMaxValue = std::numeric_limits<T>::max();

  // Resolve axis.
  int num_resolved_axis = 0;
  if (!ResolveAxis(input_shape.DimensionsCount(), axis, num_axis_dimensions, resolved_axis, &num_resolved_axis)) {
    return false;
  }
  ...
  if (!Reduce<T, int32_t>(input_data, input_shape.DimsData(), output_shape.DimsData(), input_shape.DimensionsCount(),
                          output_shape.DimensionsCount(), resolved_axis, num_resolved_axis, temp_index, reducer_first,
  ...
  for (int i = 0; i < output_shape.FlatSize(); i++) {
    int32_t result =
        MultiplyByQuantizedMultiplier(static_cast<int64_t>(temp_prod[i]), scaling_multiplier, scaling_shift) +
        output_zero_point;
