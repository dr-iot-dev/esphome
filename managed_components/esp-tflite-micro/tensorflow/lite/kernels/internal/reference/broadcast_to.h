...
template<int N>
void BroadcastImpl(const NdArrayDesc<N> &input_desc, const char *input_data, const NdArrayDesc<N> &output_desc,
                   char *output_data, int indexes[N], int dim, const int last_broadcasting_dim, const int type_size) {
  ...
  for (indexes[dim] = 0; indexes[dim] < input_desc.extents[dim]; ++indexes[dim]) {
    BroadcastImpl<N>(input_desc, input_data, output_desc, output_data, indexes, dim + 1, last_broadcasting_dim,
                     type_size);
  }
...
template<int N>
inline void BroadcastTo(const RuntimeShape &unextended_input_shape, const char *input_data,
