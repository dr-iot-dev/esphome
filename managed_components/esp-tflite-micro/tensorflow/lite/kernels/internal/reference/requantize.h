...
inline void Requantize(const input_type *input_data, int32_t size, int32_t effective_scale_multiplier,
  ...
  if (same_scale) {
    ...
    // needed.
    if ((mixed_type_int8_uint8 && zero_point_diff == -128) || (mixed_type_uint8_int8 && zero_point_diff == 128)) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    const int32_t output =
        MultiplyByQuantizedMultiplier(input, effective_scale_multiplier, effective_scale_shift) + output_zeropoint;
