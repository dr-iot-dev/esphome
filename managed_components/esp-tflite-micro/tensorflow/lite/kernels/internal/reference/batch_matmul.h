...
inline void BatchMatMul(const RuntimeShape &lhs_shape, const Ta *lhs_data, const RuntimeShape &rhs_shape,
  ...
  for (int b0 = 0; b0 < batch_dim0; ++b0) {
    ...
    for (int b1 = 0; b1 < batch_dim1; ++b1) {
      ...
      for (int b2 = 0; b2 < batch_dim2; ++b2) {
        ...
        Tout *out_ptr = output_data + ((b0 * batch_dim1 * batch_dim2) + b1 * batch_dim2 + b2) * lhs_rows * rhs_cols;
        for (int j = 0; j < rhs_cols; ++j) {
          for (int i = 0; i < lhs_rows; ++i) {
            Tout total = 0;
            for (int k = 0; k < accum_depth; ++k) {
              total +=
                  static_cast<Tout>(lhs_ptr2[accum_depth * i + k]) * static_cast<Tout>(rhs_ptr2[j * accum_depth + k]);
            }
...
inline void BatchMatMul(const RuntimeShape &lhs_shape, const int8_t *lhs_data, const RuntimeShape &rhs_shape,
  ...
  for (int b0 = 0; b0 < batch_dim0; ++b0) {
    ...
    for (int b1 = 0; b1 < batch_dim1; ++b1) {
      ...
      for (int b2 = 0; b2 < batch_dim2; ++b2) {
        ...
        float *out_ptr = output_data + ((b0 * batch_dim1 * batch_dim2) + b1 * batch_dim2 + b2) * lhs_rows * rhs_cols;
        for (int j = 0; j < rhs_cols; ++j) {
...
inline void BatchMatMul(const FullyConnectedParams &params, const RuntimeShape &lhs_shape, const T *lhs_data,
  ...
  for (int b0 = 0; b0 < batch_dim0; ++b0) {
    ...
    for (int b1 = 0; b1 < batch_dim1; ++b1) {
      ...
      for (int b2 = 0; b2 < batch_dim2; ++b2) {
        ...
        T *out_ptr = output_data + ((b0 * batch_dim1 * batch_dim2) + b1 * batch_dim2 + b2) * lhs_rows * rhs_cols;
        ...
        for (int j = 0; j < rhs_cols; ++j) {
          for (int i = 0; i < lhs_rows; ++i) {
            ...
            int32_t total_scaled = MultiplyByQuantizedMultiplier(total, output_multiplier, output_shift);
