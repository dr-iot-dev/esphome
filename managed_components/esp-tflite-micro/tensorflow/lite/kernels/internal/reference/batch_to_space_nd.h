...
template<typename T>
inline void BatchToSpaceND(const RuntimeShape &unextended_input1_shape, const T *input1_data,
  ...
  const int block_shape_width = unextended_input1_shape.DimensionsCount() == 4 ? block_shape_data[1] : 1;
  ...
  for (int in_batch = 0; in_batch < input_batch_size; ++in_batch) {
    ...
    for (int in_h = 0; in_h < input_height; ++in_h) {
      ...
      for (int in_w = 0; in_w < input_width; ++in_w) {
        const int out_w = in_w * block_shape_width + spatial_offset % block_shape_width - crops_left;
