...
inline void LogSoftmax(const SoftmaxParams &params, const RuntimeShape &input_shape, const float *input_data,
...
inline void LogSoftmax(const SoftmaxParams &params, const RuntimeShape &input_shape, const uint8_t *input_data,
  ...
  for (int i = 0; i < outer_size; ++i) {
    ...
    const int rescaled_diff_min = fixed_log_sum_of_exps + std::numeric_limits<int32_t>::lowest();
    const int adjusted_diff_min =
        std::max(static_cast<int32_t>(diff_min - 1),  // Note use of > below instead of >= above.
    ...
    for (int c = 0; c < depth; ++c) {
      ...
      if (input_diff > adjusted_diff_min) {
        const int32_t input_diff_rescaled =
            MultiplyByQuantizedMultiplierGreaterThanOne(input_diff, input_multiplier, input_left_shift);
        int32_t unsat_output = gemmlowp::RoundingDivideByPOT((input_diff_rescaled - fixed_log_sum_of_exps),
                                                             31 - kScaledDiffIntegerBits - kOutputIntegerBits) +
                               255;

        output_data[i * depth + c] =
            static_cast<uint8_t>(std::max(std::min(unsat_output, static_cast<int32_t>(255)), static_cast<int32_t>(0)));
      } else {
...
template<typename T>
inline void LogSoftmaxQuantized(const SoftmaxParams &params, const size_t outer_size, const size_t depth,
  ...
  for (size_t outer_index = 0; outer_index < outer_size; ++outer_index) {
    ...
    for (size_t inner_index = 0; inner_index < depth; ++inner_index) {
      int32_t input_diff = static_cast<int32_t>(input_data[outer_index * depth + inner_index]) - max_in_row;
      if (input_diff >= diff_min) {
        const int32_t input_diff_in_q5 = MultiplyByQuantizedMultiplier(input_diff, input_multiplier, input_left_shift);
        sum_of_exps_in_q12 = sum_of_exps_in_q12 + gemmlowp::Rescale<kAccumulationIntegerBits>(
    ...
    for (size_t inner_index = 0; inner_index < depth; ++inner_index) {
      ...
      if (input_diff > adjusted_diff_min) {
        ...
        int32_t output_in_q27 = gemmlowp::RoundingDivideByPOT((input_diff_in_q5 - log_sum_of_exps_in_q5),
                                                              31 - kInputIntegerBits - kOutputIntegerBits) +
                                kMaxT8;

        output_in_q27 = std::max(std::min(output_in_q27, static_cast<int32_t>(kMaxT8)), static_cast<int32_t>(kMinT8));
...
inline void LogSoftmax(const SoftmaxParams &params, const size_t outer_size, const size_t depth,
