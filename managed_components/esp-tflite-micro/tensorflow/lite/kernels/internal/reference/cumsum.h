...
template<typename T>
inline void CumSum(const T *input_data, const RuntimeShape &shape, int32_t axis, bool exclusive, bool reverse,
                   T *output_data) {
...
//
inline void CumSum(const ArithmeticParams &params, const int8_t *input_data, const RuntimeShape &shape, int32_t axis,
                   bool exclusive, bool reverse, int8_t *output_data) {
  ...
  for (size_t outer_index = 0; outer_index < outer; outer_index++) {
    ...
    for (size_t inner_index = 0; inner_index < inner; inner_index++) {
      ...
      accumulator =
          MultiplyByQuantizedMultiplierSmallerThanOneExp(accumulator, params.input1_multiplier, params.input1_shift);
