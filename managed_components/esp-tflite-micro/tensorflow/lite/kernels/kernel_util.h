...
inline int NumInputs(const TfLiteNode *node) { return node->inputs == nullptr ? 0 : node->inputs->size; }
inline int NumOutputs(const TfLiteNode *node) { return node->outputs == nullptr ? 0 : node->outputs->size; }
...
inline int64_t NumElements(const TfLiteIntArray *dims) { return NumElements(dims->data, dims->size); }
