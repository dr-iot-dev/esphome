...
MockMicroGraph::MockMicroGraph(SingleArenaBufferAllocator *allocator)
  ...
  mock_tensor_ = reinterpret_cast<TfLiteEvalTensor *>(
      allocator_->AllocatePersistentBuffer(sizeof(TfLiteEvalTensor), alignof(TfLiteEvalTensor)));
  int *dims_array = reinterpret_cast<int *>(allocator_->AllocatePersistentBuffer(3 * sizeof(int), alignof(int)));
  float *data_array =
      reinterpret_cast<float *>(allocator_->AllocatePersistentBuffer(2 * sizeof(float), alignof(float)));
  int dims[] = {2, 1, 2};
...
TfLiteEvalTensor *MockMicroGraph::GetSubgraphInput(int subgraph_idx, int tensor_idx) { return mock_tensor_; }

size_t MockMicroGraph::NumSubgraphOutputs(int subgraph_idx) { return 1; }

TfLiteEvalTensor *MockMicroGraph::GetSubgraphOutput(int subgraph_idx, int tensor_idx) { return mock_tensor_; }

int MockMicroGraph::NumSubgraphs() { return kMaxSubgraphs; }

MicroResourceVariables *MockMicroGraph::GetResourceVariables() { return nullptr; }
