...
void AllocationInfoBuilder::UpdateFirstCreated(AllocationInfo *current, int allocation_scope_count) {
...
void AllocationInfoBuilder::UpdateLastUsed(AllocationInfo *current, int allocation_scope_count) {
...
TfLiteStatus AllocationInfoBuilder::MarkSubgraphLifetimesIfNecessary(
  ...
  const OperatorCode *opcode = model_->operator_codes()->Get(op->opcode_index());
  switch (opcode->builtin_code()) {
    case BuiltinOperator_IF: {
      first_subgraph_index = op->builtin_options_as_IfOptions()->then_subgraph_index();
      second_subgraph_index = op->builtin_options_as_IfOptions()->else_subgraph_index();
      break;
    }
    case BuiltinOperator_CALL_ONCE: {
      first_subgraph_index = op->builtin_options_as_CallOnceOptions()->init_subgraph_index();
      break;
    }
    case BuiltinOperator_WHILE: {
      first_subgraph_index = op->builtin_options_as_WhileOptions()->cond_subgraph_index();
      second_subgraph_index = op->builtin_options_as_WhileOptions()->body_subgraph_index();
      break;
    }
    default: {
      break;
  ...
  if (first_subgraph_index != -1) {
    ...
    TF_LITE_ENSURE_STATUS(
        MarkAllocationLifetimes(first_subgraph_index, scratch_buffer_requests, scratch_buffer_handles, allocations));
  }
  if (second_subgraph_index != -1) {
    // Enter a new allocation scope for each subgraph.
    allocation_scope_count_++;
    TF_LITE_ENSURE_STATUS(
        MarkAllocationLifetimes(second_subgraph_index, scratch_buffer_requests, scratch_buffer_handles, allocations));
  }
  return kTfLiteOk;
}

TfLiteStatus AllocationInfoBuilder::CreateAllocationInfo(int scratch_buffer_request_count) {
  size_t subgraph_offsets_length = model_->subgraphs()->size() * sizeof(size_t);
  info_.subgraph_offsets =
      reinterpret_cast<size_t *>(non_persistent_allocator_->AllocateTemp(subgraph_offsets_length, alignof(size_t)));
  if (info_.subgraph_offsets == nullptr) {
    MicroPrintf("Failed to allocate memory for memory planning, %d bytes required", subgraph_offsets_length);
    return kTfLiteError;
  }
  size_t tensor_count = 0;
  for (size_t subgraph_idx = 0; subgraph_idx < model_->subgraphs()->size(); subgraph_idx++) {
  ...
  info_.allocation_info =
      reinterpret_cast<AllocationInfo *>(non_persistent_allocator_->AllocateTemp(bytes, alignof(AllocationInfo)));
  if (info_.allocation_info == nullptr) {
    MicroPrintf("Failed to allocate memory for memory planning, %d bytes required", bytes);
    return kTfLiteError;
  }
  return kTfLiteOk;
}
...
TfLiteStatus AllocationInfoBuilder::FreeAllocationInfo() {
  non_persistent_allocator_->DeallocateTemp(reinterpret_cast<uint8_t *>(info_.allocation_info));
  non_persistent_allocator_->DeallocateTemp(reinterpret_cast<uint8_t *>(info_.subgraph_offsets));
  return kTfLiteOk;
}
...
TfLiteStatus AllocationInfoBuilder::ValidateSubgraph(const SubGraph *subgraph, TfLiteEvalTensor *eval_tensors) {
  ...
  for (uint32_t i = 0; i < operators_size; i++) {
    ...
    for (size_t n = 0; op->intermediates() != nullptr && n < op->intermediates()->size(); n++) {
      ...
      TF_LITE_ENSURE_STATUS(TfLiteEvalTensorByteLength(&eval_tensors[tensor_index], &tensor_size));
      if (tensor_size != 0) {
        MicroPrintf("Does not support intermediate tensor with non-zero size: %d", tensor_size);
        return kTfLiteError;
      }
    }
  }
...
TfLiteStatus AllocationInfoBuilder::InitializeAllocationInfo(const int32_t *offline_offsets,
  ...
  for (size_t subgraph_idx = 0; subgraph_idx < model_->subgraphs()->size(); subgraph_idx++) {
    ...
    AllocationInfo *subgraph_allocation_info = &allocation_info[info_.subgraph_offsets[subgraph_idx]];
    ...
    for (size_t i = 0; i < subgraph->tensors()->size(); ++i) {
      ...
      TF_LITE_ENSURE_STATUS(TfLiteEvalTensorByteLength(&eval_tensors[i], &current->bytes));
      ...
      current->needs_allocating = (eval_tensors[i].data.data == nullptr) &&
                                  (!subgraph->tensors()->Get(i)->is_variable()) && (current->bytes != 0);
      if (offline_offsets) {
        ...
        if (subgraph->tensors()->Get(i)->is_variable() && current->offline_offset != kOnlinePlannedBuffer) {
  ...
  AllocationInfo *scratch_allocation_info = &allocation_info[info_.scratch_offset];
...
TfLiteStatus AllocationInfoBuilder::MarkAllocationLifetimes(int subgraph_idx,
  ...
  for (size_t i = 0; subgraph->inputs() != nullptr && i < subgraph->inputs()->size(); ++i) {
  ...
  for (uint32_t i = 0; i < operators_size; i++) {
    ...
    for (size_t n = 0; op->outputs() != nullptr && n < op->outputs()->size(); ++n) {
    ...
    MarkSubgraphLifetimesIfNecessary(op, scratch_buffer_requests, scratch_buffer_handles, allocations);

    // Figure out when the last use of each tensor is.
    for (size_t n = 0; op->inputs() != nullptr && n < op->inputs()->size(); ++n) {
    ...
    AllocationInfo *scratch_allocation_info = &allocation_info[info_.scratch_offset];
    for (size_t scratch_idx = 0; scratch_idx < info_.scratch_buffer_count; scratch_idx++) {
      internal::ScratchBufferRequest request = scratch_buffer_requests[scratch_idx];
      AllocationInfo *current = &scratch_allocation_info[scratch_idx];
      if (request.node_idx == static_cast<int>(i) && request.subgraph_idx == static_cast<int>(subgraph_idx)) {
        ScratchBufferHandle *current_handle = &(scratch_buffer_handles[scratch_idx]);
  ...
  for (size_t i = 0; subgraph->outputs() != nullptr && i < subgraph->outputs()->size(); ++i) {
...
TfLiteStatus AllocationInfoBuilder::GetOfflinePlannedOffsets(const int32_t **offline_planner_offsets) {
  if (model_->metadata()) {
    for (size_t i = 0; i < model_->metadata()->size(); ++i) {
      ...
      if (metadata->name()) {
        ...
        if ((strncmp(metadata->name()->c_str(), kOfflineMemAllocMetadata,
                     std::min(metadata_name_size, strlen(kOfflineMemAllocMetadata))) == 0) &&
          ...
          const uint32_t *metadata_buffer = reinterpret_cast<const uint32_t *>(array->data());
          const size_t nbr_tensors = static_cast<size_t>(metadata_buffer[2]);
          *offline_planner_offsets = reinterpret_cast<const int32_t *>(&metadata_buffer[3]);

          if (info_.tensor_count != nbr_tensors) {
            MicroPrintf("Nbr of offline buffer offsets (%d) in metadata "
