...
class StackAllocator : public flatbuffers::Allocator {
 public:
  StackAllocator(size_t alignment) : data_size_(0) { data_ = AlignPointerUp(data_backing_, alignment); }
...
flatbuffers::FlatBufferBuilder *BuilderInstance() {
  ...
  static flatbuffers::FlatBufferBuilder *inst = new (inst_memory) flatbuffers::FlatBufferBuilder(
      StackAllocator::kStackAllocatorSize, &StackAllocator::instance(MicroArenaBufferAlignment()));
  return inst;
}
...
class ModelBuilder {
 public:
  ...
  explicit ModelBuilder(flatbuffers::FlatBufferBuilder *builder) : builder_(builder) {}
  ...
  Tensor AddVariableTensor(TensorType type, std::initializer_list<int32_t> shape) {
    return AddTensorImpl(type, /* is_variable */ true, shape);
  }

  // Adds a node to the model with given input and output Tensors.
  Node AddNode(Operator op, std::initializer_list<Tensor> inputs, std::initializer_list<Tensor> outputs,
               std::initializer_list<Tensor> intermediates = std::initializer_list<Tensor>{});

  void AddMetadata(const char *description_string, const int32_t *metadata_buffer_data, size_t num_elements);
  ...
  const Model *BuildModel(std::initializer_list<Tensor> inputs, std::initializer_list<Tensor> outputs,
                          size_t num_subgraph_inputs = 0);

 private:
  // Adds a tensor to the model.
  Tensor AddTensorImpl(TensorType type, bool is_variable, std::initializer_list<int32_t> shape);
...
ModelBuilder::Operator ModelBuilder::RegisterOp(BuiltinOperator op, const char *custom_code) {
  TFLITE_DCHECK(next_operator_code_id_ <= kMaxOperatorCodes);
  operator_codes_[next_operator_code_id_] =
      tflite::CreateOperatorCodeDirect(*builder_, /*deprecated_builtin_code=*/0, custom_code, /*version=*/0, op);
  next_operator_code_id_++;
  return next_operator_code_id_ - 1;
}

ModelBuilder::Node ModelBuilder::AddNode(ModelBuilder::Operator op, std::initializer_list<ModelBuilder::Tensor> inputs,
  ...
  operators_[next_operator_id_] = tflite::CreateOperator(
      *builder_, op, builder_->CreateVector(inputs.begin(), inputs.size()),
      builder_->CreateVector(outputs.begin(), outputs.size()), BuiltinOptions_NONE,
      /*builtin_options=*/0,
      /*custom_options=*/0, tflite::CustomOptionsFormat_FLEXBUFFERS,
      /*mutating_variable_inputs =*/0, builder_->CreateVector(intermediates.begin(), intermediates.size()));
  next_operator_id_++;
  return next_operator_id_ - 1;
}

void ModelBuilder::AddMetadata(const char *description_string, const int32_t *metadata_buffer_data,
                               size_t num_elements) {
  metadata_[ModelBuilder::nbr_of_metadata_buffers_] =
      CreateMetadata(*builder_, builder_->CreateString(description_string), 1 + ModelBuilder::nbr_of_metadata_buffers_);

  metadata_buffers_[nbr_of_metadata_buffers_] = tflite::CreateBuffer(
      *builder_, builder_->CreateVector((uint8_t *) metadata_buffer_data, sizeof(uint32_t) * num_elements));

  ModelBuilder::nbr_of_metadata_buffers_++;
}

const Model *ModelBuilder::BuildModel(std::initializer_list<ModelBuilder::Tensor> inputs,
                                      std::initializer_list<ModelBuilder::Tensor> outputs, size_t num_subgraph_inputs) {
  ...
  const flatbuffers::Offset<SubGraph> subgraphs[subgraphs_size] = {tflite::CreateSubGraph(
      *builder_, builder_->CreateVector(tensors_, next_tensor_id_),
      builder_->CreateVector(inputs.begin(), num_subgraph_inputs),
      builder_->CreateVector(outputs.begin(), outputs.size()), builder_->CreateVector(operators_, next_operator_id_),
      builder_->CreateString("test_subgraph"))};

  flatbuffers::Offset<Model> model_offset;
  if (ModelBuilder::nbr_of_metadata_buffers_ > 0) {
    model_offset =
        tflite::CreateModel(*builder_, 0, builder_->CreateVector(operator_codes_, next_operator_code_id_),
                            builder_->CreateVector(subgraphs, subgraphs_size), builder_->CreateString("teset_model"),
...
ModelBuilder::Tensor ModelBuilder::AddTensorImpl(TensorType type, bool is_variable,
                                                 std::initializer_list<int32_t> shape) {
  TFLITE_DCHECK(next_tensor_id_ <= kMaxTensors);
  tensors_[next_tensor_id_] = tflite::CreateTensor(*builder_, builder_->CreateVector(shape.begin(), shape.size()), type,
...
const Model *BuildSimpleStatefulModel() {
  ...
  const int op_id = model_builder.RegisterOp(BuiltinOperator_CUSTOM, "simple_stateful_op");
  ...
  const int invoke_count_tensor = model_builder.AddTensor(TensorType_INT32, {1});
  const int intermediate_tensor = model_builder.AddTensor(TensorType_FLOAT32, {0});

  model_builder.AddNode(op_id, {input_tensor}, {median_tensor, invoke_count_tensor}, {intermediate_tensor});
  return model_builder.BuildModel({input_tensor}, {median_tensor, invoke_count_tensor});
}
 ...
 t2  |          v
     ...
     +-->|    n2   |
         ...
         +-------|-+
                 ...
                 v
  */
  const int op_id = model_builder.RegisterOp(BuiltinOperator_CUSTOM, "mock_custom");
...
const Model *BuildModelWithOfflinePlanning(int number_of_tensors, const int32_t *metadata_buffer,
                                           NodeConnection *node_conn, int num_conns, int num_subgraph_inputs) {
  using flatbuffers::Offset;
  flatbuffers::FlatBufferBuilder *fb_builder = BuilderInstance();
  ...
  model_builder.AddMetadata("OfflineMemoryAllocation", metadata_buffer,
                            number_of_tensors + tflite::testing::kOfflinePlannerHeaderSize);

  return model_builder.BuildModel(node_conn[0].input, node_conn[num_conns - 1].output, num_subgraph_inputs);
}
...
const Model *BuildModelWithUnusedInputs() {
  ...
  const Offset<Tensor> tensors[tensors_size] = {
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT8, 0,
  ...
  const Offset<Operator> operators[operators_size] = {
      CreateOperator(*builder, 0, builder->CreateVector(operator_inputs, operator_inputs_size),
                     builder->CreateVector(operator_outputs, operator_outputs_size), BuiltinOptions_NONE),
  };
  constexpr size_t subgraphs_size = 1;
  const Offset<SubGraph> subgraphs[subgraphs_size] = {
      CreateSubGraph(*builder, builder->CreateVector(tensors, tensors_size), builder->CreateVector(inputs, inputs_size),
                     builder->CreateVector(outputs, outputs_size), builder->CreateVector(operators, operators_size),
  ...
  const Offset<OperatorCode> operator_codes[operator_codes_size] = {
      CreateOperatorCodeDirect(*builder, /*deprecated_builtin_code=*/0, "mock_custom",
                               /*version=*/0, BuiltinOperator_CUSTOM)};
  const Offset<Model> model_offset =
      CreateModel(*builder, 0, builder->CreateVector(operator_codes, operator_codes_size),
                  builder->CreateVector(subgraphs, subgraphs_size), builder->CreateString("test_model"),
...
const Model *BuildModelWith256x256Tensor() {
  ...
  const int op_id = model_builder.RegisterOp(BuiltinOperator_CUSTOM, "mock_custom");
  const int input1_tensor = model_builder.AddTensor(TensorType_INT8, {256, 256});
  const int input2_tensor = model_builder.AddTensor(TensorType_INT8, {256, 256});
  const int output_tensor = model_builder.AddTensor(TensorType_INT8, {256, 256});

  model_builder.AddNode(op_id, {input1_tensor, input2_tensor}, {output_tensor});
  return model_builder.BuildModel({input1_tensor, input2_tensor}, {output_tensor});
}
...
const Model *BuildSimpleMockModel() {
  ...
  const Offset<Buffer> buffers[buffers_size] = {
      CreateBuffer(*builder), CreateBuffer(*builder, builder->CreateVector(buffer_data, buffer_data_size))};
  constexpr size_t tensor_shape_size = 1;
  const int32_t tensor_shape[tensor_shape_size] = {1};
  constexpr size_t tensors_size = 4;
  const Offset<Tensor> tensors[tensors_size] = {
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT32, 0,
                   builder->CreateString("test_input_tensor"), 0, false),
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT8, 1,
  ...
  const Offset<Operator> operators[operators_size] = {
      CreateOperator(*builder, 0, builder->CreateVector(operator_inputs, operator_inputs_size),
                     builder->CreateVector(operator_outputs, operator_outputs_size), BuiltinOptions_NONE),
      CreateOperator(*builder, 0, builder->CreateVector(operator_inputs, operator_inputs_size),
                     builder->CreateVector(operator2_outputs, operator_outputs_size), BuiltinOptions_NONE),
  };
  constexpr size_t subgraphs_size = 1;
  const Offset<SubGraph> subgraphs[subgraphs_size] = {
      CreateSubGraph(*builder, builder->CreateVector(tensors, tensors_size), builder->CreateVector(inputs, inputs_size),
...
const Model *BuildComplexMockModel() {
  ...
  const Offset<Buffer> buffers[buffers_size] = {
      ...
      CreateBuffer(*builder),
      CreateBuffer(*builder, builder->CreateVector(buffer_data_1, buffer_data_size)),
      // Op 2 buffers:
      CreateBuffer(*builder),
      CreateBuffer(*builder, builder->CreateVector(buffer_data_2, buffer_data_size)),
      // Op 3 buffers:
      CreateBuffer(*builder),
      CreateBuffer(*builder, builder->CreateVector(buffer_data_3, buffer_data_size)),
  };
  constexpr size_t tensor_shape_size = 1;
  const int32_t tensor_shape[tensor_shape_size] = {1};
  ...
  const Offset<Tensor> tensors[tensors_size] = {
      // Op 1 inputs:
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT32, 0,
                   builder->CreateString("test_input_tensor_1"), 0, false /* is_variable */),
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT32, 1,
                   builder->CreateString("test_variable_tensor_1"), 0, true /* is_variable */),
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT8, 2,
                   builder->CreateString("test_weight_tensor_1"), 0, false /* is_variable */),
      // Op 1 output / Op 2 input:
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT32, 0,
                   builder->CreateString("test_output_tensor_1"), 0, false /* is_variable */),
      // Op 2 inputs:
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT32, 1,
                   builder->CreateString("test_variable_tensor_2"), 0, true /* is_variable */),
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT8, 2,
                   builder->CreateString("test_weight_tensor_2"), 0, false /* is_variable */),
      // Op 2 output / Op 3 input:
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT32, 0,
                   builder->CreateString("test_output_tensor_2"), 0, false /* is_variable */),
      // Op 3 inputs:
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT32, 1,
                   builder->CreateString("test_variable_tensor_3"), 0, true /* is_variable */),
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT8, 2,
                   builder->CreateString("test_weight_tensor_3"), 0, false /* is_variable */),
      // Op 3 output:
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT32, 0,
                   builder->CreateString("test_output_tensor_3"), 0, false /* is_variable */),
  };
  ...
  Offset<Operator> operators[operators_size];
  {
    ...
    operators[0] = {CreateOperator(*builder, 0, builder->CreateVector(operator_inputs, operator_inputs_size),
...

  {
    ...
    operators[1] = {CreateOperator(*builder, 0, builder->CreateVector(operator_inputs, operator_inputs_size),
                                   builder->CreateVector(operator_outputs, operator_outputs_size),
                                   BuiltinOptions_NONE)};
  }

  {
    ...
    operators[2] = {CreateOperator(*builder, 0, builder->CreateVector(operator_inputs, operator_inputs_size),
                                   builder->CreateVector(operator_outputs, operator_outputs_size),
                                   BuiltinOptions_NONE)};
  }
...
const Model *BuildSimpleModelWithSubgraphsAndIf() {
  ...
  const Offset<Tensor> subgraph1_tensors[tensors_size] = {
      CreateTensor(*builder, builder->CreateVector(condition_tensor_shape, 1), TensorType_BOOL, 0,
                   builder->CreateString("condition tensor"), 0, false),
      CreateTensor(*builder, builder->CreateVector(data_tensor_shape, 2), TensorType_FLOAT32, 0,
  ...
  const Offset<Operator> subgraph1_operators[operators_size] = {
      CreateOperator(*builder, 0, builder->CreateVector(if_inputs, if_inputs_size),
                     builder->CreateVector(if_outputs, outputs_size), BuiltinOptions_IfOptions,
                     CreateIfOptions(*builder, 1, 2).Union()),
  };
  const Offset<Operator> subgraph2_operators[operators_size] = {
      CreateOperator(*builder, 1, builder->CreateVector(operator_inputs, operator_inputs_size),
                     builder->CreateVector(operator_outputs, outputs_size), BuiltinOptions_NONE),
  };
  const Offset<Operator> subgraph3_operators[operators_size] = {
      CreateOperator(*builder, 2, builder->CreateVector(operator_inputs, operator_inputs_size),
                     builder->CreateVector(operator_outputs, outputs_size), BuiltinOptions_NONE),
  };
  constexpr size_t subgraphs_size = 3;
  const Offset<SubGraph> subgraphs[subgraphs_size] = {
      CreateSubGraph(*builder, builder->CreateVector(subgraph1_tensors, 4),
                     builder->CreateVector(if_inputs, if_inputs_size), builder->CreateVector(if_outputs, outputs_size),
                     builder->CreateVector(subgraph1_operators, operators_size), builder->CreateString("if_subgraph")),
      ...
      CreateSubGraph(*builder, builder->CreateVector(subgraph3_tensors, 3),
                     builder->CreateVector(operator_inputs, operator_inputs_size),
                     builder->CreateVector(operator_outputs, outputs_size),
...
const Model *BuildSimpleModelWithIfAndEmptySubgraph() {
  ...
  const Offset<SubGraph> subgraphs[subgraphs_size] = {
      ...
      CreateSubGraph(*builder, builder->CreateVector(subgraph3_tensors, 3),
                     builder->CreateVector(operator_inputs, operator_inputs_size),
                     builder->CreateVector(operator_outputs, outputs_size), 0, builder->CreateString("else_subgraph")),
  };
  constexpr size_t operator_codes_size = 3;
  const Offset<OperatorCode> operator_codes[operator_codes_size] = {
      CreateOperatorCodeDirect(*builder, /*deprecated_builtin_code=*/0, "multiple_inputs_op",
...
const Model *BuildSimpleModelWithSubgraphsAndWhile() {
  ...
  const Offset<Tensor> subgraph0_tensors[while_tensors_size] = {
      CreateTensor(*builder, builder->CreateVector(data_tensor_shape, 1), TensorType_FLOAT32, 0,
  ...
  const Offset<Tensor> subgraph1_tensors[op_tensors_size] = {
      ...
      CreateTensor(*builder, builder->CreateVector(data_tensor_shape, 1), TensorType_FLOAT32, 0,
                   builder->CreateString("input_tensor2"), 0, false),
      CreateTensor(*builder, builder->CreateVector(data_tensor_shape, 1), TensorType_BOOL, 0,
  ...
  const Offset<Operator> subgraph0_operators[operators_size] = {
      CreateOperator(*builder, 0, builder->CreateVector(inputs, inputs_size),
                     builder->CreateVector(while_outputs, while_outputs_size), BuiltinOptions_WhileOptions,
  ...
  const Offset<Operator> subgraph2_operators[operators_size] = {
      CreateOperator(*builder, 2, builder->CreateVector(inputs, inputs_size),
                     builder->CreateVector(add_outputs, add_outputs_size), BuiltinOptions_NONE),
  };
  constexpr size_t subgraphs_size = 3;
  const Offset<SubGraph> subgraphs[subgraphs_size] = {
      CreateSubGraph(*builder, builder->CreateVector(subgraph0_tensors, 4), builder->CreateVector(inputs, inputs_size),
      ...
      CreateSubGraph(*builder, builder->CreateVector(subgraph1_tensors, 3), builder->CreateVector(inputs, inputs_size),
      ...
      CreateSubGraph(*builder, builder->CreateVector(subgraph2_tensors, 3), builder->CreateVector(inputs, inputs_size),
                     builder->CreateVector(add_subgraph_outputs, add_subgraph_outputs_size),
                     builder->CreateVector(subgraph2_operators, operators_size),
...
const Model *BuildModelWithIfAndSubgraphInputTensorOverlap() {
  ...
  constexpr int kBlockSize = tflite::MicroArenaBufferAlignment() / sizeof(int32_t);
  ...
  const Offset<Tensor> kSubgraph0Tensors[kSubgraph0TensorsCount] = {
      CreateTensor(*builder, builder->CreateVector(kConditionTensorShape, 1), TensorType_BOOL, 0,
                   builder->CreateString("condition tensor"), 0, false),
      CreateTensor(*builder, builder->CreateVector(kIfInput1TensorShape, 1), kTensorType, 0,
                   builder->CreateString("if_input_tensor1"), 0, false),
      CreateTensor(*builder, builder->CreateVector(kIfInput2TensorShape, 1), kTensorType, 0,
                   builder->CreateString("if_input_tensor2"), 0, false),
      CreateTensor(*builder, builder->CreateVector(kIfOutputTensorShape, 1), kTensorType, 0,
                   builder->CreateString("if_output_tensor"), 0, false),
      CreateTensor(*builder, builder->CreateVector(kFinalOutputTensorShape, 1), kTensorType, 0,
  ...
  const Offset<Operator> kSubgraph0Operators[kOperatorsCount] = {
      CreateOperator(*builder, kIfOpCodeIndex, builder->CreateVector(kIfInputs, kIfInputsCount),
                     builder->CreateVector(kIfOutputs, kOutputsCount), BuiltinOptions_IfOptions,
                     CreateIfOptions(*builder, 1, 2).Union()),
      CreateOperator(*builder, kCustomOpCodeIndex, builder->CreateVector(kOpAfterIfInputs, kOpAfterIfInputsCount),
  ...
  const Offset<Operator> kSubgraph1Operators[kSubgraph1OperatorsCount] = {
      CreateOperator(*builder, kCustomOpCodeIndex, builder->CreateVector(kSubgraph1Inputs, kSubgraph1InputsCount),
                     builder->CreateVector(kSubgraph1Outputs, kSubgraph1OutputsCount), BuiltinOptions_NONE),
  };
  ...
  const Offset<Operator> kSubgraph2Operators[kSubgraph2OperatorsCount] = {
      CreateOperator(*builder, kCustomOpCodeIndex, builder->CreateVector(kSubgraph2Inputs, kSubgraph2InputsCount),
                     builder->CreateVector(kSubgraph2Outputs, kSubgraph2OutputsCount), BuiltinOptions_NONE),
  };
  ...
  const Offset<SubGraph> kSubgraphs[kSubgraphsCount] = {
      CreateSubGraph(*builder, builder->CreateVector(kSubgraph0Tensors, kSubgraph0TensorsCount),
                     ...
                     builder->CreateVector(kOpAfterIfOutputs, kOutputsCount),
                     builder->CreateVector(kSubgraph0Operators, kOperatorsCount), builder->CreateString("if_subgraph")),
      CreateSubGraph(*builder, builder->CreateVector(kSubgraph1Tensors, kSubgraph1TensorsCount),
                     ...
                     builder->CreateString("then_subgraph")),
      CreateSubGraph(*builder, builder->CreateVector(kSubgraph2Tensors, kSubgraph2TensorsCount),
  ...
  const Offset<OperatorCode> kOperatorCodes[kOperatorCodesCount] = {
      CreateOperatorCodeDirect(*builder, /*deprecated_builtin_code=*/0, "if",
                               /*version=*/0, BuiltinOperator_IF),
      CreateOperatorCodeDirect(*builder, /*deprecated_builtin_code=*/0, "custom_packer_op",
                               /*version=*/0, BuiltinOperator_CUSTOM),
  };
  const Offset<Model> kModelOffset =
      CreateModel(*builder, 0, builder->CreateVector(kOperatorCodes, kOperatorCodesCount),
                  builder->CreateVector(kSubgraphs, kSubgraphsCount), builder->CreateString("test_model"),
...
const Model *BuildSimpleMockModelWithNullInputsOutputs() {
  ...
  constexpr size_t operators_size = 1;
  const Offset<Operator> subgraph0_operators[operators_size] = {
      CreateOperator(*builder, 0, {}, {}, BuiltinOptions_CallOnceOptions, CreateCallOnceOptions(*builder, 1).Union()),
  };
  const Offset<Operator> subgraph1_operators[operators_size] = {
  ...
  const Offset<OperatorCode> operator_codes[operator_codes_size] = {
      CreateOperatorCodeDirect(*builder, /*deprecated_builtin_code=*/0, "call_once_op",
...
const TFLMRegistration *SimpleStatefulOp::getRegistration() { return GetMutableRegistration(); }
...
void *SimpleStatefulOp::Init(TfLiteContext *context, const char *buffer, size_t length) {
...
TfLiteStatus SimpleStatefulOp::Prepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  data->invoke_count = reinterpret_cast<int *>(context->AllocatePersistentBuffer(context, sizeof(int)));
  *data->invoke_count = 0;
...
TfLiteStatus SimpleStatefulOp::Invoke(TfLiteContext *context, TfLiteNode *node) {
  OpData *data = reinterpret_cast<OpData *>(node->user_data);
  *data->invoke_count += 1;
  ...
  uint8_t *sorting_buffer = reinterpret_cast<uint8_t *>(context->GetScratchBuffer(context, data->sorting_buffer));
...
const TFLMRegistration *MockCustom::getRegistration() { return GetMutableRegistration(); }
...
void *MockCustom::Init(TfLiteContext *context, const char *buffer, size_t length) {
...
TfLiteStatus MockCustom::Prepare(TfLiteContext *context, TfLiteNode *node) { return kTfLiteOk; }

TfLiteStatus MockCustom::Invoke(TfLiteContext *context, TfLiteNode *node) {
  const TfLiteEvalTensor *input = tflite::micro::GetEvalInput(context, node, 0);
  TF_LITE_ENSURE(context, input != nullptr);
  const int32_t *input_data = input->data.i32;
  const TfLiteEvalTensor *weight = tflite::micro::GetEvalInput(context, node, 1);
...
const TFLMRegistration *MultipleInputs::getRegistration() { return GetMutableRegistration(); }
...
void *MultipleInputs::Init(TfLiteContext *context, const char *buffer, size_t length) {
  // We don't support delegate in TFL micro. This is a weak check to test if
  // context struct being zero-initialized.
  TFLITE_DCHECK(context->ReplaceNodeSubsetsWithDelegateKernels == nullptr);
  freed_ = false;
...
void MultipleInputs::Free(TfLiteContext *context, void *buffer) { freed_ = true; }

TfLiteStatus MultipleInputs::Prepare(TfLiteContext *context, TfLiteNode *node) { return kTfLiteOk; }
...
const TFLMRegistration *NoOp::getRegistration() { return GetMutableRegistration(); }
...
TfLiteStatus NoOp::Prepare(TfLiteContext *context, TfLiteNode *node) { return kTfLiteOk; }

TfLiteStatus NoOp::Invoke(TfLiteContext *context, TfLiteNode *node) { return kTfLiteOk; }

bool NoOp::freed_ = false;

TfLiteStatus GetTestingOpResolver(tflite::testing::TestingOpResolver &op_resolver) {
  TF_LITE_ENSURE_STATUS(op_resolver.AddCustom("mock_custom", MockCustom::GetMutableRegistration()));
  TF_LITE_ENSURE_STATUS(op_resolver.AddCustom("simple_stateful_op", SimpleStatefulOp::GetMutableRegistration()));
  TF_LITE_ENSURE_STATUS(op_resolver.AddCustom("multiple_inputs_op", MultipleInputs::GetMutableRegistration()));
  TF_LITE_ENSURE_STATUS(op_resolver.AddCustom("no_op", NoOp::GetMutableRegistration()));
  TF_LITE_ENSURE_STATUS(op_resolver.AddCustom("custom_packer_op", PackerOp::GetMutableRegistration()));
...
const Model *GetModelWithOfflinePlanning(int num_tensors, const int32_t *metadata_buffer, NodeConnection *node_conn,
                                         int num_conns, int num_subgraph_inputs) {
  const Model *model =
      BuildModelWithOfflinePlanning(num_tensors, metadata_buffer, node_conn, num_conns, num_subgraph_inputs);
  return model;
}
...
const Tensor *Create1dFlatbufferTensor(int size, bool is_variable) {
  ...
  const Offset<Tensor> tensor_offset =
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT32, 0,
                   builder->CreateString("test_tensor"), 0, is_variable);
...
const Tensor *CreateQuantizedFlatbufferTensor(int size) {
  ...
  const Offset<QuantizationParameters> quant_params =
      CreateQuantizationParameters(*builder,
  ...
  const Offset<Tensor> tensor_offset =
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT32, 0,
                   builder->CreateString("test_tensor"), quant_params, false);
  builder->Finish(tensor_offset);
  void *tensor_pointer = builder->GetBufferPointer();
  const Tensor *tensor = flatbuffers::GetRoot<Tensor>(tensor_pointer);
  return tensor;
...
const Tensor *CreateMissingQuantizationFlatbufferTensor(int size) {
  ...
  flatbuffers::FlatBufferBuilder *builder = BuilderInstance();
  const Offset<QuantizationParameters> quant_params =
      CreateQuantizationParameters(*builder, 0, 0, 0, 0, QuantizationDetails_NONE, 0, 0);
  constexpr size_t tensor_shape_size = 1;
  const int32_t tensor_shape[tensor_shape_size] = {size};
  const Offset<Tensor> tensor_offset =
      CreateTensor(*builder, builder->CreateVector(tensor_shape, tensor_shape_size), TensorType_INT32, 0,
...
const flatbuffers::Vector<flatbuffers::Offset<Buffer>> *CreateFlatbufferBuffers() {
  ...
  const flatbuffers::Offset<flatbuffers::Vector<flatbuffers::Offset<Buffer>>> buffers_offset =
      builder->CreateVector(buffers, buffers_size);
  ...
  const flatbuffers::Vector<flatbuffers::Offset<Buffer>> *result =
      flatbuffers::GetRoot<flatbuffers::Vector<flatbuffers::Offset<Buffer>>>(buffers_pointer);
  return result;
}
...
TfLiteTensor CreateQuantizedBiasTensor(const float *data, int16_t *quantized, TfLiteIntArray *dims, float input_scale,
  ...
  // processing.
  TfLiteTensor result = CreateQuantizedTensor(quantized, dims, bias_scale, 0, is_variable);
  return result;
}

TfLiteTensor CreateQuantizedBiasTensor(const float *data, int32_t *quantized, TfLiteIntArray *dims, float input_scale,
                                       float weights_scale, bool is_variable) {
  float bias_scale = input_scale * weights_scale;
  tflite::SymmetricQuantize(data, quantized, ElementCount(*dims), bias_scale);
...
TfLiteTensor CreateQuantizedBiasTensor(const float *data, std::int64_t *quantized, TfLiteIntArray *dims,
                                       float input_scale, float weights_scale, bool is_variable) {
  float bias_scale = input_scale * weights_scale;
  tflite::SymmetricQuantize(data, quantized, ElementCount(*dims), bias_scale);
...
template<typename T>
TfLiteTensor CreatePerChannelQuantizedBiasTensor(const float *input, T *quantized, TfLiteIntArray *dims,
  ...
  SymmetricPerChannelQuantize<T>(input, quantized, input_size, num_channels, scales_array);
...
TfLiteTensor CreatePerChannelQuantizedBiasTensor(const float *input, int32_t *quantized, TfLiteIntArray *dims,
                                                 float input_scale, float *weight_scales, float *scales,
                                                 int *zero_points, TfLiteAffineQuantization *affine_quant,
                                                 int quantized_dimension, bool is_variable) {
  return CreatePerChannelQuantizedBiasTensor<int32_t>(input, quantized, dims, input_scale, weight_scales, scales,
                                                      zero_points, affine_quant, quantized_dimension, is_variable);
}

TfLiteTensor CreatePerChannelQuantizedBiasTensor(const float *input, std::int64_t *quantized, TfLiteIntArray *dims,
                                                 float input_scale, float *weight_scales, float *scales,
                                                 int *zero_points, TfLiteAffineQuantization *affine_quant,
                                                 int quantized_dimension, bool is_variable) {
  return CreatePerChannelQuantizedBiasTensor<std::int64_t>(input, quantized, dims, input_scale, weight_scales, scales,
                                                           zero_points, affine_quant, quantized_dimension, is_variable);
}

TfLiteTensor CreateSymmetricPerChannelQuantizedTensor(const float *input, int8_t *quantized, TfLiteIntArray *dims,
                                                      float *scales, int *zero_points,
                                                      TfLiteAffineQuantization *affine_quant, int quantized_dimension,
                                                      bool is_variable, TfLiteType tensor_weight_type) {
  int channel_count = dims->data[quantized_dimension];

  scales[0] = static_cast<float>(channel_count);
  zero_points[0] = channel_count;

  SignedSymmetricPerChannelQuantize(input, dims, quantized_dimension, quantized, &scales[1], tensor_weight_type);
  ...
  affine_quant->zero_point = IntArrayFromInts(zero_points);
  affine_quant->quantized_dimension = quantized_dimension;
  TfLiteTensor result = CreateTensor(quantized, dims, is_variable, tensor_weight_type);
  result.quantization = {kTfLiteAffineQuantization, affine_quant};
  return result;
}
