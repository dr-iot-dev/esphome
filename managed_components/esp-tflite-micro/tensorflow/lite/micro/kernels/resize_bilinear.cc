...
TfLiteStatus ResizeBilinearPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TF_LITE_ENSURE_MSG(context, IsConstantTensor(size), "Non constant size tensor not supported");

  // Ensure params are valid.
  auto *params = reinterpret_cast<TfLiteResizeBilinearParams *>(node->builtin_data);
...
TfLiteStatus ResizeBilinearEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (output->type == kTfLiteFloat32) {
    ...
    reference_ops::ResizeBilinear(op_params, tflite::micro::GetTensorShape(input),
                                  tflite::micro::GetTensorData<float>(input), tflite::micro::GetTensorShape(size),
                                  tflite::micro::GetTensorData<int32_t>(size), tflite::micro::GetTensorShape(output),
...
TFLMRegistration Register_RESIZE_BILINEAR() {
  return tflite::micro::RegisterOp(nullptr, ResizeBilinearPrepare, ResizeBilinearEval);
}
