...
TfLiteStatus NegEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  switch (input->type) {
    ...
    case kTfLiteFloat32:
      reference_ops::Negate(tflite::micro::GetTensorShape(input), tflite::micro::GetTensorData<float>(input),
...
TFLMRegistration Register_NEG() { return tflite::micro::RegisterOp(nullptr, nullptr, NegEval); }
