...
template<typename T>
TfLiteStatus EnsureEqImpl(TfLiteContext *context, const TfLiteIntArray *array, const TfLiteTensor *tensor) {
...
TfLiteStatus EnsureEq(TfLiteContext *context, const TfLiteIntArray *array, const TfLiteTensor *tensor) {
  ...
  switch (tensor->type) {
    ...
    default:
      MicroPrintf("cannot compare int array to tensor of type %d.", tensor->type);
      return kTfLiteError;
  }
}
...
TfLiteStatus FillPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteTensor *value = micro_context->AllocateTempInputTensor(node, kValueTensor);
...
template<typename T> void FillImpl(const TfLiteEvalTensor *value, TfLiteEvalTensor *output) {
  reference_ops::Fill(micro::GetTensorShape(value), micro::GetTensorData<T>(value), micro::GetTensorShape(output),
                      micro::GetTensorData<T>(output));
}

TfLiteStatus FillEval(TfLiteContext *context, TfLiteNode *node) {
  const TfLiteEvalTensor *value = micro::GetEvalInput(context, node, kValueTensor);
  ...
  switch (value->type) {
    ...
    case kTfLiteInt8:
      ...
      break;
    default:
      MicroPrintf("Fill only currently supports float32 for input 1, got %d.", TfLiteTypeGetName(value->type));
      return kTfLiteError;
  }
...
TFLMRegistration Register_FILL() { return tflite::micro::RegisterOp(nullptr, FillPrepare, FillEval); }
