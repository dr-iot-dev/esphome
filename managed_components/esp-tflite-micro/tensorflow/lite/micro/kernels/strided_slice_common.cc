...
struct StridedSliceContext {
  StridedSliceContext(TfLiteContext *context, TfLiteNode *node) {
    ...
    input = micro_context->AllocateTempInputTensor(node, kStridedSliceInputTensor);
    begin = micro_context->AllocateTempInputTensor(node, kStridedSliceBeginTensor);
    end = micro_context->AllocateTempInputTensor(node, kStridedSliceEndTensor);
    strides = micro_context->AllocateTempInputTensor(node, kStridedSliceStridesTensor);
    output = micro_context->AllocateTempOutputTensor(node, kStridedSliceOutputTensor);
...
tflite::StridedSliceParams BuildStridedSliceParams(StridedSliceContext *op_context) {
...
TfLiteStatus CheckOutputSize(TfLiteContext *context, StridedSliceContext *op_context) {
...
TfLiteStatus StridedSlicePrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  StridedSliceParams *op_params = static_cast<StridedSliceParams *>(node->user_data);
  TF_LITE_ENSURE_EQ(context, NumInputs(node), 4);
  TF_LITE_ENSURE_EQ(context, NumOutputs(node), 1);
  StridedSliceContext op_context(context, node);
  TF_LITE_ENSURE_MSG(context, op_context.dims <= kMaxDim, "input dim should not exceed 4");
