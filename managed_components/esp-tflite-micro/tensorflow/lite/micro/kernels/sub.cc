...
void EvalSub(TfLiteContext *context, TfLiteNode *node, TfLiteSubParams *params, const OpDataSub *data,
...
TfLiteStatus EvalSubQuantized(TfLiteContext *context, TfLiteNode *node, TfLiteSubParams *params, const OpDataSub *data,
  ...
  switch (output->type) {
    ...
    default:
      MicroPrintf("Quantized type %s not currently supported.", TfLiteTypeGetName(output->type));
      return kTfLiteError;
  }
  return kTfLiteOk;
}
...
TfLiteStatus SubEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  const TfLiteEvalTensor *input1 = tflite::micro::GetEvalInput(context, node, kSubInputTensor1);
  const TfLiteEvalTensor *input2 = tflite::micro::GetEvalInput(context, node, kSubInputTensor2);
  TfLiteEvalTensor *output = tflite::micro::GetEvalOutput(context, node, kSubOutputTensor);
  ...
  if (output->type == kTfLiteFloat32) {
    ...
    TF_LITE_ENSURE_OK(context, EvalSubQuantized(context, node, params, &data, input1, input2, output));
  } else {
...
TFLMRegistration Register_SUB() { return tflite::micro::RegisterOp(SubInit, SubPrepare, SubEval); }
