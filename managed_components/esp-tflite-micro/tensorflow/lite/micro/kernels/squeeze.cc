...
TfLiteStatus SqueezePrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (num_squeeze_dims == 0) {
    ...
    for (int idx = 0; idx < num_squeeze_dims; ++idx) {
      ...
      TF_LITE_ENSURE(context, current >= 0 && current < input_num_dims && input_dims->data[current] == 1);
...
TfLiteStatus SqueezeEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  TF_LITE_ENSURE_OK(context, TfLiteEvalTensorByteLength(input, &input_byte_size));
  TF_LITE_ENSURE_OK(context, TfLiteEvalTensorByteLength(output, &output_byte_size));
...
TFLMRegistration Register_SQUEEZE() { return tflite::micro::RegisterOp(nullptr, SqueezePrepare, SqueezeEval); }
