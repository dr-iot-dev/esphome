...
TfLiteStatus ReshapeOutputTensor(TfLiteContext *context, TfLiteNode *node, const RuntimeShape &extended_lhs_shape,
                                 const RuntimeShape &extended_rhs_shape, bool adj_x, bool adj_y, int output_rank,
                                 TfLiteTensor *output) {
...
TfLiteEvalTensor *AllocInitTransposeTensorFromTfLiteTensor(TfLiteContext *context, const TfLiteTensor &tensor) {
  MicroContext *micro_context = GetMicroContext(context);
  TfLiteEvalTensor *eval_tensor =
      static_cast<TfLiteEvalTensor *>(micro_context->AllocatePersistentBuffer(sizeof(TfLiteEvalTensor)));
  ...
  eval_tensor->dims = static_cast<TfLiteIntArray *>(micro_context->AllocatePersistentBuffer(eval_dims_size));
  ...
  const size_t eval_data_size = static_cast<size_t>(NumElements(&tensor)) * TfLiteTypeGetSize(tensor.type);
  eval_tensor->data.data = micro_context->AllocatePersistentBuffer(eval_data_size);
...
TfLiteStatus InitializeTemporaries(TfLiteContext *context, TfLiteNode *node, const PrepareOpContext &op_context) {
  ...
  if (lhs->type == kTfLiteInt8 || lhs->type == kTfLiteInt16) {
    op_data->quantization = static_cast<decltype(op_data->quantization)>(
        micro_context->AllocatePersistentBuffer(sizeof(*op_data->quantization)));
    TF_LITE_ENSURE(context, op_data->quantization != nullptr);
  }

  // tensor for Transposed LHS;
  if (op_context.params->adj_x) {
    op_data->lhs_transposed_tensor = AllocInitTransposeTensorFromTfLiteTensor(context, *lhs);
  ...
  if (!op_context.params->adj_y) {
    op_data->rhs_transposed_tensor = AllocInitTransposeTensorFromTfLiteTensor(context, *rhs);
...
void TransposeRowsColumnsImpl(const TfLiteEvalTensor &tensor_in, TfLiteEvalTensor *tensor_out) {
...
TfLiteStatus TransposeRowsColumns(const TfLiteEvalTensor &tensor_in, TfLiteEvalTensor *tensor_out) {
  if (tensor_in.type == kTfLiteFloat32) {
    ...
    MicroPrintf("BATCH_MATMUL can only transpose tensors with FLOAT32, INT8, INT16 "
                "type.");
  }
  return kTfLiteError;
}
...
TfLiteStatus BatchMatMulPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (lhs_data->type == kTfLiteInt8 || lhs_data->type == kTfLiteInt16) {
    ...
    TF_LITE_ENSURE_STATUS(GetQuantizedConvolutionMultipler(context, lhs_data, rhs_data, output, &real_multiplier));
    QuantizeMultiplier(real_multiplier, &op_data->quantization->output_multiplier,
    ...
    if (lhs_data->type == kTfLiteInt8) {
      op_data->quantization->output_activation_min = std::numeric_limits<int8_t>::min();
      op_data->quantization->output_activation_max = std::numeric_limits<int8_t>::max();
    } else {
      op_data->quantization->output_activation_min = std::numeric_limits<int16_t>::min();
      op_data->quantization->output_activation_max = std::numeric_limits<int16_t>::max();
  ...
  const RuntimeShape extended_lhs_shape = RuntimeShape::ExtendedShape(output_rank, GetTensorShape(lhs_data));
  const RuntimeShape extended_rhs_shape = RuntimeShape::ExtendedShape(output_rank, GetTensorShape(rhs_data));
  ...
  int accum_dim_lhs = adj_x ? extended_lhs_shape.Dims(output_rank - 2) : extended_lhs_shape.Dims(output_rank - 1);
  int accum_dim_rhs = adj_y ? extended_rhs_shape.Dims(output_rank - 1) : extended_rhs_shape.Dims(output_rank - 2);

  TF_LITE_ENSURE_EQ(context, accum_dim_lhs, accum_dim_rhs);
  TfLiteStatus status =
      ReshapeOutputTensor(context, node, extended_lhs_shape, extended_rhs_shape, adj_x, adj_y, output_rank, output);
  return status;
}

TfLiteStatus EvalInt8(TfLiteContext *context, const OpData &data, const RuntimeShape &lhs_shape,
                      const TfLiteEvalTensor &lhs, const RuntimeShape &rhs_shape, const TfLiteEvalTensor &rhs,
                      const RuntimeShape &output_shape, TfLiteEvalTensor *output) {
  ...
  op_params.weights_offset = -data.quantization->rhs_zero_point;  // filter offset
  ...
  reference_ops::BatchMatMul<int8_t, int32_t>(op_params, rhs_shape, tflite::micro::GetTensorData<int8_t>(&rhs),
...
TfLiteStatus EvalInt16(TfLiteContext *context, const OpData &data, const RuntimeShape &lhs_shape,
                       const TfLiteEvalTensor &lhs, const RuntimeShape &rhs_shape, const TfLiteEvalTensor &rhs,
                       const RuntimeShape &output_shape, TfLiteEvalTensor *output) {
  TF_LITE_ENSURE(context, data.quantization != nullptr);
  // Reuse params struct from FullyConnected Op.
  ...
  // Note we pass RHS args first, LHS args second. See note for Eval.
  reference_ops::BatchMatMul<int16_t, int64_t>(op_params, rhs_shape, tflite::micro::GetTensorData<int16_t>(&rhs),
...
TfLiteStatus BatchMatMulEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (rhs_dims_count > 2 && lhs_dims_count > 2) {
    ...
    if (rhs_one == 1) {
      ...
      tmp_l.SetDim(lhs_dims_count - 3, lhs_dims[lhs_dims_count - 3] * lhs_dims[lhs_dims_count - 2]);
  ...
  TfLiteEvalTensor *rhs_tensor = adj_y ? const_cast<TfLiteEvalTensor *>(rhs) : op_data->rhs_transposed_tensor;
  TfLiteEvalTensor *lhs_tensor = adj_x ? op_data->lhs_transposed_tensor : const_cast<TfLiteEvalTensor *>(lhs);
  ...
  RuntimeShape rhs_shape = adj_y ? orig_rhs_shape : SwapRowColumnDims(orig_rhs_shape);
  RuntimeShape lhs_shape = adj_x ? orig_lhs_shape : SwapRowColumnDims(orig_lhs_shape);

  switch (lhs->type) {
    case kTfLiteFloat32:
      // Note we pass RHS args first, LHS args second. See note above.
      reference_ops::BatchMatMul(rhs_shape, tflite::micro::GetTensorData<float>(rhs_tensor), lhs_shape,
                                 tflite::micro::GetTensorData<float>(lhs_tensor), tflite::micro::GetTensorShape(output),
                                 tflite::micro::GetTensorData<float>(output));
      break;
    case kTfLiteInt8:
      return EvalInt8(context, *op_data, lhs_shape, *lhs_tensor, rhs_shape, *rhs_tensor,
                      tflite::micro::GetTensorShape(output), output);
    case kTfLiteInt16:
      return EvalInt16(context, *op_data, lhs_shape, *lhs_tensor, rhs_shape, *rhs_tensor,
                       tflite::micro::GetTensorShape(output), output);
    default:
      MicroPrintf("BATCH_MATMUL doesn't support input type %s", TfLiteTypeGetName(lhs->type));
      return kTfLiteError;
  }
  return kTfLiteOk;
}
