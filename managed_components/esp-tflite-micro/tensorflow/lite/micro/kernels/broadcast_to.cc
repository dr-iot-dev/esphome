...
TfLiteStatus ValidateOutputTensor(TfLiteContext *context, TfLiteTensor *input, TfLiteTensor *shape,
                                  TfLiteTensor *output) {
  ...
  TF_LITE_ENSURE_MSG(context, output_num_dims <= kMaxDims, "BroadcastTo only supports 1-5D tensor.");
  ...
  for (int idx = 0; idx < input_num_dims; ++idx) {
    TF_LITE_ENSURE_MSG(
        context,
        (SizeOfDimension(input, idx) == 1 || SizeOfDimension(input, idx) == get_shape_data(extending_dims + idx)),
        "Output shape must be broadcastable from input shape.");
  }
...
TfLiteStatus BroadcastToPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteTensor *shape = micro_context->AllocateTempInputTensor(node, kShapeTensor);
  TfLiteTensor *output = micro_context->AllocateTempOutputTensor(node, kOutputTensor);

  TF_LITE_ENSURE_MSG(context, (NumDimensions(input) <= kMaxDims), "BroadcastTo only supports 1-5D tensor.");

  TF_LITE_ENSURE(context, shape->type == kTfLiteInt32 || shape->type == kTfLiteInt64);
...
TfLiteStatus BroadcastToEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  reference_ops::BroadcastTo<kMaxDims>(micro::GetTensorShape(input), input->data.raw, micro::GetTensorShape(output),
                                       output->data.raw, input->type);
  return kTfLiteOk;
}
}  // namespace

TFLMRegistration Register_BROADCAST_TO() {
  return tflite::micro::RegisterOp(nullptr, BroadcastToPrepare, BroadcastToEval);
}
