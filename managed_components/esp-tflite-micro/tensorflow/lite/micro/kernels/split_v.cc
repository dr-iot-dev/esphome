...
template<typename T>
TfLiteStatus SplitImpl(TfLiteContext *context, TfLiteNode *node, const TfLiteEvalTensor *input, int axis_value) {
  ...
  for (int k = 0; k < outer_size; ++k) {
    for (int i = 0; i < output_count; ++i) {
      TfLiteEvalTensor *output_tensor = tflite::micro::GetEvalOutput(context, node, i);
      T *output_data = tflite::micro::GetTensorData<T>(output_tensor);
      const int copy_size = output_tensor->dims->data[axis_value] * base_inner_size;
...
TfLiteStatus SplitVPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TF_LITE_ENSURE_MSG(context, IsConstantTensor(axis), "Non constant axis tensor not supported");
...
TFLMRegistration Register_SPLIT_V() { return tflite::micro::RegisterOp(nullptr, SplitVPrepare, SplitVEval); }
