...
TfLiteStatus ValidateWeightTensorSize(TfLiteContext *context, const TfLiteTensor *tensor, int dim1_size,
                                      int dim2_size) {
...
TfLiteStatus ValidateTensorSize(TfLiteContext *context, const LstmTensors &tensors, const LstmSizeInfo &size_info) {
  ...
  for (size_t i = 1; i < 5; i++) {
    TF_LITE_ENSURE_OK(context, ValidateWeightTensorSize(context, tensors.GetInternalTensor(i),
                                                        size_info.state_dimension, size_info.input_dimension));
  }
  // Recurrent FC weights
  for (size_t i = 5; i < 9; i++) {
    TF_LITE_ENSURE_OK(context, ValidateWeightTensorSize(context, tensors.GetInternalTensor(i),
                                                        size_info.state_dimension, size_info.state_dimension));
  }
  // Biases
  for (size_t i = 12; i < 16; i++) {
    TF_LITE_ENSURE_OK(context,
                      ValidateBiasTensorSize(context, tensors.GetInternalTensor(i), size_info.state_dimension));
  }
  ...
  TF_LITE_ENSURE_EQ(context, NumElements(tensors.CellStateTensor()), size_info.batch_size * size_info.state_dimension);

  // Check the shape of output tensor against that of input tensor
  TF_LITE_ENSURE_EQ(context, tensors.OutputTensor()->dims->size, 3);
  TF_LITE_ENSURE_EQ(context, tensors.GetInternalTensor(kLstmInputTensor)->dims->data[0],
                    tensors.OutputTensor()->dims->data[0]);
  TF_LITE_ENSURE_EQ(context, tensors.GetInternalTensor(kLstmInputTensor)->dims->data[1],
                    tensors.OutputTensor()->dims->data[1]);
  TF_LITE_ENSURE_EQ(context, tensors.OutputTensor()->dims->data[2], size_info.state_dimension);
  return kTfLiteOk;
}
...
TfLiteStatus CreateGateParams(TfLiteContext *context,
  ...
  // tensor scales
  TF_LITE_ENSURE_STATUS(CalculateOpDataFullyConnected(context, kTfLiteActNone, input->type, input, input_weight,
  ...
  GetQuantizedConvolutionMultipler(context, input, input_weight, nullptr, &fc_output_temp, &real_multiplier);

  TF_LITE_ENSURE_STATUS(CalculateOpDataFullyConnected(context, kTfLiteActNone, hidden_state->type, hidden_state,
...
TfLiteStatus PrepareGateParametersFloat(TfLiteContext *context, const LstmTensors &lstm_tensors,
  ...
  op_data_lstm->inter_gate_parameters.forget_cell_mul_params = CreateInterGateMulParamsFloat();
  op_data_lstm->inter_gate_parameters.input_mul_params = CreateInterGateMulParamsFloat();
  op_data_lstm->inter_gate_parameters.output_mul_params = CreateInterGateMulParamsFloat();
  return kTfLiteOk;
}
...
TfLiteStatus PrepareGateParametersInteger(TfLiteContext *context, const LstmTensors &lstm_tensors,
  ...
  TF_LITE_ENSURE_OK(context, CreateGateParams(context, lstm_tensors.GetInternalTensor(kLstmInputTensor),
  ...
  op_data_lstm->inter_gate_parameters.forget_cell_mul_params =
      CreateInterGateMulParams(nonlinear_output_scale, cell_state_scale, cell_state_scale, kTfLiteInt16);
  // input gate output x cell gate output -> cell state
  op_data_lstm->inter_gate_parameters.input_mul_params =
      CreateInterGateMulParams(nonlinear_output_scale, nonlinear_output_scale, cell_state_scale, kTfLiteInt16);
  // tanh output x output gate output -> hidden state (potentially asymmetric)
  op_data_lstm->inter_gate_parameters.output_mul_params = CreateInterGateMulParams(
      nonlinear_output_scale, nonlinear_output_scale, lstm_tensors.HiddenStateTensor()->params.scale,
      lstm_tensors.HiddenStateTensor()->type, lstm_tensors.HiddenStateTensor()->params.zero_point);
  return kTfLiteOk;
}
...
LSTMKernelContents CreateLSTMKernelContent(TfLiteContext *context, TfLiteNode *node) {
  ...
  for (size_t i = 0; i < 24; i++) {
    kernel_content.internal_tensors[i] = tflite::micro::GetMutableEvalInput(context, node, i);
  }
  // Output tensor
