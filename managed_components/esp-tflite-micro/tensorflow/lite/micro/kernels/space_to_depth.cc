...
TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteEvalTensor *output_eval = micro::GetEvalOutput(context, node, kOutputTensor);
  TF_LITE_ENSURE_OK(context, micro::CreateWritableTensorDimsWithCopy(context, output, output_eval));
  ...
  output->dims->data[kDepthRank] = input->dims->data[kDepthRank] * block_size * block_size;
...
TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) {
  ...
  switch (input->type) {  // Already know in/out types are same.
    case kTfLiteFloat32:
      reference_ops::SpaceToDepth(op_params, micro::GetTensorShape(input), micro::GetTensorData<float>(input),
                                  micro::GetTensorShape(output), micro::GetTensorData<float>(output));
      break;
    case kTfLiteInt8:
      reference_ops::SpaceToDepth(op_params, micro::GetTensorShape(input), micro::GetTensorData<int8_t>(input),
                                  micro::GetTensorShape(output), micro::GetTensorData<int8_t>(output));
      break;
    default:
      MicroPrintf("SPACE_TO_DEPTH only supports FLOAT32 and INT8, got %s.", TfLiteTypeGetName(input->type));
      return kTfLiteError;
  }
...
TFLMRegistration Register_SPACE_TO_DEPTH() { return tflite::micro::RegisterOp(nullptr, Prepare, Eval); }
