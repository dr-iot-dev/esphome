...
TfLiteStatus CircularBufferPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteTensor *input = micro_context->AllocateTempInputTensor(node, kCircularBufferInputTensor);
  TfLiteTensor *output = micro_context->AllocateTempOutputTensor(node, kCircularBufferOutputTensor);
  ...
  if (op_data->cycles_max <= 0) {
    ...
    if (output->dims->data[1] == 5 || output->dims->data[1] == 13 || output->dims->data[1] == 25 ||
        (cb_prepare_count == 5 && output->dims->data[2] == 2 && output->dims->data[3] == 96)) {
