...
void *SpaceToBatchNDInit(TfLiteContext *context, const char *buffer, size_t length) {
...
TfLiteStatus SpaceToBatchNDEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  const SpaceToBatchParams &params = *(static_cast<const SpaceToBatchParams *>(node->user_data));
  ...
  switch (input->type) {  // Already know in/out types are same.
    case kTfLiteFloat32:
      reference_ops::SpaceToBatchND(
          params, tflite::micro::GetTensorShape(input), tflite::micro::GetTensorData<float>(input),
...
TFLMRegistration Register_SPACE_TO_BATCH_ND() {
  return tflite::micro::RegisterOp(SpaceToBatchNDInit, SpaceToBatchNDPrepare, SpaceToBatchNDEval);
}
