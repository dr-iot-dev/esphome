...
TfLiteStatus CalculateOpData(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (output->type == kTfLiteInt8) {
    ...
    const double twice_max_input_scale = 2 * static_cast<double>(input->params.scale);
    const double real_input_multiplier = static_cast<double>(input->params.scale) / twice_max_input_scale;
...
TfLiteStatus CumSumPrepare(TfLiteContext *context, TfLiteNode *node) { return CalculateOpData(context, node); }
...
TfLiteStatus CumSumEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  const TfLiteEvalTensor *axis_tensor = tflite::micro::GetEvalInput(context, node, kAxisTensor);
  ...
  switch (input->type) {
    case kTfLiteFloat32: {
      reference_ops::CumSum(tflite::micro::GetTensorData<float>(input), input_shape, axis, cs_params->exclusive,
                            cs_params->reverse, tflite::micro::GetTensorData<float>(output));
      return kTfLiteOk;
    } break;
    ...
    case kTfLiteInt8: {
      ...
      reference_ops::CumSum(params, tflite::micro::GetTensorData<int8_t>(input), input_shape, axis,
                            cs_params->exclusive, cs_params->reverse, tflite::micro::GetTensorData<int8_t>(output));
      return kTfLiteOk;
    } break;

    default: {
      MicroPrintf("CUMSUM only supports FLOAT32 and INT8, got %s.", TfLiteTypeGetName(output->type));
      return kTfLiteError;
    }
  }
...
TFLMRegistration Register_CUMSUM() { return tflite::micro::RegisterOp(nullptr, CumSumPrepare, CumSumEval); }
