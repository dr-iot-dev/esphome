...
TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteTensor *input_resource_id_tensor = micro_context->AllocateTempInputTensor(node, kInputVariableId);
...
TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteEvalTensor *output_value = tflite::micro::GetEvalOutput(context, node, kOutputValue);
  ...
  if (resources == nullptr) {
    MicroPrintf("READ_VARIABLE requires resource variables. Please create "
                "ResourceVariables and pass it to the interpreter.");
    return kTfLiteError;
  }
  TF_LITE_ENSURE_OK(context, resources->Read(input_resource_id_tensor->data.i32[0], output_value));
  return kTfLiteOk;
}

}  // namespace.

TFLMRegistration Register_READ_VARIABLE() { return tflite::micro::RegisterOp(nullptr, Prepare, Eval); }
