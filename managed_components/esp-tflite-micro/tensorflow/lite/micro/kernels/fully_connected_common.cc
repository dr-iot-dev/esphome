...
FullyConnectedParams FullyConnectedParamsFloat(TfLiteFusedActivation activation) {
  FullyConnectedParams op_params;
  CalculateActivationRange(activation, &op_params.float_activation_min, &op_params.float_activation_max);
  return op_params;
}

TfLiteStatus CalculateOpDataFullyConnected(TfLiteContext *context, TfLiteFusedActivation activation,
  ...
  if (filter->quantization.type == kTfLiteAffineQuantization && filter->quantization.params != nullptr) {
    TfLiteAffineQuantization *affine_quantization =
        reinterpret_cast<TfLiteAffineQuantization *>(filter->quantization.params);
    TF_LITE_ENSURE(context, affine_quantization->scale);
    TF_LITE_ENSURE_MSG(context, affine_quantization->scale->size == 1,
  ...
  if (data_type != kTfLiteFloat32) {
    ...
    return CalculateActivationRangeQuantized(context, activation, output, &data->output_activation_min,
