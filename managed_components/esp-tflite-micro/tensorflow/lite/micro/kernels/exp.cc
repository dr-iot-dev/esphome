...
TfLiteStatus ExpEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (input->type == kTfLiteFloat32) {
    reference_ops::Exp(tflite::micro::GetTensorData<float>(input), static_cast<size_t>(flat_size),
                       tflite::micro::GetTensorData<float>(output));
  } else {
    MicroPrintf("Type %s (%d) currently not supported by Exp.", TfLiteTypeGetName(input->type), input->type);
    return kTfLiteError;
  }
  return kTfLiteOk;
}
...
TFLMRegistration Register_EXP() { return tflite::micro::RegisterOp(nullptr, ExpPrepare, ExpEval); }
