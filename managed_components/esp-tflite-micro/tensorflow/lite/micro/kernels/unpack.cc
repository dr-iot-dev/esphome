...
template<typename T>
TfLiteStatus UnpackImpl(TfLiteContext *context, TfLiteNode *node, const TfLiteEvalTensor *input, int output_count,
                        int axis) {
...
TfLiteStatus UnpackEval(TfLiteContext *context, TfLiteNode *node) {
  TfLiteUnpackParams *data = reinterpret_cast<TfLiteUnpackParams *>(node->builtin_data);
  ...
  switch (input->type) {
    ...
    default: {
      MicroPrintf("Type '%s' is not supported by unpack.", TfLiteTypeGetName(input->type));
      return kTfLiteError;
    }
  }
...
TFLMRegistration Register_UNPACK() { return tflite::micro::RegisterOp(nullptr, nullptr, UnpackEval); }
