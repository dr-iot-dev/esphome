...
TfLiteStatus CallOncePrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  const auto *params = reinterpret_cast<const TfLiteCallOnceParams *>(node->builtin_data);
  ...
  TF_LITE_ENSURE(context, op_data->init_subgraph_index < graph_info.NumSubgraphs());
...
TfLiteStatus CallOnceEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  TF_LITE_ENSURE_OK(context, graph_info.InvokeSubgraph(op_data->init_subgraph_index));
...
TFLMRegistration Register_CALL_ONCE() { return tflite::micro::RegisterOp(CallOnceInit, CallOncePrepare, CallOnceEval); }
