...
void *CircularBufferInit(TfLiteContext *context, const char *buffer, size_t length) {
  ...
  OpDataCircularBuffer *op_data =
      static_cast<OpDataCircularBuffer *>(context->AllocatePersistentBuffer(context, sizeof(OpDataCircularBuffer)));
...
TfLiteStatus CircularBufferEval(TfLiteContext *context, TfLiteNode *node) {
  const TfLiteEvalTensor *input = tflite::micro::GetEvalInput(context, node, kCircularBufferInputTensor);
  TfLiteEvalTensor *output = tflite::micro::GetEvalOutput(context, node, kCircularBufferOutputTensor);
