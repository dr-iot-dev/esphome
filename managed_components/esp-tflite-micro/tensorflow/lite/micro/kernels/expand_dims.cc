...
TfLiteStatus GetAxisValueFromTensor(TfLiteContext *context, const TfLiteTensor *axis, int32_t *axis_value) {
  ...
  if (kTfLiteInt32 == (axis->type)) {
    ...
    *axis_value = axis_ptr[0];
    ...
    MicroPrintf("Axis type %s (%d) not supported by Expand_Dims.", TfLiteTypeGetName(axis->type), axis->type);
    return kTfLiteError;
  }
}
...
TfLiteStatus VerifyTensorDim(TfLiteContext *context, const TfLiteTensor *input, const TfLiteTensor *axis_tensor,
                             const TfLiteTensor *output) {
  int32_t axis_value = 0;
  TF_LITE_ENSURE_OK(context, GetAxisValueFromTensor(context, axis_tensor, &axis_value));
  ...
  TF_LITE_ENSURE(context, output_shape.DimensionsCount() == input_shape.DimensionsCount() + 1);
...
template<typename T> void memCopyN(T *out, const T *in, const int num_elements) {
...
TfLiteStatus ExpandDimsEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  switch (input->type) {
    ...
    case kTfLiteInt16: {
      memCopyN(tflite::micro::GetTensorData<int16_t>(output), tflite::micro::GetTensorData<int16_t>(input), flat_size);
    } break;
    case kTfLiteInt8: {
      memCopyN(tflite::micro::GetTensorData<int8_t>(output), tflite::micro::GetTensorData<int8_t>(input), flat_size);
    } break;
    default:
      MicroPrintf("Expand_Dims only currently supports int8, int16 and float32, got "
                  "%d.",
                  input->type);
      return kTfLiteError;
  }
