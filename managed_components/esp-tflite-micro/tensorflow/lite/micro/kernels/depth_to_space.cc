...
TfLiteStatus DepthToSpacePrepare(TfLiteContext *context, TfLiteNode *node) { return CalculateOpData(context, node); }
...
TfLiteStatus DepthToSpaceEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  switch (input->type) {  // Already know in/out types are same.
    case kTfLiteFloat32:
      reference_ops::DepthToSpace(op_params, tflite::micro::GetTensorShape(input),
    ...
    case kTfLiteInt8:
      ...
      break;
    default:
      MicroPrintf("DEPTH_TO_SPACE only supports FLOAT32 and INT8, got %s.", TfLiteTypeGetName(output->type));
      return kTfLiteError;
  }
