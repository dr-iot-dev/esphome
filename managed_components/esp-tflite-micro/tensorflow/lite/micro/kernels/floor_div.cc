...
void *FloorDivInit(TfLiteContext *context, const char *buffer, size_t length) { return nullptr; }

TfLiteStatus FloorDivPrepare(TfLiteContext *context, TfLiteNode *node) { return CalculateOpData(context, node); }

template<typename T>
TfLiteStatus EvalFloorDiv(TfLiteContext *context, const TfLiteEvalTensor *input1, const TfLiteEvalTensor *input2,
...
TfLiteStatus FloorDivEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  switch (input1->type) {
    ...
    default: {
      MicroPrintf("Type '%s' is not supported by FLOOR_DIV.", TfLiteTypeGetName(input1->type));
      return kTfLiteError;
    }
  }
}
...
TFLMRegistration Register_FLOOR_DIV() { return tflite::micro::RegisterOp(FloorDivInit, FloorDivPrepare, FloorDivEval); }
