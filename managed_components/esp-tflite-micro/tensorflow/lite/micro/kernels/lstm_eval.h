...
class LstmTensors {
 public:
  ...
  const TfLiteTensor *GetInternalTensor(const int tensor_index) const { return internal_tensors_[tensor_index]; }

  const TfLiteTensor *HiddenStateTensor() const { return internal_tensors_[kLstmOutputStateTensor]; }
...
TfLiteStatus PrepareGateParametersFloat(TfLiteContext *context, const LstmTensors &lstm_tensors,
                                        OpDataLSTM *op_data_lstm);

TfLiteStatus PrepareGateParametersInteger(TfLiteContext *context, const LstmTensors &lstm_tensors,
                                          OpDataLSTM *op_data_lstm);

LSTMKernelContents CreateLSTMKernelContent(TfLiteContext *context, TfLiteNode *node);

template<typename CellType> LSTMBuffers<CellType> CreateLSTMBuffers(TfLiteContext *context, const int *buffer_indices) {
  LSTMBuffers<CellType> buffers;
  buffers.buffer0 = reinterpret_cast<CellType *>(context->GetScratchBuffer(context, buffer_indices[0]));
  buffers.buffer1 = reinterpret_cast<CellType *>(context->GetScratchBuffer(context, buffer_indices[1]));
  buffers.buffer2 = reinterpret_cast<CellType *>(context->GetScratchBuffer(context, buffer_indices[2]));
  buffers.buffer3 = reinterpret_cast<CellType *>(context->GetScratchBuffer(context, buffer_indices[3]));
  return buffers;
}
...
void Tanh(int32_t cell_state_scale_power, const RuntimeShape &input_data_shape, int16_t *input_data,
          const RuntimeShape &output_data_shape, int16_t *output_data);

void Tanh(int32_t cell_state_scale_power, const RuntimeShape &input_data_shape, float *input_data,
          const RuntimeShape &output_data_shape, float *output_data);

void Mul(const RuntimeShape &shape, const ArithmeticParams &params, const int16_t *input1_data,
         const int16_t *input2_data, int8_t *output_data);

void Mul(const RuntimeShape &shape, const ArithmeticParams &params, const int16_t *input1_data,
         const int16_t *input2_data, int16_t *output_data);

void Mul(const RuntimeShape &shape, const ArithmeticParams &params, const float *input1_data, const float *input2_data,
         float *output_data);

void FullyConnected(const FullyConnectedParams &params, const RuntimeShape &input_shape, const int8_t *input_data,
...
void FullyConnected(const FullyConnectedParams &params, const RuntimeShape &input_shape, const int16_t *input_data,
                    const RuntimeShape &filter_shape, const int8_t *filter_data, const RuntimeShape &bias_shape,
                    const int64_t *bias_data, const RuntimeShape &output_shape, int16_t *output_data);

void FullyConnected(const FullyConnectedParams &params, const RuntimeShape &input_shape, const float *input_data,
...
void CalculateLstmGate(const LstmStepManager &step_info, const GateParameters &gate_params,
                       // Input FC
  ...
  TFLITE_DCHECK_LE(step_info.HiddenStateOffset() + step_info.StateShape().FlatSize(),
  ...
  FullyConnected(gate_params.input_fc_params, step_info.InputShape(),
                 ...
                 tflite::micro::GetTensorShape(input_bias), tflite::micro::GetOptionalTensorData<BiasType>(input_bias),
  ...
  FullyConnected(gate_params.recurrent_fc_params, step_info.StateShape(),
                 ...
                 tflite::micro::GetOptionalTensorData<BiasType>(recurrent_bias), gate_output_shape, fc_output_buffer);
  ...
  switch (activation) {
    ...
    case kTfLiteActTanh: {
      ...
      Tanh(/*cell_state_scale_power=*/-12, gate_output_shape, gate_output, gate_output_shape, gate_output);
    } break;
    default:
...
void UpdateLstmCell(const LstmStepManager &step_info, TfLiteEvalTensor *cell_state,
                    // Gate outputs
                    CellType *forget_gate_output, const CellType *input_gate_output, const CellType *cell_gate_output,
                    // Mul parameters
  ...
  // Check offset validity to avoid memory overflow
  TFLITE_DCHECK_LE(step_info.CellStateOffset() + step_info.StateShape().FlatSize(),
  ...
  AddElementWise(tflite::micro::GetTensorData<CellType>(cell_state) + step_info.CellStateOffset(), buffer,
...
void UpdateLstmHidden(const LstmStepManager &step_info, TfLiteEvalTensor *cell_state, TfLiteEvalTensor *hidden_state,
  ...
  auto cell_state_shape = step_info.StateShape();
  CellType *cell_state_data = tflite::micro::GetTensorData<CellType>(cell_state) + step_info.CellStateOffset();
  // Tanh(cell_state)
...
template<typename ActivationType, typename WeightType, typename CellType, typename BiasType>
void LstmStep(const LstmStepManager &step_info, const OpDataLSTM &op_data, LSTMKernelContents &kernel_content,
  ...
  CalculateLstmGate<ActivationType, WeightType, CellType, BiasType>(
      ...
      // Recurrent FC
      kernel_content.HiddenStateTensor(), kernel_content.GetInternalTensor(tflite::kLstmRecurrentToForgetWeightsTensor),
  ...
  CalculateLstmGate<ActivationType, WeightType, CellType, BiasType>(
      ...
      // Recurrent FC
      kernel_content.HiddenStateTensor(), kernel_content.GetInternalTensor(tflite::kLstmRecurrentToInputWeightsTensor),
      /*recurrent_bias*/ nullptr,
      // Output
      input_gate_output,
      // Scratch arrays
  ...
  CalculateLstmGate<ActivationType, WeightType, CellType, BiasType>(
      ...
      // Recurrent FC
      kernel_content.HiddenStateTensor(), kernel_content.GetInternalTensor(tflite::kLstmRecurrentToCellWeightsTensor),
      /*recurrent_bias*/ nullptr,
      // Output
      cell_gate_output,
      // Scratch arrays
  ...
  UpdateLstmCell<CellType>(step_info, kernel_content.CellStateTensor(), forget_gate_output, input_gate_output,
  ...
  CalculateLstmGate<ActivationType, WeightType, CellType, BiasType>(
      ...
      // Recurrent FC
      kernel_content.HiddenStateTensor(), kernel_content.GetInternalTensor(tflite::kLstmRecurrentToOutputWeightsTensor),
      /*recurrent_bias*/ nullptr,
      // Output
      output_gate_output,
      // Scratch arrays
  ...
  tflite::lstm_internal::UpdateLstmHidden<CellType, ActivationType>(
      step_info, kernel_content.CellStateTensor(), kernel_content.HiddenStateTensor(), output_gate_output,
      inter_gate_params.output_mul_params, op_data.cell_state_info.cell_state_scale_power, tanh_activated_cell_buffer);

  /*Step4: copy the update the hidden state to output*/
  // Check offset validity to avoid memory overflow
  TFLITE_DCHECK_LE(step_info.OutputOffset() + step_info.StateShape().FlatSize(),
                   tflite::micro::GetTensorShape(kernel_content.output_tensor).FlatSize());
  // record the output (from the updated hidden state)
  ActivationType *output_ptr = tflite::micro::GetTensorData<ActivationType>(kernel_content.output_tensor);
...
// Since
template<typename ActivationType, typename WeightType, typename CellType, typename BiasType>
TfLiteStatus EvalLstm(const OpDataLSTM &op_data, LSTMKernelContents &kernel_content,
                      const LSTMBuffers<CellType> &buffers) {
  ...
  if (size_info.time_major) {
    for (int t = 0; t < size_info.time_steps; t++) {
      lstm_internal::LstmStep<ActivationType, WeightType, CellType, BiasType>(step_info, op_data, kernel_content,
                                                                              buffers);
