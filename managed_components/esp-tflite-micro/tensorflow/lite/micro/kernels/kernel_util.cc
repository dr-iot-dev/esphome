...
int ValidateTensorIndexing(const TfLiteContext *context, int index, int max_size, const int *tensor_indices) {
...
TFLMRegistration RegisterOp(void *(*init)(TfLiteContext *context, const char *buffer, size_t length),
...
TFLMInferenceRegistration RegisterOp(TfLiteStatus (*invoke)(TfLiteContext *context, TfLiteNode *node),
                                     void (*reset)(TfLiteContext *context, void *buffer)) {
  return {
      /*invoke=*/invoke,
      /*reset*/ reset,
...
TfLiteEvalTensor *GetMutableEvalInput(const TfLiteContext *context, const TfLiteNode *node, int index) {
...
const TfLiteEvalTensor *GetEvalInput(const TfLiteContext *context, const TfLiteNode *node, int index) {
...
TfLiteStatus CreateWritableTensorDimsWithCopy(TfLiteContext *context, TfLiteTensor *tensor,
  ...
  TfLiteIntArray *new_dims = static_cast<TfLiteIntArray *>(context->AllocatePersistentBuffer(context, alloc_size));
...
size_t ValidateAndGetTensorSizes(const TfLiteEvalTensor *tensor1, const TfLiteEvalTensor *tensor2) {
...
// TfLiteEvalTensor*
void PrintNBytes(const TfLiteEvalTensor *tensor, int n_bytes, const char *prefix) {
...
TfLiteStatus CopyOpInputsToSubgraphInputs(TfLiteContext *context, TfLiteNode *node, MicroGraph *graph_info,
                                          int subgraph_idx, int first_tensor_idx) {
  TF_LITE_ENSURE(context, static_cast<size_t>(node->inputs->size - first_tensor_idx) ==
                              graph_info->NumSubgraphInputs(subgraph_idx));
  for (int i = 0; i < node->inputs->size - first_tensor_idx; i++) {
    const TfLiteEvalTensor *input = tflite::micro::GetEvalInput(context, node, i + first_tensor_idx);
    TfLiteEvalTensor *subgraph_input = graph_info->GetSubgraphInput(subgraph_idx, i);
...
TfLiteStatus CopyOpOutputsToSubgraphInputs(TfLiteContext *context, TfLiteNode *node, MicroGraph *graph_info,
                                           int subgraph_idx) {
  TF_LITE_ENSURE(context, static_cast<size_t>(node->outputs->size) == graph_info->NumSubgraphInputs(subgraph_idx));
...
TfLiteStatus CopySubgraphOutputsToOpOutputs(TfLiteContext *context, TfLiteNode *node, MicroGraph *graph_info,
                                            int subgraph_idx) {
  if (graph_info->NumSubgraphOutputs(subgraph_idx) == 0)
    return kTfLiteOk;
  TF_LITE_ENSURE(context, static_cast<size_t>(node->outputs->size) == graph_info->NumSubgraphOutputs(subgraph_idx));
  for (int i = 0; i < node->outputs->size; i++) {
    TfLiteEvalTensor *output = tflite::micro::GetEvalOutput(context, node, i);
    TfLiteEvalTensor *subgraph_output = graph_info->GetSubgraphOutput(subgraph_idx, i);
...
TfLiteEvalTensor MakeUnpackedInt4Tensor(TfLiteContext *context, int scratch_buffer_index,
  ...
  new_tensor.data.data = static_cast<int8_t *>(context->GetScratchBuffer(context, scratch_buffer_index));
  new_tensor.dims = tensor->dims;
  new_tensor.type = kTfLiteInt8;
  tflite::tensor_utils::UnpackDenseInt4IntoInt8(tflite::micro::GetTensorData<int8_t>(tensor),
