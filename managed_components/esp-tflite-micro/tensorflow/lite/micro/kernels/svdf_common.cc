...
const int kSvdfInputActivationStateTensor = 4;  // This is a variable tensor, and will be modified by this op.
const int kSvdfOutputTensor = 0;

template<typename T>
void EvalIntegerSvdfReference(TfLiteContext *context, TfLiteNode *node, const TfLiteEvalTensor *input_tensor,
                              const TfLiteEvalTensor *weights_feature_tensor,
                              const TfLiteEvalTensor *weights_time_tensor, const TfLiteEvalTensor *bias_tensor,
                              const TfLiteSVDFParams *params, TfLiteEvalTensor *activation_state_tensor,
                              TfLiteEvalTensor *output_tensor, const OpDataSvdf &data) {
  ...
  int32_t *scratch_tensor = static_cast<int32_t *>(context->GetScratchBuffer(context, data.scratch_tensor_index));
  int32_t *scratch_output_tensor =
      static_cast<int32_t *>(context->GetScratchBuffer(context, data.scratch_output_tensor_index));
  ...
  // Feature matmul.
  {
    ...
    const int8_t *weight_feature = tflite::micro::GetTensorData<int8_t>(weights_feature_tensor);
    ...
    for (int b = 0; b < n_batch; b++) {
      ...
      for (int r = 0; r < n_filter; r++) {
        ...
        for (int c = 0; c < n_input; c++) {
          dot_prod += *matrix_ptr++ * (*vector_in_batch++ - data.input_zero_point);
        }
        dot_prod = MultiplyByQuantizedMultiplier(dot_prod, data.effective_scale_1_a, data.effective_scale_1_b);
  ...
  // Time.
  {
    for (int b = 0; b < n_batch; ++b) {
      ...
      const T *vector1_ptr = tflite::micro::GetTensorData<T>(weights_time_tensor);
      const T *vector2_ptr = tflite::micro::GetTensorData<T>(activation_state_tensor) + b * n_memory * n_filter;

      for (int i = 0; i < n_filter; i++) {
        *scratch_ptr_batch = 0;
        for (int j = 0; j < n_memory; j++) {
          *scratch_ptr_batch += *vector1_ptr++ * (*vector2_ptr++ - data.activation_state_zero_point);
        }
        scratch_ptr_batch++;
      }
    }
  ...
  // Reduce, add bias, rescale, activation.
  {
    ...
    if (bias_tensor) {
      // Vector batch assign:
      const int32_t *bias_data = tflite::micro::GetTensorData<int32_t>(bias_tensor);
    ...
    for (int i = 0; i < n_batch * n_unit; ++i) {
      ...
      int32_t x2 = MultiplyByQuantizedMultiplier(x1, data.effective_scale_2_a, data.effective_scale_2_b);
      int32_t x3 = x2 + data.output_zero_point;
      int32_t x4 = std::min(std::max(output_min, x3), output_max);
      tflite::micro::GetTensorData<int8_t>(output_tensor)[i] = static_cast<int8_t>(x4);
    }
  }
}
...
void EvalInt16SvdfReference(TfLiteContext *context, TfLiteNode *node, const TfLiteEvalTensor *input_tensor,
  ...
  EvalIntegerSvdfReference<int16_t>(context, node, input_tensor, weights_feature_tensor, weights_time_tensor,
...
void EvalInt8SvdfReference(TfLiteContext *context, TfLiteNode *node, const TfLiteEvalTensor *input_tensor,
                           ...
                           TfLiteEvalTensor *activation_state_tensor, TfLiteEvalTensor *output_tensor,
                           const OpDataSvdf &data) {
  EvalIntegerSvdfReference<int8_t>(context, node, input_tensor, weights_feature_tensor, weights_time_tensor,
                                   bias_tensor, params, activation_state_tensor, output_tensor, data);
}

static inline void ApplyTimeWeightsBiasAndActivation(int batch_size, int memory_size, int num_filters, int num_units,
                                                     int rank, const float *const weights_time_ptr,
                                                     const float *const bias_ptr, TfLiteFusedActivation activation,
                                                     float *const state_ptr, float *const scratch_ptr,
  ...
  for (int b = 0; b < batch_size; ++b) {
    float *output_ptr_batch = output_ptr + b * num_units;
    for (int i = 0; i < num_units; ++i) {
      *output_ptr_batch = tflite::ops::micro::ActivationValFloat(activation, *output_ptr_batch);
      ++output_ptr_batch;
    }
  }
}
...
void EvalFloatSvdfReference(TfLiteContext *context, TfLiteNode *node, const TfLiteEvalTensor *input,
  ...
  const float *weights_feature_ptr = tflite::micro::GetTensorData<float>(weights_feature);
  const float *weights_time_ptr = tflite::micro::GetTensorData<float>(weights_time);
  ...
  float *scratch_ptr = static_cast<float *>(context->GetScratchBuffer(context, scratch_tensor_index));
  ...
  // Left shift the activation_state.
  {
    ...
    const float *old_state_end = state_ptr + batch_size * num_filters * memory_size;
    while (old_state_start != old_state_end) {
      *new_state_start++ = *old_state_start++;
    }
  }
  ...
  ApplyTimeWeightsBiasAndActivation(batch_size, memory_size, num_filters, num_units, rank, weights_time_ptr, bias_ptr,
                                    params->activation, state_ptr, scratch_ptr, output_ptr);
}
...
TfLiteStatus PrepareSvdf(TfLiteContext *context, TfLiteNode *node) {
  ...
  TfLiteTensor *input = micro_context->AllocateTempInputTensor(node, kSvdfInputTensor);
  TF_LITE_ENSURE(context, input != nullptr);
  TfLiteTensor *weights_feature = micro_context->AllocateTempInputTensor(node, kSvdfWeightsFeatureTensor);
  TF_LITE_ENSURE(context, weights_feature != nullptr);
  TfLiteTensor *weights_time = micro_context->AllocateTempInputTensor(node, kSvdfWeightsTimeTensor);
  TF_LITE_ENSURE(context, weights_time != nullptr);
  TfLiteTensor *bias = micro_context->AllocateTempInputTensor(node, kSvdfBiasTensor);
  TfLiteTensor *activation_state = micro_context->AllocateTempInputTensor(node, kSvdfInputActivationStateTensor);
  ...
  TfLiteTensor *output = micro_context->AllocateTempOutputTensor(node, kSvdfOutputTensor);
  ...
  TF_LITE_ENSURE_EQ(context, activation_state->dims->data[1], memory_size * num_filters);
  ...
  if (input->type == kTfLiteInt8) {
    ...
    TF_LITE_ENSURE(context, (weights_time->type == kTfLiteInt16) || (weights_time->type == kTfLiteInt8));
    TF_LITE_ENSURE(context, (activation_state->type == kTfLiteInt16) || (activation_state->type == kTfLiteInt8));
    if (bias != nullptr) {
    ...
    const double effective_scale_1 =
        static_cast<double>(input->params.scale * weights_feature->params.scale / activation_state->params.scale);
    const double effective_scale_2 =
        static_cast<double>(activation_state->params.scale * weights_time->params.scale / output->params.scale);

    // TODO(b/162018098): Use TF_LITE_ENSURE_NEAR when it is ready.
    // TODO(#1751): account for optional bias tensor
    TF_LITE_ENSURE(context,
                   std::abs(static_cast<double>(bias->params.scale) -
                            static_cast<double>(activation_state->params.scale * weights_time->params.scale)) < 1e-5);

    QuantizeMultiplier(effective_scale_1, &(data->effective_scale_1_a), &(data->effective_scale_1_b));
    QuantizeMultiplier(effective_scale_2, &(data->effective_scale_2_a), &(data->effective_scale_2_b));
    ...
    const TfLiteStatus scratch_status = context->RequestScratchBufferInArena(
        context, batch_size * num_filters * sizeof(int32_t), &(data->scratch_tensor_index));
    TF_LITE_ENSURE_OK(context, scratch_status);

    const TfLiteStatus scratch_output_status = context->RequestScratchBufferInArena(
        context, batch_size * num_units * sizeof(int32_t), &(data->scratch_output_tensor_index));
    ...
    TFLITE_DCHECK(context->RequestScratchBufferInArena != nullptr);
    const TfLiteStatus scratch_status = context->RequestScratchBufferInArena(
        context, batch_size * num_filters * sizeof(float), &(data->scratch_tensor_index));
    TF_LITE_ENSURE_OK(context, scratch_status);
  }
