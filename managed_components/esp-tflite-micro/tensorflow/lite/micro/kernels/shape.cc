...
TfLiteStatus ShapeEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (output->type != kTfLiteInt32) {
    MicroPrintf("Output type %s (%d) not supported.", TfLiteTypeGetName(output->type), output->type);
    return kTfLiteError;
  } else {
...
TFLMRegistration Register_SHAPE() { return tflite::micro::RegisterOp(nullptr, ShapePrepare, ShapeEval); }
