...
tflite::FullyConnectedParams CreateFCParams(const TensorQuantizationParameters &input_quant_params,
  ...
  double effective_scale = input_product_scale / static_cast<double>(nonlinear_activation_input_scale);
...
inline tflite::FullyConnectedParams CreateFCParamsFloat() {
  ...
  CalculateActivationRange(kTfLiteActNone, &op_params.float_activation_min, &op_params.float_activation_max);
  return op_params;
}

// Wrapper function to create gate parameters for the four internal LSTM gates
template<typename CellType>
tflite::GateParameters CreateGateParams(const TensorQuantizationParameters &input_quant_params,
  ...
  gate_params.input_fc_params = CreateFCParams<CellType>(
      input_quant_params, gate_quantization_settings.activation_weight, nonlinear_activation_input_scale);
  gate_params.recurrent_fc_params = CreateFCParams<CellType>(
      hidden_state_quant_params, gate_quantization_settings.recurrent_weight, nonlinear_activation_input_scale);
  return gate_params;
}
...
tflite::ArithmeticParams CreateInterGateMulParams(const float input1_scale, const float input2_scale,
  ...
  const double input_product_scale = static_cast<double>(input1_scale) * static_cast<double>(input2_scale);
...
CellStateInfo CreateLstmCellStateInfo(const float cell_state_scale, const float cell_clip) {
  ...
  cell_state_info.quantized_cell_clip = static_cast<int16_t>(
      std::min(std::max(static_cast<double>(cell_clip) / static_cast<double>(cell_state_scale), -32768.0), 32767.0));
  return cell_state_info;
}

// Create LSTMKernelContents from LstmNodeContent by copying TfLiteEvalTensor
// pointers
template<typename ActivationType, typename WeightType, typename BiasType, typename CellType, int batch_size,
         int time_steps, int input_dimension, int state_dimension>
LSTMKernelContents CreateLSTMKernelContent(
    LstmNodeContent<ActivationType, WeightType, BiasType, CellType, batch_size, time_steps, input_dimension,
                    state_dimension> &node_contents) {
  LSTMKernelContents kernel_content;
  // Point to correct tensors
  kernel_content.internal_tensors[kLstmInputTensor] = node_contents.GetEvalTensor(kLstmInputTensor);
  ...
  kernel_content.internal_tensors[kLstmInputGateBiasTensor] = node_contents.GetEvalTensor(kLstmInputGateBiasTensor);
  kernel_content.internal_tensors[kLstmForgetGateBiasTensor] = node_contents.GetEvalTensor(kLstmForgetGateBiasTensor);
  kernel_content.internal_tensors[kLstmCellGateBiasTensor] = node_contents.GetEvalTensor(kLstmCellGateBiasTensor);
  kernel_content.internal_tensors[kLstmOutputGateBiasTensor] = node_contents.GetEvalTensor(kLstmOutputGateBiasTensor);
  kernel_content.internal_tensors[kLstmOutputStateTensor] = node_contents.GetEvalTensor(kLstmOutputStateTensor);
  kernel_content.internal_tensors[kLstmOutputGateBiasTensor] = node_contents.GetEvalTensor(kLstmOutputGateBiasTensor);
  kernel_content.internal_tensors[kLstmCellStateTensor] = node_contents.GetEvalTensor(kLstmCellStateTensor);
...
// tensor
LstmSizeInfo CreateLstmSizeInfo(const bool time_major, const TfLiteIntArray *input_tensor_shape,
...
template<typename ActivationType, typename WeightType, typename BiasType, typename CellType, int batch_size,
         int time_steps, int input_dimension, int state_dimension>
OpDataLSTM CreateLstmOpData(LstmNodeContent<ActivationType, WeightType, BiasType, CellType, batch_size, time_steps,
  ...
  op_data.size_info = CreateLstmSizeInfo(builtin_data.time_major, node_contents.GetEvalTensor(kLstmInputTensor)->dims,
                                         node_contents.HiddenStateEvalTensor()->dims);

  op_data.cell_state_info = CreateLstmCellStateInfo(quantization_settings.cell_state.scale, builtin_data.cell_clip);

  // Gate Parameters
  op_data.forget_gate_parameters = CreateGateParams<CellType>(
      quantization_settings.input, quantization_settings.hidden_state, quantization_settings.forget_gate,
      quantization_settings.nonlinear_activation_input_scale);
  op_data.input_gate_parameters = CreateGateParams<CellType>(
      quantization_settings.input, quantization_settings.hidden_state, quantization_settings.input_gate,
      quantization_settings.nonlinear_activation_input_scale);
  op_data.cell_gate_parameters = CreateGateParams<CellType>(
      quantization_settings.input, quantization_settings.hidden_state, quantization_settings.cell_gate,
      quantization_settings.nonlinear_activation_input_scale);
  op_data.output_gate_parameters = CreateGateParams<CellType>(
      quantization_settings.input, quantization_settings.hidden_state, quantization_settings.output_gate,
      quantization_settings.nonlinear_activation_input_scale);
  ...
  op_data.inter_gate_parameters.forget_cell_mul_params = CreateInterGateMulParams<CellType>(
      quantization_settings.nonlinear_activation_output_scale, quantization_settings.cell_state.scale,
      quantization_settings.cell_state.scale);
  op_data.inter_gate_parameters.input_mul_params = CreateInterGateMulParams<CellType>(
      quantization_settings.nonlinear_activation_output_scale, quantization_settings.nonlinear_activation_output_scale,
      quantization_settings.cell_state.scale);
...
OpDataLSTM CreateLstmOpDataFloat(LstmNodeContent<float, float, float, float, batch_size, time_steps, input_dimension,
                                                 state_dimension> &node_contents) {
  const auto &builtin_data = node_contents.BuiltinData();
  OpDataLSTM op_data;
...
template<typename T>
void ValidateResultGoldens(const T *golden, const T *output_data, const int output_len, const float tolerance) {
...
void TestCalculateLstmGateFloat(const TfLiteEvalTensor *input, const TfLiteEvalTensor *input_weight,
  ...
  tflite::lstm_internal::CalculateLstmGate<float, float, float, float>(step_info, gate_params,
                                                                       // Input FC
...
template<typename ActivationType, typename WeightType, typename BiasType, typename CellType, int batch_size,
         int state_dimension>
void TestCalculateLstmGateInteger(const TfLiteEvalTensor *input, const TfLiteEvalTensor *input_weight,
                                  const TfLiteEvalTensor *input_bias,
                                  // Recurrent FC
                                  const TfLiteEvalTensor *recurrent, const TfLiteEvalTensor *recurrent_weight,
                                  const TfLiteEvalTensor *recurrent_bias,
  ...
  tflite::GateParameters gate_params = CreateGateParams<CellType>(
      node_quantization_settings.input, node_quantization_settings.hidden_state, gate_quantization_settings,
  ...
  Dequantize(gate_output, batch_size * state_dimension, node_quantization_settings.nonlinear_activation_output_scale, 0,
             gate_output_float);

  ValidateResultGoldens(expected_vals, gate_output_float, batch_size * state_dimension, tolerance);
}

template<int batch_size, int time_steps, int input_dimension, int state_dimension>
void TestUpdateLstmCellFloat(
    const GateOutputCheckData<batch_size * input_dimension, batch_size * state_dimension> &gate_output_data,
    LstmNodeContent<float, float, float, float, batch_size, time_steps, input_dimension, state_dimension> &node_content,
  ...
  std::memcpy(forget_gate, gate_output_data.expected_forget_gate_output, batch_size * state_dimension * sizeof(float));
  ...
  tflite::lstm_internal::UpdateLstmCell<float>(
      ...
      gate_output_data.expected_cell_gate_output, forget_cell_mul_params, input_mul_params, cell_state_info, buffer);

  ValidateResultGoldens(gate_output_data.expected_updated_cell, tflite::micro::GetTensorData<float>(cell_state),
...
void TestUpdateLstmCellInteger(
  ...
  tflite::Quantize(gate_output_data.expected_forget_gate_output, quantized_forget_gate, batch_size * state_dimension,
                   quantization_settings.nonlinear_activation_output_scale, 0);

  CellType quantized_input_gate[batch_size * state_dimension] = {};
  tflite::Quantize(gate_output_data.expected_input_gate_output, quantized_input_gate, batch_size * state_dimension,
                   quantization_settings.nonlinear_activation_output_scale, 0);

  CellType quantized_cell_gate[batch_size * state_dimension] = {};
  tflite::Quantize(gate_output_data.expected_cell_gate_output, quantized_cell_gate, batch_size * state_dimension,
                   quantization_settings.nonlinear_activation_output_scale, 0);
  ...
  auto forget_cell_mul_params = CreateInterGateMulParams<CellType>(
      quantization_settings.nonlinear_activation_output_scale, quantization_settings.cell_state.scale,
      quantization_settings.cell_state.scale);
  auto input_mul_params = CreateInterGateMulParams<CellType>(quantization_settings.nonlinear_activation_output_scale,
                                                             quantization_settings.nonlinear_activation_output_scale,
                                                             quantization_settings.cell_state.scale);

  auto cell_state_info =
      CreateLstmCellStateInfo(quantization_settings.cell_state.scale, node_content.BuiltinData().cell_clip);
  ...
  // Call the function to be tested
  tflite::lstm_internal::UpdateLstmCell<CellType>(step_info, cell_state, quantized_forget_gate, quantized_input_gate,
  ...
  Dequantize(tflite::micro::GetTensorData<CellType>(cell_state), batch_size * state_dimension,
             quantization_settings.cell_state.scale, quantization_settings.cell_state.zero_point, cell_state_float);

  ValidateResultGoldens(gate_output_data.expected_updated_cell, cell_state_float, batch_size * state_dimension,
                        tolerance);
}
...
void TestUpdateLstmHiddenFloat(
  ...
  tflite::lstm_internal::UpdateLstmHidden<float, float>(step_info, cell_state, hidden_state,
                                                        gate_output_data.expected_output_gate_output, mul_params,
                                                        cell_state_scale_power, buffer);

  ValidateResultGoldens(gate_output_data.expected_updated_hidden, tflite::micro::GetTensorData<float>(hidden_state),
                        batch_size * state_dimension, tolerance);
}
...
void TestUpdateLstmHiddenInteger(
  ...
  tflite::Quantize(gate_output_data.expected_output_gate_output, quantized_output_gate, batch_size * state_dimension,
                   quantization_settings.nonlinear_activation_output_scale, 0);
  ...
  tflite::lstm_internal::UpdateLstmHidden<CellType, ActivationType>(
      step_info, cell_state, hidden_state, quantized_output_gate, mul_params, cell_state_scale_power, buffer);

  float hidden_state_float[batch_size * state_dimension] = {};
  Dequantize(tflite::micro::GetTensorData<ActivationType>(hidden_state), batch_size * state_dimension,
             quantization_settings.hidden_state.scale, quantization_settings.hidden_state.zero_point,
             hidden_state_float);

  ValidateResultGoldens(gate_output_data.expected_updated_hidden, hidden_state_float, batch_size * state_dimension,
                        tolerance);
}
...
void TestLstmStepFloat(
    ...
    LstmNodeContent<float, float, float, float, batch_size, time_steps, input_dimension, state_dimension>
        &node_contents) {
  ...
  tflite::lstm_internal::LstmStep<float, float, float, float>(step_info, op_data, kernel_content, buffers);
...
void TestLstmStepInteger(
  ...
  op_data.size_info.time_major = true;
  tflite::lstm_internal::LstmStepManager step_info(&op_data.size_info);
  tflite::lstm_internal::LstmStep<ActivationType, WeightType, CellType, BiasType>(step_info, op_data, kernel_content,
                                                                                  buffers);

  const auto &quantization_settings = node_contents.QuantizationSettings();
  float dequantized_hidden_state[batch_size * state_dimension] = {};
  Dequantize(tflite::micro::GetTensorData<ActivationType>(kernel_content.HiddenStateTensor()),
  ...
  Dequantize(tflite::micro::GetTensorData<CellType>(kernel_content.CellStateTensor()), batch_size * state_dimension,
             quantization_settings.cell_state.scale, quantization_settings.cell_state.zero_point,
             dequantized_cell_state);

  ValidateResultGoldens(gate_output_data.expected_updated_hidden, dequantized_hidden_state,
                        batch_size * state_dimension, hidden_state_tolerance);
  ValidateResultGoldens(gate_output_data.expected_updated_cell, dequantized_cell_state, batch_size * state_dimension,
                        cell_state_tolerance);
}

template<int batch_size, int time_steps, int input_dimension, int state_dimension>
void TestEvalLstmFloat(const LstmEvalCheckData<batch_size * time_steps * input_dimension, batch_size * state_dimension,
                                               batch_size * state_dimension * time_steps> &eval_check_data,
                       const float hidden_state_tolerance, const float cell_state_tolerance,
                       LstmNodeContent<float, float, float, float, batch_size, time_steps, input_dimension,
                                       state_dimension> &node_contents) {
  ...
  ValidateResultGoldens(eval_check_data.expected_hidden_state, node_contents.GetHiddenStateData(),
                        batch_size * state_dimension, hidden_state_tolerance);

  ValidateResultGoldens(eval_check_data.expected_cell_state, node_contents.GetCellStateData(),
                        batch_size * state_dimension, cell_state_tolerance);

  ValidateResultGoldens(eval_check_data.expected_output, node_contents.GetOutputData(), batch_size * state_dimension,
...
void TestEvalLstmInteger(
    const LstmEvalCheckData<batch_size * time_steps * input_dimension, batch_size * state_dimension,
                            batch_size * state_dimension * time_steps> &eval_check_data,
    const float hidden_state_tolerance, const float cell_state_tolerance,
    LstmNodeContent<ActivationType, WeightType, BiasType, CellType, batch_size, time_steps, input_dimension,
                    state_dimension> &node_contents) {
  ...
  tflite::EvalLstm<ActivationType, WeightType, CellType, BiasType>(op_data, kernel_content, buffers);

  const auto &quantization_settings = node_contents.QuantizationSettings();
  float dequantized_hidden_state[batch_size * state_dimension] = {};
  Dequantize(node_contents.GetHiddenStateData(), batch_size * state_dimension, quantization_settings.hidden_state.scale,
             quantization_settings.hidden_state.zero_point, dequantized_hidden_state);

  ValidateResultGoldens(eval_check_data.expected_hidden_state, dequantized_hidden_state, batch_size * state_dimension,
                        hidden_state_tolerance);

  float dequantized_cell_state[batch_size * state_dimension] = {};
  Dequantize(node_contents.GetCellStateData(), batch_size * state_dimension, quantization_settings.cell_state.scale,
             quantization_settings.cell_state.zero_point, dequantized_cell_state);
  ValidateResultGoldens(eval_check_data.expected_cell_state, dequantized_cell_state, batch_size * state_dimension,
                        cell_state_tolerance);

  float dequantized_output[batch_size * state_dimension * time_steps] = {};
  Dequantize(node_contents.GetOutputData(), batch_size * state_dimension * time_steps,
             quantization_settings.output.scale, quantization_settings.output.zero_point, dequantized_output);
  ValidateResultGoldens(eval_check_data.expected_output, dequantized_output, batch_size * state_dimension,
                        hidden_state_tolerance);
}
