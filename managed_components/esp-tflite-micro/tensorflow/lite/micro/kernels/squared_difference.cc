...
template<typename T> T SquaredDifference(T input1, T input2) {
...
void PrepareQuantized(const TfLiteQuantizationParams &input1_quantization_params,
                      const TfLiteQuantizationParams &input2_quantization_params,
                      const TfLiteQuantizationParams &output_quantization_params, const int left_shift,
                      const int32_t quantized_activation_min, const int32_t quantized_activation_max, OpData *data) {
  data->arithmetic_params.input1_offset = -input1_quantization_params.zero_point;
  data->arithmetic_params.input2_offset = -input2_quantization_params.zero_point;
  ...
  const double twice_max_input_scale =
      2.0 * static_cast<double>(std::max(input1_quantization_params.scale, input2_quantization_params.scale));
  const double real_input1_multiplier = static_cast<double>(input1_quantization_params.scale) / twice_max_input_scale;
  double real_input2_multiplier = static_cast<double>(input2_quantization_params.scale) / twice_max_input_scale;
  const double real_output_multiplier =
      (twice_max_input_scale * twice_max_input_scale) /
      static_cast<double>((1 << data->arithmetic_params.left_shift * 2) * output_quantization_params.scale);
  QuantizeMultiplierSmallerThanOneExp(real_input1_multiplier, &data->arithmetic_params.input1_multiplier,
                                      &data->arithmetic_params.input1_shift);
  QuantizeMultiplierSmallerThanOneExp(real_input2_multiplier, &data->arithmetic_params.input2_multiplier,
                                      &data->arithmetic_params.input2_shift);
  QuantizeMultiplier(real_output_multiplier, &data->arithmetic_params.output_multiplier,
...
TfLiteStatus SquaredDifferencePrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (input1->type == kTfLiteInt8) {
    ...
    TF_LITE_ENSURE(context, input1_quantization_params.zero_point >= integer_type_min);
    TF_LITE_ENSURE(context, input1_quantization_params.zero_point <= integer_type_max);
    TF_LITE_ENSURE(context, input2_quantization_params.zero_point >= integer_type_min);
    TF_LITE_ENSURE(context, input2_quantization_params.zero_point <= integer_type_max);
    TF_LITE_ENSURE(context, output_quantization_params.zero_point >= integer_type_min);
    TF_LITE_ENSURE(context, output_quantization_params.zero_point <= integer_type_max);
    // leftshift = 7 is selected so that maximum shifted result 255^2 * (1 << (7
    // * 2 )) does not overflow signed 32-bit integer
    PrepareQuantized(input1_quantization_params, input2_quantization_params, output_quantization_params,
                     /*left_shift=*/7,
...
template<typename T> T SquaredDifference(T x, T y, const ArithmeticParams &params) {
  ...
  const int32_t scaled_input2_val =
      MultiplyByQuantizedMultiplierSmallerThanOneExp(shifted_input2_val, params.input2_multiplier, params.input2_shift);
  ...
  const int32_t raw_output =
      MultiplyByQuantizedMultiplier(squared_raw_diff, params.output_multiplier, params.output_shift) +
...
template<typename T>
void EvalQuantizedSquaredDifference(TfLiteContext *context, TfLiteNode *node, const OpData *data,
  ...
  if (data->requires_broadcast) {
    reference_integer_ops::BroadcastBinaryFunction4DSlow(
        op_data->arithmetic_params, tflite::micro::GetTensorShape(input1), tflite::micro::GetTensorData<T>(input1),
        tflite::micro::GetTensorShape(input2), tflite::micro::GetTensorData<T>(input2),
    ...
    reference_integer_ops::ElementWise(flat_size, op_data->arithmetic_params, tflite::micro::GetTensorData<T>(input1),
                                       tflite::micro::GetTensorData<T>(input2), tflite::micro::GetTensorData<T>(output),
                                       reference_integer_ops::CheckArithmeticParams, SquaredDifference);
  }
}

template<typename T>
void EvalSquaredDifference(TfLiteContext *context, TfLiteNode *node, const OpData *data, const TfLiteEvalTensor *input1,
                           const TfLiteEvalTensor *input2, TfLiteEvalTensor *output) {
  if (data->requires_broadcast) {
    reference_ops::BroadcastBinaryFunction4DSlow<T, T, T>(
        tflite::micro::GetTensorShape(input1), tflite::micro::GetTensorData<T>(input1),
        tflite::micro::GetTensorShape(input2), tflite::micro::GetTensorData<T>(input2),
        tflite::micro::GetTensorShape(output), tflite::micro::GetTensorData<T>(output), SquaredDifference<T>);
  } else {
...
TfLiteStatus SquaredDifferenceEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (output->type == kTfLiteFloat32) {
    ...
    EvalQuantizedSquaredDifference<int8_t>(context, node, data, input1, input2, output);
  } else if (output->type == kTfLiteInt16) {
    EvalQuantizedSquaredDifference<int16_t>(context, node, data, input1, input2, output);
  } else {
    MicroPrintf("SquaredDifference only supports FLOAT32, INT32 , INT16 and INT8 now, "
                "got %d.",
                output->type);
    return kTfLiteError;
  }
...
TFLMRegistration Register_SQUARED_DIFFERENCE() {
  return tflite::micro::RegisterOp(SquaredDifferenceInit, SquaredDifferencePrepare, SquaredDifferenceEval);
}
