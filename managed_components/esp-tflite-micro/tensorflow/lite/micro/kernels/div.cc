...
TfLiteStatus CalculateOpDataDiv(TfLiteContext *context, TfLiteTensor *input1, TfLiteTensor *input2,
                                TfLiteTensor *output, TfLiteDivParams *params, OpDataDiv *data) {
  ...
  if (output->type == kTfLiteInt8) {
    ...
    const double real_multiplier =
        static_cast<double>(input1->params.scale / (input2->params.scale * output->params.scale));
...
TfLiteStatus DivPrepare(TfLiteContext *context, TfLiteNode *node) {
  ...
  TF_LITE_ENSURE_STATUS(CalculateOpDataDiv(context, input1, input2, output, params, data));
...
template<typename T>
void EvalDiv(TfLiteContext *context, TfLiteNode *node, TfLiteDivParams *params, const OpDataDiv *data,
...
#define TF_LITE_DIV(type, opname, data_type) \
  ...
  type::opname(op_params, tflite::micro::GetTensorShape(input1), tflite::micro::GetTensorData<data_type>(input1), \
               tflite::micro::GetTensorShape(input2), tflite::micro::GetTensorData<data_type>(input2), \
               tflite::micro::GetTensorShape(output), tflite::micro::GetTensorData<data_type>(output))

  bool requires_broadcast = reference_ops::ProcessBroadcastShapes(tflite::micro::GetTensorShape(input1),
...
TfLiteStatus EvalQuantized(TfLiteContext *context, TfLiteNode *node, TfLiteDivParams *params, const OpDataDiv *data,
                           const TfLiteEvalTensor *input1, const TfLiteEvalTensor *input2, TfLiteEvalTensor *output) {
  tflite::ArithmeticParams op_params = {};

#define TF_LITE_DIV(type, opname, dtype) \
  type::opname(op_params, tflite::micro::GetTensorShape(input1), tflite::micro::GetTensorData<dtype>(input1), \
               tflite::micro::GetTensorShape(input2), tflite::micro::GetTensorData<dtype>(input2), \
               tflite::micro::GetTensorShape(output), tflite::micro::GetTensorData<dtype>(output))

  if (input1->type == kTfLiteInt8 && input2->type == kTfLiteInt8 && output->type == kTfLiteInt8) {
...
TfLiteStatus DivEval(TfLiteContext *context, TfLiteNode *node) {
  ...
  if (output->type == kTfLiteFloat32) {
    ...
    TF_LITE_ENSURE_OK(context, EvalQuantized(context, node, params, data, input1, input2, output));
  } else {
    MicroPrintf("DIV only supports FLOAT32, INT32, quantized INT8 "
                "now, got type %s (%d).",
...
TFLMRegistration Register_DIV() { return tflite::micro::RegisterOp(DivInit, DivPrepare, DivEval); }
