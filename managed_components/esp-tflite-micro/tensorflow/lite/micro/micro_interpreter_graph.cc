...
TfLiteStatus MicroInterpreterGraph::InitSubgraphs() {
  ...
  for (size_t subgraph_idx = 0; subgraph_idx < subgraphs_->size(); subgraph_idx++) {
    ...
    for (size_t i = 0; i < operators_size; ++i) {
      TfLiteNode *node = &(subgraph_allocations_[subgraph_idx].node_and_registrations[i].node);
      const TFLMRegistration *registration = subgraph_allocations_[subgraph_idx].node_and_registrations[i].registration;
    ...
    for (size_t i = 0; i < operators_size; ++i) {
      ...
      if (registration->prepare != nullptr) {
        ...
        if (prepare_status != kTfLiteOk) {
          MicroPrintf("Node %s (number %df) failed to prepare with status %d", OpNameFromRegistration(registration), i,
                      prepare_status);
          return kTfLiteError;
        }
      }
...
// only defined for builds with the error strings.
    ...
    if (invoke_status == kTfLiteError) {
      MicroPrintf("Node %s (number %d) failed to invoke with status %d", OpNameFromRegistration(registration), i,
                  invoke_status);
      return kTfLiteError;
