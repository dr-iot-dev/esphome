...
TfLiteStatus GreedyMemoryPlanner::Init(unsigned char *scratch_buffer, int scratch_buffer_size) {
...
TfLiteStatus GreedyMemoryPlanner::AddBuffer(int size, int first_time_used, int last_time_used) {
...
TfLiteStatus GreedyMemoryPlanner::AddBuffer(int size, int first_time_used, int last_time_used, int offline_offset) {
  BufferRequirements *current = &requirements_[buffer_count_];
...
bool GreedyMemoryPlanner::DoesEntryOverlapInTime(const GreedyMemoryPlanner::ListEntry *entry, const int first_time_used,
...
GreedyMemoryPlanner::ListEntry *GreedyMemoryPlanner::NextSimultaneouslyActiveBuffer(
    const GreedyMemoryPlanner::ListEntry *start, const int first_time_used, const int last_time_used) {
...
void GreedyMemoryPlanner::CalculateOffsetsIfNeeded() {
  ...
  ReverseSortInPlace(&buffer_sizes_sorted_[idx_from_head], &buffer_ids_sorted_[idx_from_head],
    ...
    if (wanted_requirements->offline_offset == kOnlinePlannedBuffer) {
      ...
      while (true) {
        ...
        ListEntry *next_entry =
            NextSimultaneouslyActiveBuffer(prior_entry, wanted_first_time_used, wanted_last_time_used);

        if (prior_entry) {
          BufferRequirements *candidate_requirements = &requirements_[prior_entry->requirements_index];
...
void GreedyMemoryPlanner::PrintMemoryPlan() {
  ...
  for (int i = 0; i < buffer_count_; ++i) {
    MicroPrintf("%c (id=%d): size=%d, offset=%d, first_used=%d last_used=%d", GetOrdinalCharacter(i), i,
                requirements_[i].size, buffer_offsets_[i], requirements_[i].first_time_used,
  ...
  for (int t = 0; t <= max_time; ++t) {
    ...
    MicroPrintf("%s%d: %s (%dk)", t < 10 ? " " : "", t, (const char *) line, (memory_use + 1023) / 1024);
  }
}

int GreedyMemoryPlanner::GetBufferCount() { return buffer_count_; }

TfLiteStatus GreedyMemoryPlanner::GetOffsetForBuffer(int buffer_index, int *offset) {
  CalculateOffsetsIfNeeded();
  if ((buffer_index < 0) || (buffer_index >= buffer_count_)) {
    MicroPrintf("buffer index %d is outside range 0 to %d", buffer_index, buffer_count_);
    return kTfLiteError;
  }
...
bool GreedyMemoryPlanner::DoAnyBuffersOverlap() {
  ...
  for (int i = 0; i < buffer_count_; ++i) {
    ...
    for (int j = 0; j < buffer_count_; ++j) {
      ...
      if ((a_first_time_used > b_last_time_used) || (b_first_time_used > a_last_time_used)) {
        // Buffers don't overlap in time.
        continue;
      }
      if ((a_start_offset >= b_end_offset) || (b_start_offset >= a_end_offset)) {
        // No overlap in memory.
        continue;
      }
      were_overlaps_found = true;
      MicroPrintf("Overlap: %d (%d=>%d, %d->%d) vs %d (%d=>%d, %d->%d)", i, a_first_time_used, a_last_time_used,
                  a_start_offset, a_end_offset, j, b_first_time_used, b_last_time_used, b_start_offset, b_end_offset);
    }
  }
