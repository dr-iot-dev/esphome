...
MicroResourceVariables *MicroResourceVariables::Create(MicroAllocator *allocator, int max_num_variables) {
  TFLITE_DCHECK(allocator != nullptr);

  uint8_t *allocator_buffer =
      static_cast<uint8_t *>(allocator->AllocatePersistentBuffer(sizeof(MicroResourceVariables)));
  MicroResourceVariable *variable_array = static_cast<MicroResourceVariable *>(
      allocator->AllocatePersistentBuffer(sizeof(MicroResourceVariable) * max_num_variables));
  MicroResourceVariables *variables = new (allocator_buffer) MicroResourceVariables(variable_array, max_num_variables);
  return variables;
}

int MicroResourceVariables::CreateIdIfNoneFound(const char *container, const char *shared_name) {
  ...
  if (num_resource_variables_ >= max_variable_count_) {
    MicroPrintf("Failed to allocate resource variable. Maximum resource variable count "
                "(%d) "
                "reached.",
                max_variable_count_);
    return -1;
...
TfLiteStatus MicroResourceVariables::Read(int id, const TfLiteEvalTensor *tensor) {
...
TfLiteStatus MicroResourceVariables::Allocate(int id, TfLiteContext *context, const TfLiteTensor *tensor) {
  if (id < 0 || id >= num_resource_variables_) {
    MicroPrintf("Attempting to read non-existent resource variable %d", id);
    return kTfLiteError;
  }
  ...
  if (variable.resource_buffer == nullptr) {
    ...
    variable.resource_buffer = context->AllocatePersistentBuffer(context, tensor->bytes);
    if (variable.resource_buffer == nullptr) {
    ...
    if (tensor->quantization.params != nullptr) {
      auto *quantization_data = reinterpret_cast<TfLiteAffineQuantization *>(tensor->quantization.params);
...
TfLiteStatus MicroResourceVariables::Assign(int id, const TfLiteEvalTensor *tensor) {
  if (id < 0 || id >= num_resource_variables_) {
    MicroPrintf("Attempting to read non-existent resource variable %d", id);
    return kTfLiteError;
  }
  ...
  if (variable.resource_buffer == nullptr) {
    MicroPrintf("Attempting to assign from a TfLiteEvalTensor before the resource "
...
int MicroResourceVariables::FindId(const char *container, const char *shared_name) {
  for (int i = 0; i < num_resource_variables_; i++) {
    // Some TFLite flatbuffers contain null container names to save space.
    if ((container == nullptr || !strcmp(container, resource_variables_[i].container)) &&
