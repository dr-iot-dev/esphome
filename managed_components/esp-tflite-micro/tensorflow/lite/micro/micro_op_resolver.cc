...
TfLiteStatus GetRegistrationFromOpCode(const OperatorCode *opcode, const MicroOpResolver &op_resolver,
  ...
  if (builtin_code > BuiltinOperator_MAX) {
    ...
    *registration = op_resolver.FindOp(builtin_code);
    if (*registration == nullptr) {
      MicroPrintf("Didn't find op for builtin opcode '%s'", EnumNameBuiltinOperator(builtin_code));
      status = kTfLiteError;
    }
