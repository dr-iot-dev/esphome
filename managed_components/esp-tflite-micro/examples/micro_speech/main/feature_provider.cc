...
TfLiteStatus FeatureProvider::PopulateFeatureData(int32_t last_time_in_ms, int32_t time_in_ms,
  ...
  *how_many_new_slices = slices_needed;
  ...
  if (slices_needed > 0) {
    for (int new_slice = slices_to_keep; new_slice < kFeatureCount; ++new_slice) {
      ...
      GetAudioSamples((slice_start_ms > 0 ? slice_start_ms : 0), kFeatureDurationMs, &audio_samples_size,
                      &audio_samples);
