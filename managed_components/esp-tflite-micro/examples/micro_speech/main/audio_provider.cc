...
#define NO_I2S_SUPPORT \
  CONFIG_IDF_TARGET_ESP32C2 || (CONFIG_IDF_TARGET_ESP32C3 && (ESP_IDF_VERSION < ESP_IDF_VERSION_VAL(4, 4, 0)))
...
static void CaptureSamples(void *arg) {
  ...
  while (1) {
    ...
    if (bytes_read <= 0) {
      ...
      int bytes_written =
          rb_write(g_audio_capture_buffer, (uint8_t *) g_i2s_read_buffer, bytes_read, pdMS_TO_TICKS(100));
...
TfLiteStatus GetAudioSamples(int start_ms, int duration_ms, int *audio_samples_size, int16_t **audio_samples) {
  ...
  int bytes_read = rb_read(g_audio_capture_buffer, ((uint8_t *) (g_audio_output_buffer + history_samples_to_keep)),
  ...
  if (bytes_read < 0) {
    ...
    ESP_LOGV(TAG, " Could only read %d bytes when required %d bytes ", bytes_read,
