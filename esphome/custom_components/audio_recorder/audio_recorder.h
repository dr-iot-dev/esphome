...
enum AudioRecorderFeature : uint32_t {
  FEATURE_audio_recorder = 1 << 0,
  FEATURE_SPEAKER = 1 << 1,
...
class AudioRecorder : public Component {
 public:
  ...
  uint32_t get_feature_flags() const {
    uint32_t flags = 0;
    flags |= AudioRecorderFeature::FEATURE_audio_recorder;
    flags |= AudioRecorderFeature::FEATURE_API_AUDIO;
#ifdef USE_SPEAKER
    if (this->speaker_ != nullptr) {
      flags |= AudioRecorderFeature::FEATURE_SPEAKER;
    }
#endif

    if (this->has_timers_) {
      flags |= AudioRecorderFeature::FEATURE_TIMERS;
    }
  ...
  void on_event(const api::AudioRecorderEventResponse &msg);
  void on_audio(const api::AudioRecorderAudio &msg);
  void on_timer_event(const api::AudioRecorderTimerEventResponse &msg);
...
template<typename... Ts> class StartAction : public Action<Ts...>, public Parented<AudioRecorder> {
...
template<typename... Ts> class StartContinuousAction : public Action<Ts...>, public Parented<AudioRecorder> {
 public:
  void play(Ts... x) override { this->parent_->request_start(true, true); }
};

template<typename... Ts> class StopAction : public Action<Ts...>, public Parented<AudioRecorder> {
 public:
  void play(Ts... x) override { this->parent_->request_stop(); }
};

template<typename... Ts> class IsRunningCondition : public Condition<Ts...>, public Parented<AudioRecorder> {
 public:
  bool check(Ts... x) override { return this->parent_->is_running() || this->parent_->is_continuous(); }
};

template<typename... Ts> class ConnectedCondition : public Condition<Ts...>, public Parented<AudioRecorder> {
 public:
  bool check(Ts... x) override { return this->parent_->get_api_connection() != nullptr; }
};

extern AudioRecorder *global_audio_recorder;  // NOLINT(cppcoreguidelines-avoid-non-const-global-variables)

}  // namespace audio_recorder
}  // namespace esphome

#endif  // USE_audio_recorder
